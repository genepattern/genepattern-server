...
<%@ page
	import="java.io.IOException,
	        ...
	        org.genepattern.util.GPConstants,org.genepattern.util.StringUtils,
...
<%
	...
	try {
		...
		ArrayList suiteFilterAttr = (ArrayList) request.getSession()
		.getAttribute("suiteSelection");
		...
		for (Iterator itTasks = latestTmTasks.iterator(); itTasks
		.hasNext();) {
			TaskInfo taskInfo = (TaskInfo) itTasks.next();
			TaskInfoAttributes tia = taskInfo.giveTaskInfoAttributes();
			String versionlessLSID = (new LSID(tia
			.get(GPConstants.LSID))).toStringNoVersion();
...
There are currently no tasks installed on this server. You may select
and install tasks from the <a href="taskCatalog.jsp">Broad website</a>
or from your own collection. <br>
<br>
...
<table cellpadding="0" cellspacing="0" width="100%" border="0">
	...
	<tr>
		...
		<td valign='top' align='left' height='100%'><iframe
			frameborder="0" scrolling="yes" marginwidth="1"
			src="getRecentJobs.jsp" style="<%=iFrameWidth%> height: 590px"
			name="iframe" id="iframeid"> No &lt;iframes&gt; support :( </iframe>
		...
		<table bgcolor="#EFEFFF" height='100%' width='100%' border="0"
			margin='1' class='majorcell'>
			<tr height='520'>
				<td valign='top'><%@ include file="indexDocSection.jsp"%>
				</td>
			</tr>
		</table>
...
%>
<%!public String taskCatalog(Collection tmTasks, HashMap latestTaskMap,
			String selectName, String caption, String type, String userID,
			boolean bIncludePipelines) {
		StringBuffer sbCatalog = new StringBuffer();
		sbCatalog.append("<select name=\"" + selectName
				+ "\" onchange=\"taskSelect(this, '" + selectName + "')\">\n");
		...
		for (Iterator itTasks = tmTasks.iterator(); itTasks.hasNext();) {
			...
			try {
				...
				TaskInfo latestTaskInfo = (TaskInfo) latestTaskMap
						.get(versionlessLSID);
				TaskInfoAttributes latestTia = latestTaskInfo
						.giveTaskInfoAttributes();
				lsid = latestTia.get(GPConstants.LSID);

				taskType = latestTia.get(GPConstants.TASK_TYPE);
				if (type != null && !taskType.equals(type))
					continue;
				if (!bIncludePipelines
						&& taskType.equals(GPConstants.TASK_TYPE_PIPELINE))
					continue;
				...
				int halfLength = Integer.parseInt(System.getProperty(
						"gp.name.halflength", "17"));
				String shortenedName = display;
				if (display.length() > ((2 * halfLength) + 3)) {
					int len = display.length();
					int idx = display.length() - halfLength;
					shortenedName = display.substring(0, halfLength) + "..."
							+ display.substring(idx, len);
					display = shortenedName;
				}

				description = latestTaskInfo.getDescription();
				isPublic = latestTia.get(GPConstants.PRIVACY).equals(
						GPConstants.PUBLIC);
				...
				System.out.println("index.jsp: skipping " + mue.getMessage()
						+ " in " + lsid);
				continue;
			}
			...
			if (isPublic || isMine) {
				...
				sbCatalog.append("<option value=\""
						+ (lsid != null ? l.toStringNoVersion() : taskInfo
								.getName()) + "\" class=\"tasks-"
						+ authorityType + "\"" + " title=\""
						+ StringUtils.htmlEncode(description) + ", "
						+ l.getAuthority() + "\"" + ">" + display
						+ "</option>\n");
			}
		}
