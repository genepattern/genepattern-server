...
<table cellspacing="4">
	<tr>
		...
		<input type="button" value="delete checked jobs"
			onclick="deleteJobs()"></form>
		</th>
	</tr>
	<%
	                if (jobs.length == 0) {
	            ...
	            for (int i = 0; i < jobs.length; i++) {
	                ...
	                out.print("<tr><td align=\"right\"><a href=\"getJobResults.jsp?jobID=" + job.getJobNumber() + "\">"
	                        + job.getJobNumber() + "</a>");

	                out.println("<td><span onClick='showJob(" + job.getJobNumber() + ")'>" + job.getTaskName()
	                ...
	                if (status.equals(JobStatus.PROCESSING)) {
	                    out.print("<td><font color=" + htColors.get(status) + ">" + status
	                            ...
	                            + "<td><form><input type=\"checkbox\" name=\"deleteJobID\" value=\"" + job.getJobNumber()
	                            + "\"></form>");
	                    ...
	                    if (myJob) {
	                        out.print("<td><form><input type=\"checkbox\" name=\"deleteJobID\" value=\""
	                    ...
	                    if (status.equals(JobStatus.FINISHED)) {
	                        out.print("</td><td><form><input type=\"checkbox\" name=\"deleteJobID\" value=\""
	                                + job.getJobNumber() + "\"></form>");
	                    }
	                }
...
<%!public void writeParameters(String encodedUserId, List params, boolean showAll, String prefix, String serverURL,
        ...
        for (int i = 0; i < params.size(); i++) {
            ...
            out.println(prefix + "<input type=\"checkbox\" name=\"dl\" value=\"" + value + "\" checked><a href=\""
                    + fileURL + "\">" + fileName + "</a>");
