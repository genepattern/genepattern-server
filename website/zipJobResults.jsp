        ...
        try {
            ...
            for (int i = 0; i < attachmentNames.length; i++) {
                ...
                try {
                    ...
                    //TODO: log exception, shouldn't get here
                    continue;
                }
        ...
        for (int j = 0; j < deleteJob.length; j++) {
            try {
                LocalAnalysisClient analysisClient = new LocalAnalysisClient(
                        userID);
                analysisClient
                        .deleteJob(Integer.parseInt(deleteJob[j]));
            } catch (Exception e) {
            }
        }
    }
...
<form name='form1'><input type="checkbox" name="<%= SHOW_ALL %>"
  <%= showAll ? "checked" : "" %> value="<%= SHOW_ALL %>"
  onclick="javascript:window.location='zipJobResults.jsp<%= showAll ? "" : ("?" + SHOW_ALL + "=1") %>'">show
everyone's jobs <input type="checkbox" name="<%= SHOW_LOGS %>"
  <%= showLogs ? "checked" : "" %> value="<%= SHOW_LOGS %>"
  onclick="toggleLogs()">show execution logs</form>
...
<table cellspacing="4">
  <tr>
    ...
    <th valign="bottom">
    <form><a href="#" onclick="checkAll(true)">check all</a> &nbsp;&nbsp;
    <a href="#" onclick="checkAll(false)">uncheck all</a><br>
    <input type="button" value="delete checked jobs" onclick="deleteJobs()"></form>
    </th>
  </tr>
  <%
      if (jobs.length == 0) {
      ...
      for (int i = 0; i < jobs.length; i++) {
          ...
          out
                  .print("<tr><td align=\"right\"><a href=\"getJobResults.jsp?jobID="
                          + job.getJobNumber()
                          + "\">"
                          + job.getJobNumber() + "</a>");

          out.println("<td><span onClick='showJob(" + job.getJobNumber()
                  + ")'>" + job.getTaskName()
                  + "&nbsp;<img src='skin/info_obj.gif'>");

          Date submitted = job.getDateSubmitted();
          DateFormat formatter = submitted.after(midnight.getTime()) ? shortDateFormat
                  : dateFormat;
          ...
          if (completed != null && !status.equals(JobStatus.PROCESSING)) {
              formatter = completed.after(midnight.getTime()) ? shortDateFormat
                      : dateFormat;
          ...
          if (status.equals(JobStatus.PROCESSING)) {
              out
                      .print("<td><font color="
                              ...
                              + ">"
                              + status
                              + "</font> - <a href=\"zipJobResults.jsp?"
                              + STOP
                              + "="
                              + job.getJobNumber()
                              + (showAll ? ("&" + SHOW_ALL + "=1") : "")
                              + "\">stop</a></td>"
                              + "<td><form><input type=\"checkbox\" name=\"deleteJobID\" value=\""
                              + job.getJobNumber() + "\"></form>");
              ...
              out.print("<td><font color=" + htColors.get(status) + ">"
                      + status + "</font></td>");
              if (myJob) {
                  out
                          .print("<td><form><input type=\"checkbox\" name=\"deleteJobID\" value=\""
                                  + job.getJobNumber() + "\"></form>");
              } else {
                  out.print("<td>&nbsp;");
              }
              ...
              if (status.equals(JobStatus.FINISHED)) {
                  out
                          .print("</td><td><form><input type=\"checkbox\" name=\"deleteJobID\" value=\""
                                  + job.getJobNumber() + "\"></form>");
              }
          }
          ParameterInfo[] params = job.getParameterInfoArray();
          JobInfo[] children = analysisClient.getChildren(job
                  .getJobNumber());
          out.println("<form method=\"post\">");
          out.println("<input type=\"hidden\" name=\"jobID\" value=\""
                  + job.getJobNumber() + "\"/>");
          boolean hasOutputFiles = false;
          boolean isPipeline = false;
          if (showAll) {
              out.println("<input type=\"hidden\" name=\"" + SHOW_ALL
                      + "\" value=\"1\"/>");
          }
          int startId = id;
          ...
          if (hasOutputFiles) {
              ...
              out
                      .println("<input type=\"submit\" name=\"delete\" value=\"delete\" class=\"little\"");
              if (!myJob) {
                  out.println("disabled=\"true\" ");
              }
              out
                      .println("><input type=\"submit\" name=\"download\" value=\"download\" class=\"little\">");
              if (isPipeline) {
                  //	out.print("<input type=\"checkbox\" onClick=toggle('" + job.getJobNumber() + "');>");
                  out
                          .print("<input type=\"checkbox\" checked onClick=\"toggle("
                                  + startId + "," + id + ");\">");
...
<%!public void writeParameters(String encodedUserId, List params,
            boolean showAll, String prefix, String serverURL, JspWriter out)
        ...
        for (int i = 0; i < params.size(); i++) {
            ...
            String altSeperator = "/";
            if (index == -1)
                index = value.lastIndexOf(altSeperator);
            ...
            String fileURL = "jobResults/" + jobNumber + "/"
                    + URLEncoder.encode(fileName, "utf-8");
            out.println(prefix
                    + "<input type=\"checkbox\" name=\"dl\" value=\"" + value
                    + "\" checked><a href=\"" + fileURL + "\">" + fileName
                    + "</a>");

            if (AuthorizationHelper.createPipeline(encodedUserId)) {

                out.print("<span  onClick=\"createPipeline(\'"
                        + URLEncoder.encode(serverURL + fileURL, "utf-8")
                        + "\')\" \" ><nobr>");
                out.print("&nbsp;");
