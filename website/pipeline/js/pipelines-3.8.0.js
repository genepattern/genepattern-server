...
//        });
        ...
        // Init sliders
        library._initSearchSlider(library._translateModuleFormat());
        library._initBrowseSlider(library._translateCategoryFormat());

        // Search box controls
        $("#module-search-box").keyup(function(event) {
            if ($(".search-widget:visible").length === 0) {
                $("#module-search").searchslider("show");
                library._setModuleSearchTitle($(this).val())
            }
            $("#module-search").searchslider("show");
            ...
            if (event.originalEvent.keyIdentifier == "Enter") {
                $("#module-search").searchslider("select_default", $(this).val());
            }
        });
    ...
    _setModuleSearchTitle: function(filter) {
        if (filter === '') {
    ...
    _translateModuleFormat: function() {
        var returnFormat = [];

        for (var i in this.moduleVersionMap) {
            var module = library.getHighestVersion(i + ":0");
            returnFormat.push({
                lsid: module.lsid,
                name: module.name,
    ...
    _translateCategoryFormat: function() {
        var returnFormat = [];

        for (var category in this.moduleCategoryMap) {
            returnFormat.push({
                lsid: "",
                name: category,
                version: "",
    ...
    _initSearchSlider: function(all_modules) {
        var search = $('<div id="module-list-search"></div>').modulelist({
            ...
            click: function(event) {
                ...
                $('#module-search').searchslider("hide");
            }
        });
        ...
        $('#module-search').css("height", $(window).height() - 112);
        $(window).resize(function() {
            $('#module-search').css("height", $(window).height() - 112);
        });
    },
