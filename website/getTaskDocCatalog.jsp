	...
	<table>
		...
		%>
			...
			<td valign="top"><div id="<%=firstName%>" style="display:none"><%= indent %>
				<font size='-2'><a name="<%= ti.getName() %>" href="<%= !isPipeline ? "addTask.jsp" : "pipeline/index.jsf" %>?lsid=<%= lsid.toString() %>&view=1" style="white-space: nowrap;"><%= ti.getName() %> (<%= lsid.getVersion() %>)</a></font></div></td>
