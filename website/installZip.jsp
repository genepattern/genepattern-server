...
%>
Please select a zip file for upload containing GenePattern manifest and
...
<%
    ...
    if (isEncodedPost) {
        ...
        if (url != null) {
            attachedFile = new File(url);
            filename = attachedFile.toURI().toString();
            fileURL = filename;
            attachmentName = url
...
<form method="post" ENCTYPE="multipart/form-data" action=""><input
        type="checkbox" name="inclDependents" checked> include all tasks used
    within <%=attachmentName%>? <input type="hidden" name="url"
                                       value="<%= fileURL %>"> <input type="hidden" name="askedRecursive"
                                                                      value="true"> <input type="submit" name="submit"
                                                                                           value="install"> <br>
    <br>
    <table cols="2">
        <tr>
            <td valign="top" align="right"><%=attachmentName%> contents:</td>
            <td valign="top" align="left"><%
                // skip the first one, it is the pipeline task itself
                vTasks.nextElement();
                for (i = 0; vTasks.hasMoreElements(); i++) {
                    TaskInfo ti = ((TaskInfo) vTasks
                            .nextElement());
                    TaskInfoAttributes tia = ti
                            .giveTaskInfoAttributes();
                    LSID l = new LSID((String) tia
...
%>
<a
        href="runPipeline.jsp?<%= GPConstants.NAME %>=<%= lsid %>&amp;cmd=run">Try
    it out</a>
<br>
<br>
<%
