...
//
        ...
        if (suiteFilterParam != null) {
            ...
            while (strtok.hasMoreTokens()) {
                ...
                if (suiteId.endsWith("'")) {
                    suiteId = suiteId.substring(0, suiteId.length() - 1);
                }
        ...
        try {
            ...
            for (int i = 0; (i < jobs.length) && (recentCount > 0); i++) {
                ...
                if (jobName.endsWith(".pipeline")) {
                    if (recentPipes.size() >= recentCount) {
                        continue;
                    }
                    ...
                    if (recentTasks.size() >= recentCount) {
                        continue;
                    }
...
function openSuiteFilter(height) {
    filterSuiteWindow =
    window.open('chooseSuite.jsp', 'SuiteFilter', 'toolbar=no, location=no, status=no, menubar=no, resizable=yes,width=320,height=<%=height%>');
...
function jumpTo(btn) {
    ...
    if (btn.value == EDIT || btn.value == CREATE) {
        if (task.selectedIndex != 0) {
            ...
            window.location =
            "pipelineDesigner.jsp?<%= GPConstants.NAME %>=" + pipeline.options[pipeline.selectedIndex].value;
        }
    } else if (btn.value == VIEW) {
        if (task.selectedIndex != 0) {
            window.location =
            "addTask.jsp?<%= GPConstants.NAME %>=" + task.options[task.selectedIndex].value + "&view=1";
        } else if (pipeline.selectedIndex != 0) {
            window.location =
            "viewPipeline.jsp?<%= GPConstants.NAME %>=" + pipeline.options[pipeline.selectedIndex].value;
        }
        ...
        if (task.selectedIndex > 1) {
            ...
            window.location =
            "runTask.jsp?cmd=run&<%= GPConstants.NAME %>=" + pipeline.options[pipeline.selectedIndex].value;
        }
    }
}
function changeTask() {
...
function addNavbarItem(name, lsid) {
    ...
    var taskType = name.substr(-(".<%= GPConstants.TASK_TYPE_PIPELINE %>".length)) ==
                   ".<%= GPConstants.TASK_TYPE_PIPELINE %>" ? "Pipeline" : "Task";
    ...
    // highlight it
    eval("change" + taskType + "()");
    // call either changeTask() or changePipeline()
}
...
function LSID(lsid) {
    ...
    this.authorityType = (this.authority == '<%= LSIDManager.getInstance().getAuthority() %>'.replace(" ", "+")) ?
                         '<%= LSIDUtil.AUTHORITY_MINE %>' :
                         (this.authority == '<%= LSIDUtil.BROAD_AUTHORITY %>' ? '<%= LSIDUtil.AUTHORITY_BROAD %>' :
                          '<%= LSIDUtil.AUTHORITY_FOREIGN %>');
}
        ...
        <table width="100%">
            <tr>
                <td valign="top"><a href="index.jsp" class='logo'><img src='skin/logoSmall.gif' border="0" height=25
                                                                       width=25/>&nbsp;<%=
                    messages.getProperty("ApplicationName", "GenePattern")%>
                </a> &nbsp;
                </td>
                <td align="right" valign="top">
                    <nobr>
                        ...
                        <% if (userUnknown) { %>
                        <input type="text" class="little" size="30" name="<%= GPConstants.USERID %>"
                               value="<%= EMAIL_ADDRESS %>" onfocus="ufocus(this, true, '<%= EMAIL_ADDRESS %>')"
    ...
    <form action="search.jsp" name="searchForm">
        ...
        <table width="100%">
            <tr>
                <td valign="top">
                    <% if (!userUnknown) { %>
                    <%= _taskCatalog(tmTasks, recentPipes, "Pipeline", "changePipeline();",
                            GPConstants.TASK_TYPE_PIPELINE, userID, request.getParameter(GPConstants.NAME)) %>
                    <%=_taskCatalog(tmTasks, recentTasks, "Task", "changeTask();", null, userID,
                            request.getParameter(GPConstants.NAME)) %>
                    <nobr>
                    ...
                    &nbsp;&nbsp;&nbsp;
                    <a href="#"
                       onclick="openSuiteFilter()" class="navbarlink">
                        <nobr>Filter by Suite</nobr>
                    </a>
				...
				request.getParameter("search") : SEARCH %>" onfocus="ufocus(this, true, '<%= SEARCH %>')"
                                                onblur="ufocus(this, false, '<%= SEARCH %>')"><input type="image"
                                                                                                     src="skin/search.jpeg"
                                                                                                     alt="search"
                                                                                                     value="?"
...
<% } %>
<% } %>
<%! private String _taskCatalog(Collection tmTasks, Vector recent, String selectorName, String onSelectURL, String type,
                                String userID, String requestedName) {
    ...
    for (Iterator itTasks = recent.iterator(); itTasks.hasNext();) {
        ...
        selected = requestedName != null &&
                (name.equals(requestedName) || requestedName.startsWith(versionlessLSID + ":"));
        sbCatalog.append("<option value=\"" + (lsid != null ? l.toString() : name) + "\" class=\"tasks-" +
                authorityType + "\"" + " title=\"" + StringUtils.htmlEncode(description) + ", " + l.getAuthority() +
                "\"" + (selected ? " selected" : "") + "><i>" + shortName + "</i></option>\n");
    }
    if (recent.size() > 0) {
        sbCatalog.append("<option value=\"" + IGNORE + "\" disabled>" + DIVIDER + "</option>\n");
    }
    ...
    for (Iterator itTasks = tmTasks.iterator(); itTasks.hasNext();) {
        ...
        if (name.endsWith("." + GPConstants.TASK_TYPE_PIPELINE)) {
            ...
            if (type == null || !type.equals(GPConstants.TASK_TYPE_PIPELINE)) {
                continue;
            }
        }
        if (type != null && !tia.get(GPConstants.TASK_TYPE).equals(type)) {
            continue;
        }
        String shortenedName = shortName;
        if (shortName.length() > ((2 * halfLength) + 3)) {
        ...
        if (isPublic || isMine) {
            ...
            sbCatalog.append("<option value=\"" + (lsid != null ? l.toString() : name) + "\" class=\"tasks-" +
                    authorityType + "\"" + " title=\"" + StringUtils.htmlEncode(description) + ", " + l.getAuthority() +
                    "\"" + (selected ? " selected" : "") + ">" + shortName + "</option>\n");
        }
    }
