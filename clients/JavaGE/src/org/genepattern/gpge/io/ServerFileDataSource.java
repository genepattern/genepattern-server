...
public class ServerFileDataSource extends AbstractDataSource {
	...
	/**
	 * Creates a new instance of GctFileDataSource This will be aware of the
	 * specified directory and all files that the parser handles there.
	 */
	...
	protected DataObjectProxy createDataObjectProxy(final Object data) {
		System.out.println("creating DataObjectProxy from ParameterInfo "
				+ data);
		...
		try {
			final String string_url = StringUtils.replaceAll(
					ServerJobDataSource.FILE_HEADER_URL, "[resultfilename]",
					URLEncoder.encode(remote_file, "UTF-8"));
			final java.net.URL url = new java.net.URL(
					getFileDownloadURL(remote_file));
			java.net.URLConnection conn = url.openConnection();

			String size = (long) (Math.ceil(conn.getContentLength() / 1024.0))
					+ " KB"; // size returned from InputStream.available is not
							 // correct
			final InputStream in = url.openStream();

			summary = parser.createSummary(in);
			if (summary.getPrimaryinfo() != null
					&& summary.getPrimaryinfo().containsKey("Size=")) {
				summary.getPrimaryinfo().put("Size=", size);
			} else if (summary.getPrimaryinfo() != null
					&& summary.getPrimaryinfo().containsKey("Size")) {
				summary.getPrimaryinfo().put("Size", size);
			}
			in.close();
		...
		System.out.println("Remote file: " + remote_file + "\nbetter_name= "
				+ better_name);
		final DataObjectProxy proxy = new DefaultDataObjectProxy(better_name,
				this, summary);
		return proxy;
	}
	 ...
	 */
	public DataObjector getDataObject(final DataObjectProxy proxy)
	...
	public InputStream getRawInputStream(final DataObjectProxy proxy)
			throws IOException {
		final String remote_file = getRemoteFileNameFor(proxy);
		return createRemoteFileInputStream(remote_file);
	}

	public static final InputStream createRemoteFileInputStream(
			final String remote_file_name) throws IOException {
		final String just_filename = getJustFileName(remote_file_name);
		String string_url = StringUtils.replaceAll(FILE_SOURCE_URL,
				"[just_filename]", URLEncoder.encode(just_filename, "UTF-8"));
		string_url = StringUtils.replaceAll(string_url, "[just_dir]",
				URLEncoder.encode(getJustPath(remote_file_name), "UTF-8"));
		...
		try {
			...
			javax.swing.JOptionPane.showMessageDialog(GenePattern
					.getDialogParent(), "Unable to connect to server "
	 ...
	 */
	public static String getFileDownloadURL(String fileOnServer)
			throws IOException {
		String fileName = getJustFileName(fileOnServer);
		String retrieveFileURL = StringUtils.replaceAll(FILE_SOURCE_URL,
				"[just_filename]", URLEncoder.encode(fileName, "UTF-8"));
		retrieveFileURL = StringUtils.replaceAll(retrieveFileURL, "[just_dir]",
				URLEncoder.encode(getJustPath(fileOnServer), "UTF-8"));
	...
	public final String getRemoteFileNameFor(final DataObjectProxy proxy) {
		...
		if (param == null)
			throw new IllegalArgumentException(
					"The DataObjectProxy is not found!\n" + proxy);
	 ...
	 *  
	 */
	public java.io.File getAsLocalFile(DataObjectProxy proxy)
			throws IOException {
		// File file = (File)proxy_file.get(proxy);
		// if( file == null || !file.exists()) { always get the file from the
		// server
		//file = File.createTempFile(proxy.toString(),
		// '.'+parser.getFileExtensions()[0]);
		//file = File.createTempFile("", proxy.toString());
		File file = org.genepattern.io.StorageUtils.createTempFileNoMung(proxy
				.toString());
	...
	public static final String getNameNoJunk(final String name) {
		...
		if (sep_ind >= 0) {
			System.out.println("Debug: seperator=\"" + seperator
					+ "\" sep_ind=" + sep_ind);
	...
	// org.genepattern.io.StorageUtils.TEMP_FILE_MARKER;
	...
	static {
		...
		try {
			final java.util.Properties gp_props = org.genepattern.util.PropertyFactory
			...
			org.genepattern.gpge.GenePattern.showError(null,
					"Cannot get the result file source URL", ex);
		}
		FILE_SOURCE_URL = source_url;
		if (source_url == null)
			throw new IllegalStateException(
					"Cannot get the result file source URL!");
