...
public class FileDataSource extends AbstractDataSource {
	...
	protected DataObjectProxy createDataObjectProxy(final Object data) {
		...
		final String name = (show_exts) ? file.getName() : AbstractDataParser
				.getFileNameNoExt(file);
		final DataObjectProxy proxy = new DefaultDataObjectProxy(name, this,
				summary);
		return proxy;
	}
	...
	public DataObjector getDataObject(final DataObjectProxy proxy)
		...
		final FileInputStream in = new FileInputStream(file);
		DataObjector data = parser.parse(in, AbstractDataParser
				.getFileNameNoExt(file));
		in.close();
		return data;
	}
