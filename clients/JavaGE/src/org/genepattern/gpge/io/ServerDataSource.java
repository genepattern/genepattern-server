 ...
 * @author keith
 */
public class ServerDataSource extends GroupDataSource implements
		java.util.Observer {

	/**
	 * Creates a new instance of ServerDataSource This will be aware of the
	 * specified directory and all data files there.
	 */
	public ServerDataSource(
			final org.genepattern.gpge.ui.tasks.DataModel analysis_model,
			final org.genepattern.gpge.ui.tasks.ResultsPanel analysis_results_panel,
			final DataParser[] parsers/* DataSource[] sources */)
		...
		server_node = new javax.swing.tree.DefaultMutableTreeNode(
				"Server (remote access)", true/* allows childern */);

		final Properties p = org.genepattern.util.PropertyFactory.getInstance()
	 ...
	 * 
	 * @param o
	 *            <code>Observable<code> object is the omiview.analysis
	 *          package's <code>DataModel<code>
	 * @param arg Unused...
	 */
	public void update(java.util.Observable observable, Object obj) {
		System.out.println("updating observer ServerDataSource obj=" + obj
				+ " (should be null...)");
		this.updateList();
	}
	...
	/**
	 * updates the list of data files previously found Note: this should be run
	...
	/**
	 * the node factory that will create TreeNode objects with DataObjectProxy
	 * objs
	 */
	private final TreeNodeFactory node_factory = new TreeNodeFactory() {
		public javax.swing.tree.MutableTreeNode createNode(
				final javax.swing.tree.MutableTreeNode node) {
			...
			try {
				System.out
						.println("before replacement: file_header_source_url="
								+ file_header_source_url);
				final String string_url = StringUtils.replaceAll(
						file_header_source_url, "[resultfilename]", URLEncoder
								.encode(server_file_name, "UTF-8"));
				System.out.println(" after: file_header_source_url="
						+ file_header_source_url);
			...
			if (summary == null) {
				final DataParser parser = UniversalDecoder
						.getParserWithExt(server_file_name);
				summary = new SummaryError(server_file_name, exception,
						(parser != null) ? parser : parsers[0]);
			}
			//final String name =
			// AbstractDataParser.getFileNameNoExt(server_file_name);
			//final DataObjectProxy proxy = new DefaultDataObjectProxy(name,
			// this, summary);
			final DataObjectProxy proxy = new DefaultDataObjectProxy(
					server_file_name, ServerDataSource.this, summary);
			return new javax.swing.tree.DefaultMutableTreeNode(proxy, false/*
																		    * no
																		    * childern
																		    */);
		}
