...
public abstract class AbstractDataSource implements DataSource {
     ...
     */
    abstract public DataObjector getDataObject(final DataObjectProxy proxy) throws java.io.IOException, java.text.ParseException, IllegalArgumentException;
    /** returns a description of the source- i.e. if it reads gct data from a local
     * directory, it reads sdf data from OmniGene, etc
     */
     public abstract String getDescription(DataObjectProxy proxy);
     
    
    /** returns an InputStream for reading the raw data */
    abstract public InputStream getRawInputStream(final DataObjectProxy proxy) throws IOException;
     ...
     */
    abstract public java.io.File getAsLocalFile(final DataObjectProxy proxy) throws IOException;
    ...
    public DataObjectProxy[] getDataProxies() {
        ...
        return (DataObjectProxy[])key_set.toArray(new DataObjectProxy[key_set.size()]);
    }
    /** returns the parents */
    ...
    public void updateList(final List new_data, final List removed_data) {
        if( new_data != null) {
            ...
            Thread.yield();
            try {
                show_exts = GPpropertiesManager.getBooleanProperty("databrowser.show.file.exts", true);
                //System.out.println("show.file.exts="+GenePattern.getProperty("show.file.exts")+"  show_exts="+show_exts);
            ...
            //System.out.println("A total of "+new_cnt+" data were accepted by the filer");
            ...
            Thread.yield();
            if( new_cnt > 0 ) {
                final DataObjectProxy[] new_proxies_array = (DataObjectProxy[])
                new_proxies.toArray(new DataObjectProxy[new_cnt]);
        ...
        if( removed_data != null) {
            ...
            Thread.yield();
            //final DataObjectProxy[] missing_proxies = new DataObjectProxy[missing_cnt];
            ...
            for(int j = 0; j < missing_cnt; j++) {
                final DataObjectProxy proxy = (DataObjectProxy)data_proxy.remove(removed_data.get(j));
            ...
            notifyDataSourceListenersRemove((DataObjectProxy[])missing_proxies.toArray(new DataObjectProxy[missing_proxies.size()]), parents);
     ...
     * FIXME this info should be had from the parser
     */
    ...
    public void addDataSourceUpdateListener(final DataSourceUpdateListener listener) {
        if( listeners.add(listener) ) {
            ...
            System.err.println(this.getClass()+" already had added DataSourceUpdateListener: "+listener );
        }
    }
...
//            }.start();
            ...
            System.err.println(getClass()+" didn't have the DataSourceUpdateListener to remove: "+listener);
        }
    }
    /** notifies all DataSourceUpdateListener objects that some DataObjectProxy objects
     * have become available
     */
    protected void notifyDataSourceListenersAdd(final DataObjectProxy[] proxies, final Object[] parents) {
//        new edu.mit.genome.util.SwingWorker() {
//            public Object construct() {
...
//        }.start();
    ...
    /** notifies all DataSourceUpdateListener objects that some DataObjectProxy objects
     * are no longer available or have been removed
     */
    protected void notifyDataSourceListenersRemove(final DataObjectProxy[] proxies, final Object[] parents) {
//        // run it
//        new edu.mit.genome.util.SwingWorker() {
//            public Object construct() {
                // this must be called on swing event thread
...
//        }.start();
    ...
    // fields
    /** the collection of listenrs
     *FIXME this should be WeakHashMap so that Weak references get GCed 
     */
...
//    protected final Collection proxies;
    /** the parent user objects that these proxies are members of in a tree structure */
    protected final Object[] parents;
    /** array of ProxiesWithPath objects associating parents with an array of proxies
     * only one of these by default
     */
