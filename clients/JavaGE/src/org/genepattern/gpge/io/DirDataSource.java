...
public class DirDataSource extends GroupDataSource/* implements DataSource */{
	...
	public DirDataSource(
			final DataParser[] parsers,
			final File dir,
			final DataSourceManager manager,
			final org.genepattern.modules.ui.graphics.PeriodicProgressObserver observer)
	...
	protected static final FileDataSource[] createDataSources(
			final DataParser[] parsers) {
	...
	/**
	 * helper method that converts the file to a directory or returns the file
	 * if it already is a dir
	 */
	...
	/**
	 * sets the directory note this runs in its own thread because it takes
	 * awhile
	 */
	private synchronized final void setDir(final File file)
			throws java.io.IOException {
		if (file == null)
			throw new NullPointerException("The directory cannot be null!");
		this.dir = getDirectory(file);
		if (!dir.exists())
			throw new java.io.FileNotFoundException("The directory, (" + dir
					+ "), does not exist!");
		...
		String dir_props = org.genepattern.util.GPpropertiesManager
				.getProperty("gp.project.dirs");
		System.out.println("In " + this + " gp.project.dirs='" + dir_props
				+ "'");
		dir_props = (dir_props != null && dir_props.trim().length() > 0) ? dir_props
				+ ", " + this.dir.getCanonicalPath()
				: this.dir.getCanonicalPath();
		System.out.println("after='" + dir_props + "'");
		org.genepattern.util.GPpropertiesManager.setProperty("gp.project.dirs",
				dir_props);

		refresh();
	}

	private synchronized final void setName(final File file)
			throws java.io.IOException {
		if (file == null)
			throw new NullPointerException("The directory cannot be null!");
	 ...
	 * 
	 * @param source
	 *            the file that is to be pasted
	 * @param is_temp
	 *            if true then the file is renamed otherwise it is copied
	 * @throws IOException
	 *             if some problem
	 */
	public final void paste(final File source, final boolean is_temp)
			throws java.io.IOException {
		if (!source.isFile()) {
			...
			final Thread thread = new Thread(
					new org.genepattern.util.RunLater() {
						public final void runIt() throws Exception {
							updateList(DirDataSource.this.dir
									.listFiles(file_only_filter));
						}
					});
			thread.start();
		}
	...
	/**
	 * creates a unique file name that doesn't conflict with the current names
	 * in this directory
	 */
	...
	public final boolean equals(final Object another) {
		if (another instanceof DirDataSource) {
			...
			return (this.dir == dds.dir || (this.dir != null && this.dir
					.equals(dds.dir)));
		}
		return false;
	}
	...
	/**
	 * called when this should refresh/rescan its data and update refreshes the
	 * directory
	 */
