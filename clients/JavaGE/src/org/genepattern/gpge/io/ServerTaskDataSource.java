...
public class ServerTaskDataSource extends GroupDataSource {
	...
	/**
	 * Creates a new instance of ServerTaskDataSource This will be aware of the
	 * specified directory and all data files there.
	 */
	public ServerTaskDataSource(final String name, final DataParser[] parsers,
	...
	protected void updateList(final List data) throws java.io.IOException {
		...
		for (int j = 0; j < job_cnt; j++) {
			...
			} catch (ArrayIndexOutOfBoundsException e) { // jgould, hack to fix
														 // synchronization
														 // problem
				break;
			}
			if (anal_job.getJobInfo().containsOutputFileParam()) {
				final String id = String.valueOf(anal_job.getJobInfo()
						.getJobNumber());
				...
				job.updateList(onlyFileParams(anal_job.getJobInfo()
						.getParameterInfoArray()), PARAMINFO_COMPARATOR);
			} else {
				GenePattern.logWarning("Does not contain ouput files: "
						+ anal_job);
			}
		}
	}
	...
	protected DataParser getParserToDecode(final Object data)
		...
		//         DataParser parser =
		// UniversalDecoder.getParserWithExt(remote_file_name);
		//         
	...
	// throws java.io.IOException {
	//         final String string_url = StringUtils.replaceAll(FILE_HEADER_URL,
	// "[resultfilename]", remote_name);
	...
	protected final ServerJobDataSource createServerJobDataSource(
			final String id, final AnalysisJob job) {
		final ServerJobDataSource source = new ServerJobDataSource(job,
		...
		// (DataSourceUpdateListener)iter.next();
	...
	/**
	 * the ParameterInfo Compatator - Compares two ParameterInfo objs by their
	 * names
	 */
