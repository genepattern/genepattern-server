...
/**
 * Knows how to identify remote Jobs from the server and create
 * ServerFileDataSource objects. Remote file proxies are then stored in the
 * ServerFileDataSource by the parser that accepted them.
 * 
 * @author keith
 */
public class ServerJobDataSource extends GroupDataSource {
	...
	/**
	 * Creates a new instance of ServerJobDataSource This will be aware of the
	 * specified directory and all data files there.
	 */
	public ServerJobDataSource(final AnalysisJob job,
			final DataParser[] parsers, final GroupDataSource parent,
			final DataSourceManager manager) {
		super(createDataSources(parsers), parent, manager, new Integer(job
				.getJobInfo().getJobNumber()));
		this.job = job;
		resetName();
	}
	...
	protected static final DataSource[] createDataSources(
			final DataParser[] prsrs) {
		// if parsers arn't available get some...
		final DataParser[] parsers = (prsrs.length > 0) ? prsrs
				: (DataParser[]) UniversalDecoder.PARSER_LIST
						.toArray(new DataParser[UniversalDecoder.PARSER_LIST
								.size()]);
	...
	protected void resetName() {
		...
		this.name_ref = java.text.DateFormat.getDateTimeInstance(
				java.text.DateFormat.SHORT, java.text.DateFormat.SHORT).format(
				date)
				+ " (job " + info.getJobNumber() + ")";
	}
	...
	//                        //final ResultFile file = new ResultFile(siteName,
	// job.getJobInfo().getJobNumber(), filename);
	//                        //final ResultTreeNode fileNode = new FileNode(file, this._dataModel,
	// this._treeModel, this._result);
	//                        //final MutableTreeNode fileNode = node_factory.createNode(new
	// FileNode(file, _dataModel, _treeModel, _result));
	//                        //jobNode.add(fileNode);
	//                    } else
	//                        System.out.println("Job "+job+" params["+i+"] does not have output file:
	// "+params[i]);
	//                }
	//            } else {
	...
	//    }
	/**
	 * returns a description of the source- i.e. if it reads data files from a
	...
	protected DataParser getParserToDecode(final Object data)
		...
		System.out.println("getting parser to decode remote (ParameterInfo) "
				+ data);
		...
		if (param.isOutputFile()) {
			...
			if (isError(remote_file_name)) {
				...
				if (has_seen == null || Boolean.FALSE.equals(has_seen)) {// if
																		 // not
																		 // shown
																		 // error
																		 // before
					hash.put(IS_OLD, Boolean.TRUE);
					final String string_url = FILE_SOURCE_URL
							+ URLEncoder.encode(remote_file_name, "UTF-8");
					System.out
							.println("getParserToDecode()--URL=" + string_url);
					...
					org.genepattern.io.StorageUtils.copyReaderToWriter(reader,
							swriter);
			...
			if (parser == null) {
				...
				// if this is null the file will map to the null object in next
				// step
			...
			if (parser == null) {
				...
				if (remote_file_name.endsWith("stdout")) {
					...
					if (ext != null
							&& ArrayUtils.contains(TEXT_PARSER
									.getFileExtensions(), ext))
						parser = TEXT_PARSER;
				}
			}
			if (parser == null)
			...
			System.err.println("ParameterInfo does not have output file: "
					+ param);
			return null;
		}
	}
	...
	protected DataParser getParserThatDecodesRemote(final String remote_name)
			throws java.io.IOException {
		final java.net.URL url = new java.net.URL(ServerFileDataSource
				.getFileDownloadURL(remote_name));
	...
	//    protected void updateList(final List data, final java.util.Comparator
	// comparer) throws java.io.IOException {
	...
	/**
	 * key for getting the attribute Boolean TRUE if the parameterinfo was seen
	 * before FALSE or null if not
	 */
	...
	/** the text parser */
	protected static final TextParser TEXT_PARSER = new TextParser(
			"Text Parser");
	...
	static {
		String fhu = null;
		try {
			final Properties gp_props = org.genepattern.util.PropertyFactory
					.getInstance().getProperties("omnigene.properties");
			fhu = gp_props.getProperty("result.file.header.source");
		} catch (Exception ex) {
			GenePattern.showError(null,
					"Cannot get the result file header source URL", ex);
		}
		FILE_HEADER_URL = fhu;
		if (fhu == null)
			throw new IllegalStateException(
					"Cannot get the result file header source URL");
