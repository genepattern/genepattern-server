...
/**
 * Discovers all the known and available DataObjects from local files, OmniGene
 * server, Web Services, ftp site, html site, DataBase, and etc.
 * 
 * Note that DataSources does not implement getDataProxies() and will throw a
 * NullPointerException if invoked.
 * 
 * @author keith
 */
public class DataSources extends AbstractDataSource implements
		DataSourceUpdateListener {
	...
	public void addDataSource(final GroupDataSource source) {// GroupDataSource
		...
		if (data_sources.add(source)) { // only unique datasources
			...
			//alert all listeners that there are some more DataObjects
			// available
			final DataObjectProxy[] proxies = source.getDataProxies();
			if (proxies != null && proxies.length > 0) {
				System.out
						.println("Source has " + proxies.length + " proxies!");
				notifyDataSourceListenersAdd(proxies,
						DataSourceUpdateListener.NO_PARENTS);
			}
		} else {
			System.err
					.println(getClass()
							+ ".addDataSource(DataSource) was trying to add the same source again: "
							+ source);
		}
	...
	public void removeDataSource(final GroupDataSource source) {// GroupDataSource
		if (data_sources.remove(source)) { // only unique datasources
			...
			final DataObjectProxy[] proxies = source.getDataProxies();
			if (proxies != null && proxies.length > 0) {
				notifyDataSourceListenersRemove(proxies,
						DataSourceUpdateListener.NO_PARENTS);
			}
		} else {
			System.err
					.println(getClass()
							+ ".removeDataSource(DataSource) was trying to remove an absent data source: "
							+ source);
		}
	...
	/**
	 * causes all datasources that can update to do so - note this could take
	 * some time
	 */
	public final void refresh() {
		...
		for (int i = 0; i < limit; i++) {
			final GroupDataSource source = (GroupDataSource) to_be_updated
					.get(i);
			try {
				source.refresh();
	...
	public ServerSiteDataSource setAnalysisDataModel(final String site_name,
			final org.genepattern.gpge.ui.tasks.DataModel analysis_model,
			final DataSourceManager manager) throws java.io.IOException,
			org.genepattern.webservice.PropertyNotFoundException {
		final DataParser[] parsers = (DataParser[]) UniversalDecoder.PARSER_LIST
				.toArray(new DataParser[UniversalDecoder.PARSER_LIST.size()]);
		final ServerSiteDataSource server_datasource = new ServerSiteDataSource(
				analysis_model, site_name, parsers, null, manager);
	 ...
	 */
	public synchronized final DirDataSource addDirectory(
			final java.io.File file,
			final DataSourceManager manager,
			org.genepattern.modules.ui.graphics.PeriodicProgressObserver observer)
			throws java.io.IOException, java.text.ParseException {
		final java.io.File dir = DirDataSource.getDirectory(file);
		if (dir == null) {
			throw new Warning(
					"Tried to add a Project Directory that doesn't exist: "
		...
		final DataParser[] parser_array = (DataParser[]) parsers
				.toArray(new DataParser[parsers.size()]);
		final DirDataSource dsource = new DirDataSource(parser_array, dir,
				manager, observer);
	 ...
	 *  
	 */
	public DataObjector getDataObject(DataObjectProxy proxy)
	...
	public java.io.InputStream getRawInputStream(final DataObjectProxy proxy)
