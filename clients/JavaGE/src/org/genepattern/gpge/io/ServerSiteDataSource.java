...
public class ServerSiteDataSource extends GroupDataSource {
	...
	/**
	 * Creates a new instance of ServerSiteDataSource This will be aware of the
	 * specified directory and all data files there.
	 */
	public ServerSiteDataSource(
			final org.genepattern.gpge.ui.tasks.DataModel analysis_model,
			final String site_name, final DataParser[] parsers,
		...
		// last thing
		final org.genepattern.modules.ui.listeners.SafeObserverListener observer = new org.genepattern.modules.ui.listeners.SafeObserverListener() {
			/**
			 ...
			 * @param arg Unused...
			 */
			protected final void updateIt(java.util.Observable observable,
					Object obj) throws java.io.IOException {
				System.out
						.println("updating observer ServerSiteDataSource obj ="
								+ obj);
				if (obj instanceof org.genepattern.gpge.ui.tasks.DataModel.JobAndObserver) {
					...
					final org.genepattern.webservice.JobInfo info = jao.job
							.getJobInfo();
					System.out.println("JobInfo: " + info);
					System.out.println("Has output file "
	...
	protected void updateList() throws java.io.IOException {
		...
		// create ServerTasks
		// FIXME should be a ServerSiteHandler class that creates new
		// ServerSiteDataSource
		// instances when new sites appear. It would also be the only listener
		// for
		...
		for (final Iterator jobs_iter = jobsOnsite.keySet().iterator(); jobs_iter
				.hasNext();) {
			final String task_name = (String) jobs_iter.next();
			//getting jobs
			final List jobs = (List) jobsOnsite.get(task_name);

			ServerTaskDataSource task = (ServerTaskDataSource) task_source
					.get(task_name);
			if (task == null) {
	...
	protected final ServerTaskDataSource createServerTaskDataSource(
			final String task_name) {
		final ServerTaskDataSource task = new ServerTaskDataSource(task_name,
				parsers, this, manager);
