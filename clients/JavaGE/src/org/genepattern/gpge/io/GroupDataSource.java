 ...
 * @author keith
 */
abstract public class GroupDataSource implements DataSource,
		org.genepattern.util.ProgressObservable {

	/**
	 * Creates a new instance of GroupDataSource
	 */
	public GroupDataSource(final DataSource[] srcs,
			final GroupDataSource parent, final DataSourceManager manager,
			final Object hashobject) {//throws java.io.FileNotFoundException {
		...
		if (GET_UNKNOWNS && num > 0) { // then one more DataSource with
									   // AnythingParser
	...
	/**
	 * create a DataSource from the parser This should be overridden by
	 * subclasses if needed (if GET_UNKNOWNS could be true and the sources array
	 * is not length 0 => don't use DataSource objects)
	 */
	protected DataSource createDataSource(final DataParser parser) {
		throw new UnsupportedOperationException(
				"This should be overridden if it's needed");
	}

	// abstract methods
	/** returns a parser for specified data */
	abstract protected DataParser getParserToDecode(Object data)
			throws java.io.IOException;

	/**
	 * returns a description of the source- i.e. if it reads gct files from a
	 * local directory, it reads sdf files from OmniGene, etc
	 */
	...
	/**
	 * updates the list of data objects previously found Note: this should be
	...
	protected final void updateList(final Object[] new_data,
			final java.util.Comparator comparer) {
		try {
	...
	private void _updateList(final Object[] new_data,
			final java.util.Comparator comparer) {
		// ProgressObservable stuff
		total = 7; // set in the fields
		current = 1;

		Arrays.sort(new_data, comparer); //if comparer is null the elements'
		...
		for (int i = 0; i < num_parsers; i++) {
			...
			if (added != null || removed_list != null) {
				final DataSource source = (DataSource) parser_source
						.get(parser);
		...
		//System.out.println(this+"\n old_cnt"+old_cnt+" new_cnt"+new_cnt+"
		// added list="+limit+" removed list="+/*cnt*/+" parser="+num_parsers);
		// System.out.println(this+"\n old_cnt"+old_cnt+" new_cnt"+new_cnt+"
		// added list="+limit+" removed list="+removed_list.size()+"
		// parser="+num_parsers);
	 ...
	 */
	public final DataObjector getDataObject(final DataObjectProxy proxy)
			throws java.io.IOException, java.text.ParseException,
			IllegalArgumentException {
		throw new UnsupportedOperationException(
				"Should not be called. One of the data sources should have been instead.");
	}

	/**
	 * returns a copy of the array of DataObjectProxy objects note that this is
	 * not dynamically updated but just a static array
	 * 
	 *  
	 */
	public final org.genepattern.gpge.io.DataObjectProxy[] getDataProxies() {
		//throw new UnsupportedOperationException("This shouldn't be used see
		...
		return (DataObjectProxy[]) list
				.toArray(new DataObjectProxy[list.size()]);
	}
	 ...
	 * example character (ASCII), or binary, or unknown, data.
	 */
	...
	public final java.io.InputStream getRawInputStream(
			final DataObjectProxy proxy) throws java.io.IOException {
		throw new UnsupportedOperationException("Not implemented");
	}
	 ...
	 *  
	 */
	public final void addDataSourceUpdateListener(
	 ...
	 * listeners
	 */
	public final void removeDataSourceUpdateListener(
			final DataSourceUpdateListener listener) {
		final int limit = sources.length;
		for (int i = 0; i < limit; i++) {
	...
	//FIXME these should be implemented for the various Set and Map instances
	// to work
	...
	/**
	 * the manager that needs to know about any additional GroupDataSource objs
	 * created
	 */
	...
	static {
		...
		try {
			get_unknowns = GPpropertiesManager.getBooleanProperty(unknown_prop,
					true);
		} catch (java.text.ParseException ex) { // FIXME move this code into
												// GPpropertiesManager
		...
		final List list = new ArrayList();
		final String excludes_text = GPpropertiesManager
				.getProperty("gp.file.excludes");
		if (excludes_text != null && excludes_text.trim().length() > 0) {
			final String[] excludes_array = StringUtils.splitStrings(
					excludes_text, ',');
			...
			for (int i = 0; i < limit; i++) {
				try {
					final Pattern srchPattern = compiler
							.compile(excludes_array[i]);
					list.add(srchPattern);
				} catch (com.oroinc.text.regex.MalformedPatternException ex) {
					GenePattern
							.logWarning("While compiling regular expressions: '"
									+ excludes_array[i]
									+ "'\n"
									+ ex.getMessage());
				}
		...
		DEFAULT_DIR_FILE_FILTER = new FileFilter() {
			/**
			 * accepts the file if it is a file and passes some of the file name
			 * exludes
			 */
			...
			/**
			 * array of reg expresson Patterns to exclude files see
			 * http://www.oreilly.com/catalog/regex/ for a good tutor
			 */
			private final Pattern[] excludes = (Pattern[]) list
					.toArray(new Pattern[list.size()]);
