...
public class PreferencesPanel extends javax.swing.JPanel {
	...
	public PreferencesPanel() {
		...
		advanced_tabbedPane.addTab("System", null, new PropertyViewerPanel(
				sys_props, false), "Java system properties");
			...
			public final void actionPerformed(final java.awt.event.ActionEvent e) {
				...
				try {
					if (button == save_button) {
						...
						System.err.println("event from unknown button "
								+ button);
	...
	private String[] getOgPropNames() {
		final java.io.File dir = new java.io.File(System
				.getProperty("omnigene.conf"));
	...
	private Properties[] getOgProps(final String[] names) {
		...
		for (int i = 0; i < limit; i++) {
			try {
				props[i] = org.genepattern.util.PropertyFactory.getInstance()
						.getProperties(names[i]);
			} catch (Exception ex) {
				org.genepattern.gpge.GenePattern.showError(null,
						"Cannot get the properties for " + names[i], ex);
			}
		}
		return props;
	}
	...
	private void saveProperties() throws java.io.IOException {
		...
		final int limit = names.length;
		for (int i = 0; i < limit; i++) {
			final FileOutputStream og_out = new FileOutputStream(new File(
					og_dir, names[i]));
	...
	private void closeWindow() {
		...
		for (int i = property_panels.size() - 1; i >= 0; i--) {// rev. loop
			final PropertyViewerPanel panel = (PropertyViewerPanel) property_panels
					.get(i);
			panel.stopCellEditing();
		}
		// dispose of the window
		final java.awt.Window window = javax.swing.SwingUtilities
