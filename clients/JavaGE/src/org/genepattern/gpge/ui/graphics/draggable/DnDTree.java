 ...
 * 
 * FIXME should be possible to make this a seperate class that does not extend
 * JTree but requires a JTree for the constructor
 */
public class DnDTree extends JTree implements DragSourceListener,
	...
	private Point _ptOffset = new Point(); // Where, in the drag image, the
										   // mouse was clicked
	...
	public String getToolTipText(MouseEvent evt) {
		...
		if (node.getUserObject() instanceof org.genepattern.gpge.io.DirDataSource) {
			org.genepattern.gpge.io.DirDataSource s = (org.genepattern.gpge.io.DirDataSource) node
					.getUserObject();
	...
	public static void main(String argv[]) {
		...
		JFrame frame = new JFrame("Drag Images  Java "
	...
	public final void dragGestureRecognized(final DragGestureEvent e) {
		...
		e.startDrag(java.awt.Cursor.getDefaultCursor(), _imgGhost, _ptOffset,
				transferable, this);
	...
	private static final boolean drag_image_supported = DragSource
	...
	class TreeDropTargetListener implements DropTargetListener {
		...
		private BufferedImage _imgRight = new ArrowImage(15, 15,
				ArrowImage.ARROW_RIGHT);

		private BufferedImage _imgLeft = new ArrowImage(15, 15,
		...
		public TreeDropTargetListener(DnDTree theTree) {
			...
			_timerHover = new Timer(1000, new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					System.out.println("Hover Timer exercised expand="
							+ tree.inDrag);

					if (tree.inDrag) {
						_nLeftRight = 0; // Reset left/right movement trend
						if (isRootPath(_pathLast))
							return; // Do nothing if we are hovering over the
									// root node
		...
		public final void dragOver(final DropTargetDragEvent e) {
			...
			if (!drag_image_supported) {
				...
				if (_imgGhost != null) {
					...
					//g2.drawImage(_imgGhost, (int)_raGhost.getX(),
					// (int)_raGhost.getY(), Math.min((int)_raGhost.getWidth(),
					// max_w), (int)_raGhost.getHeight(), null);
					// end fix
					g2.drawImage(_imgGhost, (int) _raGhost.getX(),
							(int) _raGhost.getY(), null);
				} else {
					...
					final Rectangle raPath = getPathBounds(path);
					_raGhost.setRect(pt.x + _ptOffset.x, pt.y + _ptOffset.y,
							raPath.getWidth(), raPath.getHeight());
			...
			if (_nLeftRight > 20) {
				...
				// AffineTransform.getTranslateInstance(pt.x - _ptOffset.x, pt.y
				// - _ptOffset.y), null);
				//g2.drawImage(_imgRight,
				// AffineTransform.getTranslateInstance(pt.x + _ptOffset.x, pt.y
				// + _ptOffset.y), null);
		...
		public final boolean isDropAcceptable(final DropTargetDropEvent e) {
			...
			// Only accept COPY or MOVE gestures (ie LINK is not supported)
			System.out
					.println("checking for Copy or Move " + e.getDropAction());
			if ((e.getDropAction() & DnDConstants.ACTION_COPY_OR_MOVE) == 0)
				return false;

			System.out
					.println("Is this flavor acceptable (TREEPATH_FLAVOR)"
							+ e
									.isDataFlavorSupported(TransferableTreePath.TREEPATH_FLAVOR));
