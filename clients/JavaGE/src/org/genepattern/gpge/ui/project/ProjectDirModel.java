...
public class ProjectDirModel extends AbstractSortableTreeTableModel {
   ...
   public boolean contains(File projectDir) {
       ...
       return Collections.binarySearch(children, new ProjectDirNode(projectDir), PROJECT_DIR_COMPARATOR) >= 0;
   }
   ...
   public void refresh(File projectDirectory) {
      int index =  Collections.binarySearch(root.getChildren(), new ProjectDirNode(projectDirectory), PROJECT_DIR_COMPARATOR);
   ...
   public void add(File projectDir) {
      ...
      if(children!=null) {
         insertionIndex = Collections.binarySearch(children, child, PROJECT_DIR_COMPARATOR);
      }
      if(insertionIndex < 0) {
         ...
         File[] files = directory.listFiles(
            new FileFilter() {
               public boolean accept(File f) {
                  ...
                  return !f.isDirectory() && !name.endsWith("~") && !name.startsWith(".");
               }
            });
         if(files != null) {
