 ...
 * @author kohm
 */
public final class DirPreview extends javax.swing.JPanel implements
	 ...
	 * 
	 * @param fc
	 *            the file chooser that this is an accessory for
	...
	//        
	//        final int returnVal = file_chooser.showDialog(null, "Select Project
	// Directory");
	...
	// {
	//            java.io.File file = (approved) ? file_chooser.getSelectedFile() :
	// dir_prev.getLastSelected();
	//            if( file == null )
	 ...
	 * 
	 * @param dir
	 *            the file that represents a directory
	 */
	public File load(final File dir) {
		...
		if (file_names == null)
			file_names = new String[0]; // fix for bug 415
	...
	/**
	 * this method is fired when the <CODE>JFileChooser</CODE> has it's state
	 * changed
	 * 
	 * @param e
	 *            the event
	...
	public final void propertyChange(final PropertyChangeEvent e) {
		...
		if (dir != null)
			jLabel1.setText("Files in Directory " + dir.getName()
					+ File.separator + ':');
	}
	...
	/**
	 * JList objects that use this model will not be item-selectable, even if
	 * the selection mode is set with setSelectionMode(int).
	 */
	private final static class NoSelectSelectionModel implements
	...
	public static final class NotDirectoryFilter implements
