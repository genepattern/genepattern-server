...
public class DataTree implements org.genepattern.gpge.io.DataSourceManager {
	...
	/**
	 * the DataSelectListener (Should be some sort of List for multiple
	 * listeners)
	 */
	...
	void removeEmptyJobNodes() {
		javax.swing.SwingUtilities.invokeLater(new Thread() {
			public void run() {
				DefaultMutableTreeNode root = (DefaultMutableTreeNode) data_tree
						.getModel().getRoot();
				for (Enumeration enum = root.breadthFirstEnumeration(); enum
						.hasMoreElements();) {
					Object obj = enum.nextElement();
					if (obj instanceof DefaultMutableTreeNode
							&& ((DefaultMutableTreeNode) obj).getUserObject() instanceof org.genepattern.gpge.io.ServerJobDataSource) {
						DefaultMutableTreeNode node = (DefaultMutableTreeNode) obj;
						org.genepattern.gpge.io.ServerJobDataSource ds = (org.genepattern.gpge.io.ServerJobDataSource) node
								.getUserObject();
						org.genepattern.webservice.AnalysisJob job = ds
								.getJob();

						if (node.getChildCount() == 0) { // job has no output
														 // files
							if (node.getParent().getChildCount() == 1) { // remove
																		 // task
																		 // from
																		 // tree
								...
								DefaultMutableTreeNode parent = (DefaultMutableTreeNode) node
										.getParent();
								org.genepattern.gpge.io.ServerTaskDataSource pds = (org.genepattern.gpge.io.ServerTaskDataSource) parent
										.getUserObject();
								((DefaultTreeModel) data_tree.getModel())
	 ...
	 *            Description of the Parameter
	 */
	public DataTree(final JTree data_tree, final DataTree other,
			final boolean enable_popup) {
		...
		Runnable r = new Runnable() {
			public void run() {
				...
				data_tree.getSelectionModel().setSelectionMode(
						TreeSelectionModel.SINGLE_TREE_SELECTION);
			}
		};
		SwingUtilities.invokeLater(r);
		data_tree.addTreeSelectionListener(new TreeSelectionListener() {//FIXME
																		// needs
																		// to be
																		// protected
																		// by
					...
					public final void valueChanged(final TreeSelectionEvent e) {
						final DefaultMutableTreeNode node = (DefaultMutableTreeNode) data_tree
						...
						if (nodeInfo instanceof DataObjectProxy) {
							notifyNodeSelectListeners(
									(DataObjectProxy) nodeInfo, node);
						} else if (nodeInfo instanceof DataModel) {
							notifyNodeSelectListeners((DataModel) nodeInfo,
									node);
						} else if (nodeInfo instanceof GroupDataSource) {
							notifyNodeSelectListeners(
									(GroupDataSource) nodeInfo, node);
						} else {
							notifyNodeSelectListenersUnkn("Unknown node type "
									+ nodeInfo);
							System.err
									.println("Don't know how to process this node: "
											+ nodeInfo);
						}
					}
				});
		...
		data_tree.addMouseListener(new MouseInputListener() {
			...
			public final void mousePressed(final MouseEvent e) {
				final int selRow = data_tree.getRowForLocation(e.getX(), e
						.getY());
				final TreePath selPath = data_tree.getPathForLocation(e.getX(),
						e.getY());
			 ...
			 */
			private void mySingleClick(final int selRow, final TreePath selPath)
					throws Exception {
			}
			 ...
			 *                Description of the Exception
			 */
			private void myDoubleClick(final int selRow, final TreePath path)
					throws Exception {
				...
				if (user_obj instanceof DataObjectProxy) {
					...
					try {
						...
						try {
							jobNode = (DefaultMutableTreeNode) node.getParent()
									.getParent();
						} catch (Exception e) {// user clicked in data objects
											   // view
							((DefaultTreeModel) data_tree.getModel())
									.removeNodeFromParent(node);
							return;
						}
						if (!(jobNode.getUserObject() instanceof org.genepattern.gpge.io.ServerJobDataSource)) {// user
																												// clicked
																												// in
																												// data
																												// objects
						...
						org.genepattern.gpge.io.ServerJobDataSource ds = (org.genepattern.gpge.io.ServerJobDataSource) jobNode
								.getUserObject();

						org.genepattern.webservice.AnalysisJob job = ds
								.getJob();// get the job for this
						org.genepattern.webservice.JobInfo jobInfo = job
								.getJobInfo();
						jobInfo.removeParameterInfo(user_obj.toString());
						DefaultMutableTreeNode dataTypeNode = (DefaultMutableTreeNode) node
								.getParent();
						if (dataTypeNode.getChildCount() == 1) {
							if (jobNode.getChildCount() == 1) {
								JOptionPane.showMessageDialog(GenePattern
										.getDialogParent(), "Job number "
										+ jobInfo.getJobNumber()
										+ " has been deleted from the server.");
								DefaultMutableTreeNode taskNode = (DefaultMutableTreeNode) jobNode
										.getParent();
								if (taskNode.getChildCount() == 1) {
									((DefaultTreeModel) data_tree.getModel())
											.removeNodeFromParent(taskNode);
									dataObjectBrowser.removeJob(job);
									org.genepattern.gpge.io.ServerTaskDataSource taskDS = (org.genepattern.gpge.io.ServerTaskDataSource) taskNode
											.getUserObject();
									dataObjectBrowser.removeTask(taskDS
											.getName());
								} else {
									((DefaultTreeModel) data_tree.getModel())
								...
								JOptionPane.showMessageDialog(GenePattern
										.getDialogParent(), user_obj
										+ " has been deleted from the server.");
								((DefaultTreeModel) data_tree.getModel())
										.removeNodeFromParent(dataTypeNode);
							}
						} else {
							JOptionPane.showMessageDialog(GenePattern
									.getDialogParent(), user_obj
						...
						// and then tell the dataObjectBrowser we are
						// disconnected
	...
	public void disconnectedFromServer() {
		TreeNode root = (TreeNode) ((DefaultTreeModel) data_tree.getModel())
				.getRoot();
	 ...
	 * 
	 * @param listener
	 *            The feature to be added to the DataSelectListener attribute
	 * @exception java.util.TooManyListenersException
	 *                Description of the Exception
	 */
	public void addDataSelectListener(final DataSelectListener listener)
	 ...
	 *            Description of the Parameter
	 */
	protected void notifyNodeSelectListeners(final DataObjectProxy proxy,
	 ...
	 *            Description of the Parameter
	 */
	protected void notifyNodeSelectListeners(final DataModel model,
			final MutableTreeNode node) {
		if (listener != null) {
	 ...
	 *            Description of the Parameter
	 */
	protected void notifyNodeSelectListeners(final GroupDataSource source,
			final MutableTreeNode node) {
		if (listener != null) {
	 ...
	 * 
	 * @param source
	 *            The feature to be added to the DataSource attribute
	 */
	...
	public void removeDataSource(final GroupDataSource source) {
		...
		final DataSourceListener dsl = (DataSourceListener) source_nodes_to_listener
				.get(source);
		...
		if (dsl != null) {
			...
			throw new IllegalArgumentException(
					"Trying to remove DataSource that is not present!\n"
							+ source);
		}
	 ...
	 *                Description of the Exception
	 */
	public final boolean visualizeProxy(final DataObjectProxy proxy)
			throws java.text.ParseException, java.io.IOException {
		final DataModel model = proxy.getDataModel();

		final boolean use_browser = GPpropertiesManager.getBooleanProperty(
				"open.files.system.browser", true);
	 ...
	 *                Description of the Exception
	 */
	protected static void showWithNativeBrowser(final DataObjectProxy proxy)
			throws java.io.IOException {
		final AbstractDataSource source = (AbstractDataSource) proxy
				.getDataSource();
		final java.io.File file = source.getAsLocalFile(proxy);
		System.out.println("Browser loading file: " + file);
		org.genepattern.util.BrowserLauncher.openURL("File://"
	 ...
	 *                Description of the Exception
	 */
	protected static void showWithJavaBrowser(final DataObjectProxy proxy)
			throws java.io.IOException {
			...
			public void run() {
				try {
					...
					JOptionPane.showMessageDialog(
							...
							"Not enough memory available to view " + proxy);
					frame.dispose();
				}
			}
		}.start();
	 ...
	 *                Description of the Exception
	 */
	protected static boolean handleWithNativeBrowser(final DataObjectProxy proxy)
			throws java.io.IOException {
		final String value = GPpropertiesManager
				.getProperty("system.browser.handle.exts",
						".html, .htm, .jsp, .asp, .jpeg, .gif, .jpg, .png, .bmp, .tiff");
		...
		final String ext = org.genepattern.util.ExampleFileFilter
				.getExtension(file);
		if (ext == null) {
			return false;
		}
		final String[] exts = StringUtils.splitStrings(value.trim()
				.toLowerCase(), ',');
	...
	protected class DataSourceListener implements DataSourceUpdateListener {
		...
		protected DataSourceListener(final GroupDataSource source,
				final DataSourceListener dependent) {
			this(
					new DefaultMutableTreeNode(source, true /* can have children */),
					source.getParent(), dependent);
			source.addDataSourceUpdateListener(this);
		}

		private DataSourceListener(final DefaultMutableTreeNode node,
				final GroupDataSource parent, final DataSourceListener dependent) {
			this.source_node = node;
			this.dependent = dependent;
			type_nodes = new HashMap();
			proxy_node = new HashMap();
			if (node != root) {
				final DefaultMutableTreeNode parent_node = (parent == null) ? root
						: getNodeFromUserObj(parent);
				Runnable r = new Runnable() {
					public void run() {
						((DefaultTreeModel) data_tree.getModel())
								.insertNodeInto(node, parent_node, parent_node
										.getChildCount());
					}
				};
				SwingUtilities.invokeLater(r);
		...
		private DefaultMutableTreeNode getNodeFromUserObj(final Object user) {
			final DefaultMutableTreeNode node = (DefaultMutableTreeNode) user_node
					.get(user);
			if (node == null) {
				throw new IllegalArgumentException(
						"User object not associated with a node!\n" + user);
			}
			return node;
		}

		/**
		 * adds a DataObject to the tree if a node for it's type doesn't exist
		 * is created
		 * 
		 * @param proxy
		 *            The feature to be added to the DataObject attribute
		 */
		protected void addDataObject(final DataObjectProxy proxy) {
			if (proxy == null) {
				throw new NullPointerException(
						"The DataObjectProxy cannot be null!");
			}
			final DataModel model = proxy.getDataModel();
		 ...
		 * 
		 * @param model
		 *            The feature to be added to the ChildNode attribute
		 * @param proxy
		 *            The feature to be added to the ChildNode attribute
		 */
		private void addChildNode(final DataModel model,
				final DataObjectProxy proxy) {
			...
			for (int i = 0, children = finalFirstOrderNode.getChildCount(); i < children; i++) {
				DefaultMutableTreeNode node = (DefaultMutableTreeNode) finalFirstOrderNode
						.getChildAt(i);
			...
			Runnable r = new Runnable() {
				public void run() {
					((DefaultTreeModel) data_tree.getModel()).insertNodeInto(
							child, finalFirstOrderNode, finalFirstOrderNode
									.getChildCount());
				}
			};
			SwingUtilities.invokeLater(r);
		...
		protected DefaultMutableTreeNode createTypeNode(final DataModel model) {
			...
			Runnable r = new Runnable() {
				public void run() {
					try {
						((DefaultTreeModel) data_tree.getModel())
								.insertNodeInto(node, source_node, source_node
										.getChildCount());
					} catch (NullPointerException npe) {
					}// jgould fail silently
				}
			};
			SwingUtilities.invokeLater(r);
			return node;
		 ...
		 *            Description of the Parameter
		 */
		public void updateAddDataObjects(final DataObjectProxy[] proxies,
			...
			final SwingWorker worker = new SwingWorker() {
				public Object construct() {
					//System.out.println("Adding "+limit+" DataObjectProxy objs
					// to the tree");
					for (int i = 0; i < limit; i++) {
		 ...
		 *            Description of the Parameter
		 */
		public void updateRemoveDataObjects(final DataObjectProxy[] proxies,
				final Object[] parents) {
			...
			final SwingWorker worker = new SwingWorker() {
				public Object construct() {
					final int limit = proxies.length;
					System.out.println("Adding " + limit
							+ " DataObjectProxy objs to the tree");
					for (int i = 0; i < limit; i++) {
						final DataObjectProxy proxy = proxies[i];
						final DefaultMutableTreeNode node = (DefaultMutableTreeNode) proxy_node
								.get(proxy);
						System.out.println("removeDO: proxy=" + proxy
						...
						if (parent.isLeaf()) {// empty project directory
							org.genepattern.data.DataModel model = (org.genepattern.data.DataModel) parent
									.getUserObject();
					...
					((DefaultTreeModel) data_tree.getModel())
							.nodeStructureChanged(source_node);
	...
	private final class PopupMenuHandler implements MouseInputListener {
		...
		PopupMenuHandler() {
			...
			final ActionListener aal = new ActionListener() {
				public final void actionPerformed(
					...
					if (item == load) {
						...
						dataObjectBrowser
								.showSaveDialog((DefaultDataObjectProxy) user);
					} else if (item == paste) {
						...
						new Thread() {
							public void run() {
								try {
									final File file = ((AbstractDataSource) proxy
											.getDataSource())
											.getAsLocalFile(proxy);
					...
					} else if (item == remove) {// remove from list of files in
												// gp properties file
						final DefaultMutableTreeNode selected_node = (DefaultMutableTreeNode) data_tree
								.getSelectionPath().getLastPathComponent();
						if (selected_node.getUserObject() instanceof DirDataSource) {// remove
																					 // project
																					 // directory
							final DirDataSource dds = (DirDataSource) selected_node
									.getUserObject();
							final int choice = JOptionPane
									.showConfirmDialog(
											...
											"Do you want to remove \""
													+ dds
													+ "\" from the list of Project Directories?",
											"Remove",
							...
							final String dir_prop = GPpropertiesManager
									.getProperty("gp.project.dirs");
							int index = dir_prop.indexOf(location + ',');
							if (index >= 0) {
								final String new_prop = StringUtils.replaceAll(
										dir_prop, location + ',', "");
								GPpropertiesManager.setProperty(
										"gp.project.dirs", new_prop);
							} else {
								final String new_prop = StringUtils.replaceAll(
										dir_prop, location, "");
								GPpropertiesManager.setProperty(
							...
							org.genepattern.gpge.io.DataSources.instance()
									.removeDataSource(dds);
							((DefaultTreeModel) data_tree.getModel())
									.removeNodeFromParent(selected_node);
						} else {// remove job
							org.genepattern.gpge.io.ServerJobDataSource ds = (org.genepattern.gpge.io.ServerJobDataSource) selected_node
									.getUserObject();
							org.genepattern.webservice.AnalysisJob job = ds
									.getJob();
							int choice = JOptionPane.showConfirmDialog(
									GenePattern.getDialogParent(),
									"Do you want to remove job number " + job
											+ " from your job results?",
							...
							if (selected_node.getParent().getChildCount() == 1) {
								DefaultMutableTreeNode parent = (DefaultMutableTreeNode) selected_node
										.getParent();
								org.genepattern.gpge.io.ServerTaskDataSource pds = (org.genepattern.gpge.io.ServerTaskDataSource) parent
										.getUserObject();
						...
						final DefaultMutableTreeNode selected_node = (DefaultMutableTreeNode) data_tree
								.getSelectionPath().getLastPathComponent();
						final GroupDataSource gds = (GroupDataSource) selected_node
								.getUserObject();
						try {
							gds.refresh();
						} catch (java.io.IOException ex) {
		...
		private void doPopup(final MouseEvent e) {
			...
			final TreePath path = data_tree.getPathForLocation(e.getX(), e
					.getY());
			if (path == null) {
				return;
			}
			...
			if (user.getClass().equals(DirDataSource.class)) {// should be
															  // TargetDataSource
															  // or some other
															  // like interface
			...
			} else if (user.getClass().equals(
					org.genepattern.gpge.io.ServerJobDataSource.class)) {
				showPopup(e, false, false, false, true);
			}
		}

		private void showPopup(final MouseEvent e, final boolean copy_enabled,
				final boolean paste_enabled, final boolean is_data_source,
