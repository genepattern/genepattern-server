 ...
 * @created May 18, 2004
 */
public class MyVisualizerSubmitter implements
	 ...
	 * @exception org.genepattern.webservice.PropertyNotFoundException
	 *                Description of the Exception
	 */
	public MyVisualizerSubmitter(final DataObjectBrowser browser)
	...
	public AnalysisJob submitTask(final AnalysisService selectedService,
			...
			if (ParameterInfo.CACHED_INPUT_MODE.equals(paramInfos[i]
				...
				substitutions.put(paramInfos[i].getName(),
						org.genepattern.gpge.io.ServerFileDataSource
								.getFileDownloadURL(paramInfos[i].getValue()));
			} else {
		...
		new Thread() {
			public void run() {
				...
				try {
					executor = new JavaGELocalTaskExecutor(dataObjectBrowser,
							task_info, substitutions,
							dataObjectBrowser.username, server);
					executor.exec();
				} catch (Exception e) {
					e.printStackTrace();
