 ...
 * @author kohm
 */
public class VisualizerTaskSubmitter implements
		org.genepattern.gpge.ui.tasks.TaskSubmitter {

	/**
	 * Creates a new instance of VisualizerTaskSubmitter Communicates the
	 * className via the TaskInfo as an Attribute
	 */
	public VisualizerTaskSubmitter(final DataObjectBrowser browser)
			throws org.genepattern.webservice.PropertyNotFoundException {
		this.messenger = browser;
		try {
			final Properties p = org.genepattern.util.PropertyFactory
					.getInstance().getProperties("omnigene.properties");
			final String url_string = p.getProperty("analysis.service.URL");
			if (url_string == null)
				throw new org.genepattern.webservice.PropertyNotFoundException(
						"Cannot get the analysis service URL!");
		...
		} catch (java.io.IOException ioe) { // catches
											// (java.net.MalformedURLException
											// mfue)
			throw new IllegalStateException(
					"Error: While creating URL of server string:\n"
							+ ioe.getMessage());
		}
	...
	public boolean check(final AnalysisService selectedService, final int id,
	 ...
	 * @returns null no job
	 */
	public AnalysisJob submitTask(final AnalysisService selectedService,
			final int id, final ParameterInfo[] paramInfos,
			final AnalysisWebServiceProxy handler) throws OmnigeneException,
			WebServiceException, java.io.IOException {
		...
		System.out.println("\nTask name=" + taskName);
		final String className = (String) task_info.getTaskInfoAttributes()
				.get("className");
		if ("edu.mit.wi.omnigene.service.analysis.genepattern.GenePatternAnalysisTask"
				.equals(className)) {
			javax.swing.JOptionPane.showMessageDialog(
					org.genepattern.gpge.GenePattern.getDialogParent(),
					taskName + " is not a correctly defined Java visualizer.");
			return null;
		}
		...
		for (int i = 0; i < limit; i++) {
			...
			if (ParameterInfo.CACHED_INPUT_MODE.equals(mode)) {
				...
				final InputStream in = ServerFileDataSource
						.createRemoteFileInputStream(value);
				...
				in.close();
				out.close();
			} else if (ParameterInfo.URL_INPUT_MODE.equals(mode)) { // a URL
																	// string
				System.out.println("url file - value:\n" + value);
				java.net.URL url = new java.net.URL(value);
				final String f_name = ServerFileDataSource
						.getJustFileName(value);
				final File tmp_file = StorageUtils.createTempFileNoMung(f_name);
				final FileOutputStream out = new FileOutputStream(tmp_file);
		...
		// {"/Users/kohm/data/res_files/test_KWO.res"};
		...
		try {
			...
			// classPath, false/*debug off*/);
			final String path_to_run = new File(PATH_TO_RUNGP)
					.getAbsolutePath();
			...
			new Thread(new org.genepattern.util.RunLater() {
				protected final void runIt() throws Exception {
					...
					if (errorGobbler.tmp_file.length() > 0L) {
						...
						org.genepattern.gpge.GenePattern.showError(null,
								taskName + " error:\n" + error_text);
					}
				}
			}).start();
	...
	// helper methods
	/**
	 * retrieves the class path note only works for Visualizer Tasks
	 */
	private String findClassPath(final AnalysisService selectedService,
			final ParameterInfo[] paramInfos) throws OmnigeneException {
		final int limit = paramInfos.length;
		System.out.println("ParameterInfo: ");
		for (int i = 0; i < limit; i++) {
			final ParameterInfo param_info = paramInfos[i];
			final String name = param_info.getName().trim();
			System.out.println("name ='" + name + "' label='"
					+ param_info.getLabel() + "' description='"
					+ param_info.getDescription() + "' value='"
					+ param_info.getValue() + "'");
			if (CLASS_NAME_KEY.equalsIgnoreCase(name))
				return param_info.getDescription();
		}
		throw new OmnigeneException(
				"Cannot find the class path for the Visualizer, "
	...
	public static void main(String[] args) throws Throwable {
		...
		//        System.setProperty("omnigene.conf",
		// home+separator+".gp"+separator+"resources"+separator);
	...
	// fields
	/**
	 * the GPConstants.TASK_TYPE should return a value of "visualizer" for
	 * Visualizers
	 */
	...
	/**
	 * static initializer Finds the path to the jar file or the dir where edu/
	 * is
	 */
	static {
		...
		if (new File(path, "javaw").exists()
				|| new File(path, "javaw.exe").exists()) {
			path = path + "javaw";

		} else
			path = path + "java";
		// is there a space in the path and is it one of Microsoft's OS products
		// then quote it
		//        if( path.indexOf(' ') >= 0 &&
		...
		final java.net.URL rgptm = org.genepattern.gpge.util.JWhich
				.findClass(RunGenePatternTaskMain.class.getName());
		...
		if (path_to_rungp.indexOf(space_rep) >= 0)
			path_to_rungp = StringUtils.replaceAll(path_to_rungp, space_rep,
					space);
