...
public class RunPipeline {
   ...
   private RunPipeline() {} // prevent instantiation
/**
* expects minimum of two args.
* pipeline name, username, args to pipeline 
*/
public static void main(String args[]) throws Exception {
	...
	if (args.length > 2){
		for (int i=2; i < args.length; i++){
			...
			while (strtok.hasMoreTokens()){
				...
				if ( (strtok.hasMoreTokens())) valbuff.append("=");
				count++;
			}
	...
	setUp(userID);
	runPipelineModel(getPipelineModel(pipelineFileName), additionalArguments);

}

/**
 * the pipelineFileName may be either a local file or a URL.  Figure out which it is and 
 * get it either way
 */
public static PipelineModel getPipelineModel(String pipelineFileName) throws Exception{
...
protected static void runPipelineModel(PipelineModel model, Properties args) throws Exception {
	...
	for (Enumeration eTasks = vTasks.elements(); eTasks.hasMoreElements(); taskNum++) {
		...
		try {
			...
			notifyPipelineOfOutputFiles(taskResult, jobSubmission.getName(), taskNum);			

			decorator.recordTaskCompletion(taskResult, jobSubmission.getName()+(taskNum+1));
			results[taskNum] = taskResult;
			
		} catch (Exception e) {
			System.err.println("execution for " + jobSubmission.getName() + " task failed:");
...
* belong to the pipeline job.
* stdout is always the next to last output file, and stderr is the very last (when they exist)
*/
protected static void notifyPipelineOfOutputFiles(JobInfo jobInfo, String taskName, int taskNum) throws Exception {
	...
	for(int j=0; j<jobParams.length; j++){
		String fullFileName = "";
	       if(jobParams[j].isOutputFile()){
			...
			if (lastIdx != -1) fileName = fileName.substring(lastIdx+1);
			lastIdx = fileName.lastIndexOf("/");
			if (lastIdx != -1) fileName = fileName.substring(lastIdx+1);
			fullFileName=jobInfo.getJobNumber()+File.separator+fileName;			
			
			String updateUrl = GP_URL + "updatePipelineStatus.jsp?jobID=" + System.getProperty("jobID") + "&" + GPConstants.NAME + "=" + taskName + (taskNum+1);
	...
	if (stdout != null) {
		HttpURLConnection uconn = (HttpURLConnection)stdout.openConnection();
      		int rc = uconn.getResponseCode();
	}
	if (stderr != null) {
		HttpURLConnection uconn = (HttpURLConnection)stderr.openConnection();
      		int rc = uconn.getResponseCode();
	}
}
...
protected static void setStatus(int status) throws Exception {
	String statusUrl = GP_URL + "updatePipelineStatus.jsp?jobID=" + System.getProperty("jobID") + "&jobStatus=" + status;
 ...
 */
protected static JobInfo executeTask(JobSubmission jobSubmission, ParameterInfo[] params, int taskNum, JobInfo[] results) throws Exception{

	if (jobSubmission.isVisualizer()) return new JobInfo(); // note: visualizers not checked to see if found on server
   ...
   if(task==null) {
      System.err.println("Task " + lsidOrTaskName + " not found."); // write to stderr file
      System.exit(0);
   }
...
public static ParameterInfo[] setInheritedJobParameters(ParameterInfo[] parameterInfo, JobInfo[] results){
...
protected static String getInheritedFilename(Map attributes, JobInfo[] results){
	// these params must be removed so that the soap lib doesn't try to send the
	...
	String url = GP_URL + "retrieveResults.jsp?job="+job.getJobNumber()+"&filename=" + fileName;
	return url;
}
...
*/
public static ParameterInfo[] setJobParametersFromArgs(String name ,int taskNum,ParameterInfo[] parameterInfo, JobInfo[] results, Properties  args){
	for (int i=0; i < parameterInfo.length; i++){
		ParameterInfo aParam = parameterInfo[i];
		if (aParam.getAttributes() != null) {
			if (aParam.getAttributes().get(PipelineModel.RUNTIME_PARAM) != null){
				...
				//System.out.println("looking for " + key + " found " + val + " in " + args);
				aParam.setValue(val);
			} 
		}
	}
...
*/
public  static String getOutputFileName(org.genepattern.server.analysis.JobInfo job, String fileStr){
...
/**
  ...
  */
public static JobInfo waitForErrorOrCompletion(AnalysisJob job) throws Exception{
	int maxtries = 100; // used only to increment sleep, not a hard limit
	int sleep = 1000;
	return waitForErrorOrCompletion(job, maxtries, sleep);
}

public static JobInfo waitForErrorOrCompletion(AnalysisJob job, int maxTries, int initialSleep) throws Exception{
	String status = "";
	JobInfo info = null;
	int count = 0;
...
/**
* make the sleep time go up as it takes longer to exec.
* eg for 100 tries of 1000ms (1 sec)
* first 20 are 1 sec each
* next 20 are 2 sec each
* next 20 are 4 sec each
* next 20 are 8 sec each
* any beyond this are 16 sec each
*/
protected static int incrementSleep(int init, int maxTries, int count){
	if (count < (maxTries * 0.2)) return init;
	if (count < (maxTries * 0.4)) return init*2;
	if (count < (maxTries * 0.6)) return init*4;
	if (count < (maxTries * 0.8)) return init*8;
	return init*16;
}
...
protected static void setUp(String userID) { 
	try {
		...
		Properties genepatternProps  = property.getProperties("genepattern.properties");
//		GP_URL = genepatternProps.getProperty("GenePatternURL");
		GP_URL = "http://" + InetAddress.getLocalHost().getCanonicalHostName() + ":" + genepatternProps.getProperty("GENEPATTERN_PORT") + "/gp/";
...
/*******************
		...
		classPath = classPath + genepatternProps.getProperty("webappDir") + "/WEB-INF/lib/hsqldb.jar";
