...
/**
 * ResultsPanel.java
 * <p>Description: Displays the result of analysis jobs. </p>
 * @author Hui Gong
...
public class ResultsPanel extends JPanel implements Observer{
    ...
    private static Category cat = Category.getInstance(ResultsPanel.class.getName());
     ...
     * @param o <code>Observable<code> object, it's <code>DataModel<code> here
     * @param arg
     */
    ...
    public void createTreeNodes(final DefaultMutableTreeNode the_root, final TreeNodeFactory node_factory){
        ...
        for(final Enumeration e = results.keys(); e.hasMoreElements(); ){
            ...
            final ResultTreeNode site = new SiteNode(siteName, this._dataModel, this._treeModel);
            final Hashtable jobsOnsite = (Hashtable)results.get(siteName);
            
            for(final Enumeration enu = jobsOnsite.keys(); enu.hasMoreElements(); ){
                final String jobType = (String)enu.nextElement();
                cat.debug("jobType: "+jobType);
                final ResultTreeNode category = new TaskNode(jobType, this._dataModel, this._treeModel);
                //getting jobs
                final Vector jobs = (Vector)jobsOnsite.get(jobType);
                for(final Iterator iterator = jobs.iterator(); iterator.hasNext(); ){
                    final AnalysisJob job = (AnalysisJob)iterator.next();
                    final ResultTreeNode jobNode = new JobNode(job, this._dataModel, this._treeModel, this._result);
                    if(job.getJobInfo().containsOutputFileParam()){
                        ...
                        for(int i=0; i< params.length; i++){
                            if(params[i].isOutputFile()){
                                ...
                                final MutableTreeNode fileNode = node_factory.createNode(new FileNode(file, _dataModel, _treeModel, _result, job.getJobInfo().getUserId(), null));
...
//            ResultTreeNode site = new SiteNode(siteName, this._dataModel, this._treeModel);
...
//                cat.debug("jobType: "+jobType);
//                ResultTreeNode category = new TaskNode(jobType, this._dataModel, this._treeModel);
//                //getting jobs
//                Vector jobs = (Vector)jobsOnsite.get(jobType);
//                Iterator iterator = jobs.iterator();
//                while(iterator.hasNext()){
//                    AnalysisJob job = (AnalysisJob)iterator.next();
//                    ResultTreeNode jobNode = new JobNode(job, this._dataModel, this._treeModel, this._result);
//                    if(job.getJobInfo().containsOutputFileParam()){
//                        ParameterInfo[] params = job.getJobInfo().getParameterInfoArray();
//                        for(int i=0; i< params.length; i++){
//                            if(params[i].isOutputFile()){
...
//                                ResultFile file = new ResultFile(siteName, job.getJobInfo().getJobNumber(), filename);
//                                ResultTreeNode fileNode = new FileNode(file, this._dataModel, this._treeModel, this._result);
//                                jobNode.add(fileNode);
//                            }
//                        }
//                    }
