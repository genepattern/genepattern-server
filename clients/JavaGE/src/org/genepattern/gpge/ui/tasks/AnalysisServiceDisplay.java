...
public class AnalysisServiceDisplay extends JPanel implements TaskDisplay {
	...
	JPanel topPanel = new TaskNamePanel(taskInfo, ChangeViewMessageRequest.SHOW_RUN_TASK_REQUEST);
	...
	if (isPipeline) {
	    ...
	    try {
		PipelineModel model = PipelineModel.toPipelineModel(((String) selectedService.getTaskInfo()
			.getTaskInfoAttributes().get(GPConstants.SERIALIZED_MODEL)));

		List<JobSubmission> tasks = model.getTasks();

		if (tasks != null) {
		    for (int i = 0; i < tasks.size(); i++) {
			runToComboBox.addItem((i + 1) + ". " + tasks.get(i).getName());
		    }
		}
		runToComboBox.setSelectedIndex(runToComboBox.getItemCount() - 1);
		...
		public void actionPerformed(ActionEvent e) {
		    MessageManager.notifyListeners(new ChangeViewMessageRequest(this,
			    ChangeViewMessageRequest.SHOW_EDIT_PIPELINE_REQUEST, selectedService));
		}
	    });
		...
		public void actionPerformed(ActionEvent e) {
		    MessageManager.notifyListeners(new ChangeViewMessageRequest(this,
			    ChangeViewMessageRequest.SHOW_VIEW_PIPELINE_REQUEST, selectedService));
		}
	    });
	...
	final JComboBox viewCodeComboBox = new JComboBox(new String[] { "- Language -", "Java", "MATLAB", "R" });
	...
	/*
	 * JTaskPaneGroup group = new JTaskPaneGroup(); group.setText("Advanced Options"); JTaskPane tp = new
	 * JTaskPane(); group.add(viewCodePanel); tp.add(group);
	 */

	bottomPanel.setBorder(GUIUtil.createBorder(UIManager.getLookAndFeelDefaults().getBorder("ScrollPane.border"),
		0, 0, 0, 2));
	...
	viewCodeButton.addActionListener(new ActionListener() {
		...
		JobInfo jobInfo = new JobInfo(-1, -1, null, null, null, parameterInfoPanel.getParameterInfoArray(),
			AnalysisServiceManager.getInstance().getUsername(), lsid, selectedService.getTaskInfo()
				.getName());
		AnalysisJob job = new AnalysisJob(selectedService.getServer(), jobInfo, TaskLauncher
			.isVisualizer(selectedService));
     ...
     * 
     * @param parameterName
     *                the unencoded parameter name as returned by ParameterInfo.getName()
     * @param parameterValue
     *                the parameter value. If the parameter contains a choice list, the value can be either the UI value
     *                of the command line value
     */
    ...
    public static void doSubmit(JButton source, final ParameterInfo[] actualParameterArray,
	...
	try {
	    ...
	    new CLThread() {
		public void run() {
		    RunTask rt = new RunTask(selectedService, actualParameterArray, AnalysisServiceManager
	...
	public final void actionPerformed(ActionEvent ae) {
	    ...
	    ParameterInfo[] actualParameterArray = parameterInfoPanel.getParameterInfoArray();
	    if (runToComboBox != null) {
		int taskNumber = runToComboBox.getSelectedIndex() + 1;
		ParameterInfo p = new ParameterInfo("PIPELINE_ARG.StopAfterTask", "" + taskNumber, "");
		ParameterInfo[] temp = new ParameterInfo[actualParameterArray.length + 1];
		System.arraycopy(actualParameterArray, 0, temp, 0, actualParameterArray.length);
