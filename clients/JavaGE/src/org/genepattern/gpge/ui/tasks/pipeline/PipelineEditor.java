...
// params start at one
public class PipelineEditor extends JPanel implements TaskDisplay, PipelineListener {
	...
	MessageManager.addGPGEMessageListener(new GPGEMessageListener() {
		...
		if (message instanceof RefreshMessage && isShowing()) {
			...
			AnalysisService svc = AnalysisServiceManager.getInstance().getAnalysisService(model.getLSID());
			if (svc == null) {
		...
		if (e.getSource() == exportButton) {
			...
			if (result == JOptionPane.YES_OPTION) {
				...
				MessageManager.notifyListeners(new ChangeViewMessageRequest(this,
					ChangeViewMessageRequest.SHOW_RUN_TASK_REQUEST, analysisService));
			    }
			}
		    } else {
			...
			if (result == JOptionPane.YES_OPTION) {
			    ...
			    MessageManager
				    .notifyListeners(new ChangeViewMessageRequest(this, message, analysisService));
			}
		    } else {
			MessageManager.notifyListeners(new ChangeViewMessageRequest(this, message, analysisService));
		    }
		}
	    }
	...
	for (int i = 0; i < taskDisplayList.size(); i++) {
		...
		if (model.isInputFile(i, j)) {
			...
			// be
			// null
			model.setInheritedFile(i, j, inheritedTaskIndex, inheritedFileName);
	...
	for (int j = 0; j < model.getParameterCount(taskIndex); j++) {
		...
		model.setInheritedFile(taskIndex, j, inheritedTaskIndex, inheritedFileName);
	...
	model.resetDocFiles();
	try {
	    MessageManager.notifyListeners(new TaskInstallMessage(this, model.getLSID()));
	...
	invalidate();
	validate();
	this.analysisService = AnalysisServiceManager.getInstance().getAnalysisService(model.getLSID());
	enableButtons();
	try {
	    GenePattern.showMessageDialog("Saved " + model.getPipelineName() + " version "
		    + new LSID(model.getLSID()).getVersion());
	} catch (MalformedURLException e) {
	    e.printStackTrace();
	}
    }
     ...
     * 
     * @param saveTask
     *                if <tt>true</tt> save the task on the server, otherwise update the paramter values in the model
	...
	for (int i = 0; i < taskDisplayList.size(); i++) {
		...
		if (inheritedTaskIndex != -1) {
			...
			errors.append("Missing value for " + (i + 1) + ". " + model.getTaskName(i) + " "
				+ AnalysisServiceDisplay.getDisplayString(model.getParameterName(i, j)) + "\n");
		    }
		    model.setInheritedFile(i, j, inheritedTaskIndex, inheritedFileName);
		} else if (pd.isPromptWhenRun()) {
		    model.setPromptWhenRun(i, j);
			...
			if (file.exists()) {
			    ...
			    String fileName = value.substring("<GenePatternURL>getFile.jsp?task=<LSID>&file=".length(),
				    value.length());
	...
	try {
	    ...
	    String lsid = new TaskIntegratorProxy(AnalysisServiceManager.getInstance().getServer(),
		    AnalysisServiceManager.getInstance().getUsername(), AnalysisServiceManager.getInstance()
			    .getPassword(), false).modifyTask(accessId, ti.getName(), ti.getDescription(), ti
		    .getParameterInfoArray(), (HashMap) ti.getTaskInfoAttributes(), (File[]) taskFiles
		    .toArray(new File[0]), (String[]) existingFileNames.toArray(new String[0]));
	...
	for (int i = 0; i < model.getTaskCount(); i++) {
		...
		if (model.isInputFile(i, j)) {
		    list.add((i + 1) + ". " + model.getTaskName(i) + " "
			    + AnalysisServiceDisplay.getDisplayString(model.getParameterName(i, j)));
		}
	    }
	}
	...
	String parameterName = taskNameAndParamName.substring(taskNameAndParamName.indexOf(" ") + 1,
		taskNameAndParamName.length());
	...
	if (svc == null) {
	    ...
	    model = new PipelineEditorModel(svc, pipelineModel, !view);
	}
	...
	public HeaderPanel(final PipelineEditorModel model, final AnalysisService analysisService, JPanel buttonPanel,
	    ...
	    descriptionField = view ? GUIUtil.createLabelLikeTextField(model.getPipelineDescription(), 40)
		    : new JTextField(model.getPipelineDescription(), 40);
	    ...
	    CellConstraints cc = new CellConstraints();
	    JPanel temp = new JPanel(new FormLayout("right:pref:none, 3dlu, right:pref:none, 3dlu, pref, pref",
		    "pref, 3dlu, pref, pref"));
	    ...
	    authorField = view ? GUIUtil.createLabelLikeTextField(model.getAuthor(), 40) : new JTextField(model
		    .getAuthor(), 40);
	    detailsPanel.add(authorLabel, cc.xy(1, 1));
	    detailsPanel.add(authorField, cc.xy(3, 1));

	    JLabel ownerLabel = new JLabel("Owner:");
	    ownerField = view ? GUIUtil.createLabelLikeTextField(model.getOwner(), 40) : new JTextField(model
		    .getOwner(), 40);
	    ...
	    versionField = view ? GUIUtil.createLabelLikeTextField(model.getVersionComment(), 40) : new JTextField(
		    model.getVersionComment(), 40);
		...
		public void actionPerformed(ActionEvent e) {
		    ...
		    if (GUIUtil.showConfirmDialog("Are you sure you want to delete "
	...
	tasksInPipelineComboBox.removeAllItems();
	for (int i = 0; i < model.getTaskCount(); i++) {
	    tasksInPipelineComboBox.addItem((i + 1) + ". " + model.getTaskName(i));
	}
    }
	...
	if (addAfter) {
		...
		title = "Add Module After " + (jobSubmissionIndex + 1) + ". " + model.getTaskName(jobSubmissionIndex);
	    }
	    insertionIndex = jobSubmissionIndex + 1;
	} else {
	    title = "Add Module Before " + (jobSubmissionIndex + 1) + ". " + model.getTaskName(jobSubmissionIndex);
	    insertionIndex = jobSubmissionIndex;
	}

	new TaskChooser(GenePattern.getDialogParent(), title, this, insertionIndex);
    }
	...
	if (e.getType() == PipelineEvent.REPLACE) {
	    TaskPanel deletedTask = (TaskPanel) taskDisplayList.remove(e.getFirstRow());
	    deletedTask.setVisible(false);
	    tasksPanel.remove(deletedTask.getParent());
	...
	int result = GUIUtil.showYesNoCancelDialog(GenePattern.getDialogParent(), "GenePattern",
		"Do you want to save the pipeline before exiting the editor?", new String[] { "Save", "Don't Save",
			"Cancel" });
	return result;
    }
	...
	public ParameterDisplay(PipelineEditorModel model, int taskIndex, int parameterIndex, boolean view) {
	    this.model = model;
	...
	int getInheritedTaskIndex() {
	    if (useOutputFromPreviousTask == null || !useOutputFromPreviousTask.isVisible()
	...
	JCheckBox createUseOutputFromPreviousTaskCheckBox() {
	    useOutputFromPreviousTask = new JCheckBox("Use output from previous task");
	    useOutputFromPreviousTask.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
		    /*
		     * if(useOutputFromPreviousTask.isSelected()) { String item = (String)
		     * inheritedTaskIndexComboBox.getSelectedItem(); if(item!=null && !item.equals(CHOOSE_TASK)) { int
		     * inheritedIndex = Integer.parseInt(item.substring(0, item.indexOf(".")));
		     * model.setInheritedFile(taskIndex, parameterIndex, inheritedIndex, (String)
	...
	private void select() {
	    ...
	    inheritedTaskIndexComboBox.setVisible(useOutputFromPreviousTask.isSelected());
	    inheritedFileNameComboBox.setVisible(useOutputFromPreviousTask.isSelected()
	...
	private void _setTaskIndex(int taskIndex) {
		...
		inheritedTaskIndexComboBox.addItem((k + 1) + ". " + model.getTaskName(k));
	    }

	    int inheritedTaskIndex = model.getInheritedTaskIndex(taskIndex, parameterIndex);
	    ...
	    if (inheritedTaskIndex != -1 && inheritedTaskIndex != PipelineEditorModel.CHOOSE_TASK_INDEX) {
		inheritedTaskIndexComboBox.setSelectedItem((inheritedTaskIndex + 1) + ". "
			+ model.getTaskName(inheritedTaskIndex));
		updateOutputFileChoices();
		inheritedFileNameComboBox.setSelectedItem(model.getInheritedFile(taskIndex, parameterIndex));

	    } else {
		((JTextComponent) inputField).setText(model.getValue(taskIndex, parameterIndex));
	    }

	}

	void updateOutputFileChoices() {
	    String selectedTaskIndex = (String) inheritedTaskIndexComboBox.getSelectedItem();
	    int inheritedTaskIndex = Integer.parseInt(selectedTaskIndex.substring(0, selectedTaskIndex.indexOf("."))) - 1;
	...
	public JPanel createInputField() {
	    CellConstraints cc = new CellConstraints();
	    String sendToString = (taskIndex + 1) + ". " + model.getTaskName(taskIndex) + "."
		    + AnalysisServiceDisplay.getDisplayString(model.getParameterName(taskIndex, parameterIndex));
	    ...
	    FormLayout inputPanelLayout = new FormLayout(
		    "left:pref:none, left:pref:none, left:pref:none, left:pref:none, left:default:none", "pref"); // input
														    // field,
														    // browse,
														    // task
														    // drop
	...
	public JComponent createChoiceInput(ParameterChoice[] choices, String value) {
	...
	Border selectedBorder = new CompoundBorder(new ShadowBorder(getBackground()), new CompoundBorder(BorderFactory
		.createLineBorder(new Color(56, 117, 215), 3), BorderFactory.createEmptyBorder(4, 4, 4, 4)));

	Border unselectedBorder = new CompoundBorder(new ShadowBorder(getBackground()), new CompoundBorder(
		BorderFactory.createLineBorder(Color.GRAY, 3), BorderFactory.createEmptyBorder(4, 4, 4, 4)));
	...
	public TaskPanel(int _taskIndex) {
	    ...
	    if (view) {
		descriptionField = GUIUtil.createWrappedLabel(model.getTaskDescription(taskIndex));
		((JTextArea) descriptionField).setColumns(40);

	    } else {
		descriptionField = new JTextField(model.getTaskDescription(taskIndex), 40);
	    }
	    minorPanel = new JPanel();
	    minorPanel.setBackground(Color.white);
	    minorPanel.add(descriptionField);

	    togglePanel = new GroupPanel((taskIndex + 1) + ". " + model.getTaskName(taskIndex), minorPanel);
	    ...
	    CellConstraints cc = new CellConstraints();
	    setBackground(Color.white);
	    layout = new FormLayout("left:pref, 3dlu, right:pref, 3dlu, default:grow", "");
	    setLayout(layout);
	    addTaskParameters();
	...
	public void setTaskIndex(int taskIndex) {
	    ...
	    togglePanel.getMajorLabel().setText((taskIndex + 1) + ". " + model.getTaskName(taskIndex));
	}
	...
	private void addTaskParameters() {
		...
		if (!view) {
		    JComboBox versionChooserComboBox = new JComboBox(versions.toArray());
			...
			public void itemStateChanged(ItemEvent e) {
			    ...
			    String newLSID = lsid.toStringNoVersion() + ":" + version;
			    updateInputFileValues();
			    updateValues(taskIndex);
			    model.replace(taskIndex, AnalysisServiceManager.getInstance().getAnalysisService(newLSID)
				    .getTaskInfo());
			}
		    ...
		    versionComponent = new JLabel("version: " + lsid.getVersion());
		    layout.appendRow(new RowSpec("pref"));
		...
		layout.appendRow(new RowSpec("pref"));
		if (!view) {
		    add(promptWhenRunLabel, cc.xyw(PROMPT_WHEN_RUN_COLUMN, layout.getRowCount(), layout
			    .getColumnCount()));
		}
	    }
	    parameters = new ParameterDisplay[model.getParameterCount(taskIndex)];
		...
		final JCheckBox promptWhenRunCheckBox = parameters[i].createPromptWhenRunCheckBox();

		promptWhenRunCheckBox.setSelected(model.isPromptWhenRun(taskIndex, i));

		togglePanel.addToggleComponent(promptWhenRunCheckBox);

		if (!view) {
		    add(promptWhenRunCheckBox, cc.xy(PROMPT_WHEN_RUN_COLUMN, layout.getRowCount(),
			    CellConstraints.LEFT, CellConstraints.CENTER));
		}

		add(label, cc.xy(INPUT_LABEL_COLUMN, layout.getRowCount(), CellConstraints.RIGHT,
		...
		if (view) {
			...
			value = "<html>Use <b>" + model.getInheritedFile(taskIndex, i) + "</b> from <b>"
			...
			valueLabel.addMouseListener(new MouseAdapter() {
				...
				TaskPanel td = (TaskPanel) taskDisplayList.get(model.getInheritedTaskIndex(taskIndex,
					parameterIndex));
				// td.togglePanel
			...
			if (choiceItems != null) {
				...
				if (choiceItems[j].equalsCmdLineOrUIValue(value)) {
				    valueLabel.setText(choiceItems[j].getUIValue());
				    break;
				}
			    }
			}
			...
			if (value.startsWith(filePrefix)) {
			    value = value.substring(filePrefix.length(), value.length());

			    String url = AnalysisServiceManager.getInstance().getServer() + "/gp/getFile.jsp?task="
				    + analysisService.getLsid() + "&file=" + value;
			    text = "<html><a href=" + url + ">" + value + "</a>";
			} else {
			    text = "<html><a href=" + value + ">" + value + "</a>";
			}
		    ...
		    add(valueComponent, cc.xy(INPUT_FIELD_COLUMN, layout.getRowCount(), CellConstraints.LEFT,
			    CellConstraints.BOTTOM));
		} else if (model.isChoiceList(taskIndex, i)) {
		    JComponent choiceInput = parameters[i].createChoiceInput(model.getChoices(taskIndex, i), model
			    .getValue(taskIndex, i));
		    add(choiceInput, cc.xy(INPUT_FIELD_COLUMN, layout.getRowCount(), CellConstraints.LEFT,
			    CellConstraints.BOTTOM));
		    ...
		    add(inputPanel, cc.xy(INPUT_FIELD_COLUMN, layout.getRowCount(), CellConstraints.LEFT,
			    CellConstraints.BOTTOM));
		} else {
		    ...
		    add(inputComponent, cc.xy(INPUT_FIELD_COLUMN, layout.getRowCount(), CellConstraints.LEFT,
			    CellConstraints.BOTTOM));
