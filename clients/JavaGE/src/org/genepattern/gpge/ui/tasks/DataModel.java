 ...
 * </p>
 * <p>
 * Description: includes all the data for the the analysis service.
 * </p>
 * 
 * @author Hui Gong
 * @version $Revision$
...
public class DataModel extends Observable implements Serializable {
	...
	private static Category cat = Category.getInstance(DataModel.class
			.getName());
	...
	public void updateStatus(AnalysisJob job) {
		...
		if (status.equals(JobStatus.PROCESSING)) {
			...
			this.setChanged();
			this
					.notifyObservers(new JobAndObserver(job,
							this.OBSERVER_HISTORY));
	...
	private void updateHistory(AnalysisJob aJob) {
		...
		for (int i = 0; i < _jobs.size(); i++) {
			...
			if (aInfo.getServer().equals(siteName) && info.getJobNumber() == id
	...
	public final void removeHistorys(final int[] locations) {
		...
		for (int i = 0; i < num; i++) {
			final AnalysisJob job = (AnalysisJob) _jobs
					.remove(locations[i] - i);
			removeResult(job);
		}
		this.setChanged();
	...
	/**
	 * Removes the job from the list of jobs with the with job number equal to
	 * job.getJobInfo().getJobNumber() and server name equal to job.getServer()
	 * 
	 * @param job
	 *            the job to remove
	 */
	public void removeJobById(AnalysisJob job) {
		...
		for (int i = 0; i < _jobs.size(); i++) {
			...
			if (temp.getJobInfo().getJobNumber() == job.getJobInfo()
					.getJobNumber()
					&& temp.getServer().equals(job.getServer())) {
				removeResult((AnalysisJob) _jobs.remove(i));
				found = true;
			}
		}
		if (!found) {
			throw new IllegalArgumentException("Job number "
					+ job.getJobInfo().getJobNumber() + " on the server "
					+ job.getServer() + " not found.");
		}
	 ...
	 * 
	 * @param parent
	 *            of the task, it's a site name.
	 * @param taskname
	 *            specified the task name for deletion.
	 */
	 ...
	 * 
	 * @param siteName
	 *            the name of the site which hosts the tasks
	 */
	...
	private void addResult(AnalysisJob job) {
		cat.debug("adding result for site:" + job.getServer() + " task: "
				+ job.getTaskName());
		String siteName = job.getServer();
		String taskName = job.getTaskName();
		Hashtable siteJobs;
	...
	public String toString() {
		return "DataModel [# jobs=" + _jobs.size() + " # sites="
				+ _results.size() + " # analysis services="
				+ _analysisServices.size() + ']';
	}

	// I N N E R C L A S S E S
	/**
	 * what job has it's status updated and which observer should be paying
	 * attention
	 */
