...
public class RunTask {
   ...
   public RunTask(AnalysisService analysisService, ParameterInfo[] actualParams, String username) {
   ...
   public void exec() {
      ...
      for(int i = 0, length = formalParams != null ? formalParams.length : 0; i < length; i++) {
         ParameterInfo actualParam = (ParameterInfo) paramName2ActualParam.get(formalParams[i].getName());
         String actualValue = actualParam!=null?actualParam.getValue():null;
      ...
      if(directoryInputs.size() > 0) {
         ...
         for(int j = 0, length = files.length; j < length; j++) {
            try {
               ...
               if(TaskLauncher.isVisualizer(analysisService)) {
                  TaskLauncher.submitVisualizer(analysisService, actualParams, username);
               } else {
         ...
         try {
            if(TaskLauncher.isVisualizer(analysisService)) {
               ...
               TaskLauncher.submitAndWaitUntilCompletionInNewThread(actualParams,
                     serviceProxy, analysisService);
            }
         } catch(Exception e) {
            throw new RunTaskException(e);
         }
