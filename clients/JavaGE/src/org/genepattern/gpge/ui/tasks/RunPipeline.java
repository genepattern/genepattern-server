...
class RunPipeline {
	 ...
	 * 
	 * @param the
	 *            analysis service to run
	 * @param directoryInputs
	 *            A list of ParameterInfo objects that contains directories as
	 *            input
	 */
	public RunPipeline(AnalysisService analysisService, List directoryInputs,
			Map paramName2ActualParamMap, String username) {
	...
	public void exec() {
		new Thread() {
			public void run() {
				...
				try {
					_serviceProxy = new AnalysisWebServiceProxy(analysisService
							.getServer(), username);
				} catch (Exception e) {
				...
				try {
					...
					try {
						model = PipelineModel
								.toPipelineModel((String) (analysisService
										.getTaskInfo().getTaskInfoAttributes()
					...
					if (directoryInputs.size() > 0) {
						...
						_files = dir.listFiles(new java.io.FileFilter() {
					...
					final int directoryTaskIndex = _directoryTaskIndex == -1 ? tasks
							.size()
							: _directoryTaskIndex;// index of task that has a
												  // directory as input
					...
					for (int i = 0; i < directoryTaskIndex; i++) {
						String taskName = ((JobSubmission) tasks.get(i))
								.getName();
						AnalysisService svc2 = null; // FIXME
													 // AnalysisServiceManager.getInstance(analysisService.getName(),
													 // username).getAnalysisService(taskName);
						...
						try {
							if (TaskLauncher.isVisualizer(svc2)) {
								...
								results[i] = TaskLauncher
										.submitAndWaitUntilCompletion(
												actualParams, serviceProxy,
												svc2);
							}
						} catch (Exception e) {
							throw new RunTaskException(e);
					...
					for (int _fileNum = 0, numFiles = files != null ? files.length
							: 0; _fileNum < numFiles; _fileNum++) {// run all
																   // tasks
																   // repeatedly
																   // that come
																   // after the
								...
								for (int j = directoryTaskIndex; j < tasks
									...
									try {
										if (TaskLauncher.isVisualizer(svc2)) {
											...
											pipelineResults[j] = TaskLauncher
													.submitAndWaitUntilCompletion(
															actualParams,
															serviceProxy, svc2);
										}
	...
	ParameterInfo[] getPipelineParameters(List tasks, int j,
		...
		for (int k = 0, length = formalParams.size(); k < length; k++) {
			...
			ParameterInfo actualParam = (ParameterInfo) paramName2ActualParamMap
					.get(key);
			String runtimePromptValue = actualParam != null ? actualParam
					.getValue() : null;
