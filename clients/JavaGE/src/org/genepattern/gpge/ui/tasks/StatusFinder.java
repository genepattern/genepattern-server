 ...
 *  <p>
 *
 *  Title: StatusFinder.java </p> <p>
 *
 *  Description: Checks the status of unfinished job.</p>
 *
 *@author     Hui Gong
 *@created    May 4, 2004
 *@version    $Revision$
...
public class StatusFinder implements Runnable, Observer {
	...
	private static Category cat = Category.getInstance(StatusFinder.class.getName());
	//fields
	 ...
	 *
	 *@param  model     Description of the Parameter
	 *@param  delay_ms  Description of the Parameter
	 */
	public StatusFinder(final DataModel model, final int delay_ms) {
		this._data = model;
		if(delay_ms <= 0) {
			throw new IllegalArgumentException("Polling delay must be a positive time in milliseconds not " + delay_ms);
		}
	...
	public void run() {
		...
		while(true) {
				...
				try {
					...
					while(iter.hasNext()) {
						...
						if(!status.equals(JobStatus.FINISHED) && !status.equals(JobStatus.ERROR)) {
							try {
								...
								System.out.println("ERROR ROOT: " + wse.getMessage());
							 ...
							 */
									Exception e) {
								System.out.println("Plain exception: " + e.getClass());
								e.printStackTrace();
					...
					System.err.println("StatusFinder: ConcurrentModificationException"
