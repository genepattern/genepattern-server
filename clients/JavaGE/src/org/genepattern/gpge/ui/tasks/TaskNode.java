 ...
 * </p>
 * <p>
 * Description: A ResultTreeNode representing a specific task.
 * </p>
 * 
 * @author Hui Gong
 * @version 1.0
	...
	public JPopupMenu getPopup() {
		...
		open.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				...
				ResultTreeNode parent = (ResultTreeNode) TaskNode.this
						.getParent();
				String siteName = (String) parent.getUserObject();
				if (TaskNode.this.getChildCount() != 0) {
					int type = JOptionPane.showConfirmDialog(null,
							"Do you want to delete all results under " + task
									+ "?");
					if (type == JOptionPane.OK_OPTION) {
						TaskNode.this._dataModel.removeTaskFromResult(siteName,
								task);
						parent.remove(TaskNode.this);
						((DefaultTreeModel) TaskNode.this._treeModel)
								.nodeStructureChanged(TaskNode.this);
					}
				} else {
					TaskNode.this._dataModel.removeTaskFromResult(siteName,
							task);
