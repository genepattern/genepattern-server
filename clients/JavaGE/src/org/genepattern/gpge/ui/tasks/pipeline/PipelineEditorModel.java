...
public class PipelineEditorModel {
	...
	try {
	    String[] fileNames = new TaskIntegratorProxy(AnalysisServiceManager.getInstance().getServer(),
		    AnalysisServiceManager.getInstance().getUsername(), AnalysisServiceManager.getInstance()
			    .getPassword()).getDocFileNames(lsid);
	...
	String loggedInUser = asm.getUsername();
	...
	for (int i = 0; i < jobSubmissions.size(); i++) {
	    ...
	    ParameterInfo[] formalParams = formalAnalysisService != null ? formalAnalysisService.getTaskInfo()
		    .getParameterInfoArray() : null;
	    if (formalParams != null) {
		for (int j = 0; j < formalParams.length; j++) {
		    ParameterInfo formalParam = formalParams[j];

		    Integer index = (Integer) paramName2ParamIndex.get(formalParam.getName());
		    if (index == null) {
			throw new IllegalArgumentException((i + 1) + ". " + js.getName() + " is missing parameter "
				+ formalParam.getName());
		    }
		    int indexInJobSubmission = index.intValue();
		    myTask.addParameter(new MyParameter(formalParam, js, indexInJobSubmission));
		}
	    } else {
		for (int j = 0; j < jsParams.size(); j++) {
		    ParameterInfo p = (ParameterInfo) jsParams.get(j);
	...
	for (int i = from + 1; i < getTaskCount(); i++) {
		...
		} else if (p.inheritedTaskIndex > from && p.inheritedTaskIndex <= to) { // tasks > from
		    // and <= to
		    // have task
		    // number
		    // decreased by
	...
	for (int i = from + 1; i < getTaskCount(); i++) {
		...
		if (p.inheritedTaskIndex == from) { // task inherits
		    p.inheritedTaskIndex = to;
		} else if (p.inheritedTaskIndex >= to && p.inheritedTaskIndex < from) { // tasks >= to
		    p.inheritedTaskIndex++;
		    // and < from
		    // have task
		    // number
	...
	for (int i = 0; i < getTaskCount(); i++) {
	    ...
	    JobSubmission js = new JobSubmission(getTaskName(i), getTaskDescription(i), (String) taskInfo
		    .getTaskInfoAttributes().get(GPConstants.LSID), null, null, TaskLauncher.isVisualizer(taskInfo),
		    null);
		...
		if (inheritedTaskIndex != -1) {
		    ...
		    attrs.put(PipelineModel.INHERIT_TASKNAME, String.valueOf(inheritedTaskIndex)); // gpUseResult
		    // indices start at
		    // 1
		    ...
		    ParameterInfo pipelineParam = new ParameterInfo(getTaskName(i) + (i + 1) + "."
		    ...
		    if (file.exists()) {
			value = "<GenePatternURL>getFile.jsp?task=<LSID>&file=" + file.getName();
	...
	pipelineTaskInfo.setParameterInfoArray((ParameterInfo[]) pipelineParameterInfoList
	...
	StringBuffer baseCmdLine = new StringBuffer(
		"<java> -cp <pipeline.cp> -Dgenepattern.properties=<resources> -DLSID=<LSID> <pipeline.main> <GenePatternURL>getPipelineModel.jsp?name=<LSID>&userid=<userid> <userid>");
    ...
    /**
     * Gets the UI values of the choices for the specified parameter in the given task
     * 
     * @param taskIndex
    ...
    public void setInheritedFile(int taskIndex, int parameterIndex, int inheritedTaskIndex, String inheritedName) {
	MyTask task = (MyTask) tasks.get(taskIndex);
	MyParameter p = task.getParameter(parameterIndex);
	...
	public void setValue(String s) {
	    value = s;
	    /*
	     * if (value .startsWith("<GenePatternURL>getFile.jsp?task=<LSID>&file=")) { value = value.substring( "<GenePatternURL>getFile.jsp?task=<LSID>&file="
	     * .length(), value.length()); }
	     */
	    inheritedOutputFileName = null;
	    inheritedTaskIndex = -1;
	...
	public void setInheritOutput(int taskIndex, String _inheritedOutputFileName) {
	...
	public static MyParameter createMissingFormalParam(ParameterInfo jobSubmissionParam) {
	    ...
	    java.util.Map pipelineAttributes = jobSubmissionParam.getAttributes();

	    String taskNumberString = null;
	    if (pipelineAttributes != null) {
		taskNumberString = (String) pipelineAttributes.get(PipelineModel.INHERIT_TASKNAME);
		if (taskNumberString != null) {
		    int inheritedTaskIndex = Integer.parseInt(taskNumberString.trim());
	...
	public MyParameter(ParameterInfo formalParam) {
	    ...
	    value = (String) formalParam.getAttributes().get(GPConstants.PARAM_INFO_DEFAULT_VALUE[0]);
	    ...
	    String optional = (String) formalParam.getAttributes().get(GPConstants.PARAM_INFO_OPTIONAL[0]);
	    isRequired = !"on".equalsIgnoreCase(optional);

	}

	public MyParameter(ParameterInfo formalParam, JobSubmission js, int indexInJobSubmission) {
	    this.isInputFile = formalParam.isInputFile();
	    String optional = (String) formalParam.getAttributes().get(GPConstants.PARAM_INFO_OPTIONAL[0]);
	    isRequired = !"on".equalsIgnoreCase(optional);
	    ParameterInfo jobSubmissionParam = (ParameterInfo) js.getParameters().get(indexInJobSubmission);
	    ...
	    if ((indexInJobSubmission < runtimePrompt.length) && (runtimePrompt[indexInJobSubmission])) {
		setPromptWhenRun();
	    }
	...
	public List getOutputFileTypes() {
	    String fileFormat = (String) formalTaskInfo.getTaskInfoAttributes().get("fileFormat");
	    if (fileFormat == null) {
