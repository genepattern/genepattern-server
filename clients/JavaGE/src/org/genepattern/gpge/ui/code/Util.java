...
public class Util {
     ...
     *            the language
     */
    public static void viewCode(org.genepattern.codegenerator.TaskCodeGenerator codeGenerator, final AnalysisJob job,
        ...
        for (int i = 0; i < params.length; i++) {
            if (!params[i].isOutputFile()) {
                ...
                if (mode != null && mode.equals(ParameterInfo.CACHED_INPUT_MODE)) {
                    String name = JobModel.getJobResultFileName(params[i].getValue());
                    int jobNumber = job.getJobInfo().getJobNumber();
                    try {
                        String url = job.getServer() + "/gp/jobResults/" + jobNumber + "/"
        ...
        if (svc != null) { // if task exists on server
            ...
            try {
                ...
                if (serializedModel != null && serializedModel.length() > 0) {
                    ...
                    org.genepattern.data.pipeline.PipelineModel model = org.genepattern.data.pipeline.PipelineModel
                            .toPipelineModel((String) taskInfo.getTaskInfoAttributes().get("serializedModel"));
                    List taskInfos = new ArrayList();
                    List jobSubmissions = model.getTasks();
                    for (int i = 0; i < jobSubmissions.size(); i++) {
                        org.genepattern.data.pipeline.JobSubmission js = (org.genepattern.data.pipeline.JobSubmission) jobSubmissions
                                .get(i);
                        ...
                        AnalysisService pipelineStep = AnalysisServiceManager.getInstance().getAnalysisService(
                                js.getLSID());
                    ...
                    code = org.genepattern.codegenerator.AbstractPipelineCodeGenerator.getCode(model, taskInfos,
                            AnalysisServiceManager.getInstance().getServer(), language);
