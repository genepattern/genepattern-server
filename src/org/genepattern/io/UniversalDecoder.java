 ...
 * Dataset dataset = (Dataset) UniversalDecoder
 * 		.parse(in, &quot;Big Dataset&quot;, file_name);
 * in.close();
 * </pre>
 * 
 * Note that using the parse method with the file extension hint (see above code
 * example) is usually much faster than using parse(InputStream, String)
 * 
 * @author kohm
 */
public class UniversalDecoder {
	 ...
	 *            the input stream
	 * @param name
	 *            the name given to the returned data object note that the value
	 *            of name is just a suggestion; the name of the object may be
	 *            defined in the data format of the input stream
	 * @throws IOException
	 ...
	 */
	public static final DataObjector parse(final InputStream in,
			final String name) throws IOException, ParseException {
		...
		final String file_ext = AbstractDataParser
				.getLowercaseFileExtension(name);
		final File tmp_file = StorageUtils.createTempFile("UniDecode."
				+ file_ext);
		// dump the InputStream to the temp file
		StorageUtils.writeToFile(tmp_file, in);

		if (DEBUG) {
			System.out.println("Temp File (" + tmp_file + ") for \"" + name
					+ "\": is " + tmp_file.length() + " bytes long");
		}
		for (int i = 0; i < limit; i++) {
			...
			// debug
			//final String cname =
			// edu.mit.genome.debug.FieldDumper.getClassNameNoPack(parser);
		...
		throw new ParseException(
				"Unsupported input stream format! Cannot create the DataObject.",
				0);
	}

	/**
	 * decodes the input stream and returns a data object of some type this is
	 * faster than parse(InputStream in, String name) because the file extension
	 * determines which parser to try.
	 * 
	 * @return DataObjector the data object
	 * @param ext
	 *            the file extension
	...
	/**
	 * returns the header information by parsing the input stream If the file
	 * extension is known and provided the routine will try to inteligently
	 * parse the stream using the parser that is associated with the file
	 * extension. The "file extension", with_ext, could be the whole file name
	 * or null if the file name is not known or doesn't have an extension.
	 * 
	 * @param in
	 *            the input stream
	 * @param with_ext
	 *            the file name with extension or just the extension could be
	 *            null if not known
	 * @throws IOException
	 *             if there was a problem reading from the input stream
	 * @throws ParseException
	 *             if none of the parsers could interpret the initial data from
	 *             the input stream
	 * @return String the header lines
	 */
	public static final String getFullHeader(final InputStream in,
			final String with_ext) throws IOException, ParseException {
		final DataParser[] parsers = getParsersWithExt(with_ext);

		//start reading the data into growable buffer that is also an input
		// stream
		final BufferedInputStream b_in = new BufferedInputStream(in);
		b_in.mark(Integer.MAX_VALUE); // this shouldn't blowup - just reading
									  // the headers
		final int limit = parsers.length;
		for (int i = 0; i < limit; i++) {
			final DataParser parser = parsers[i];
		...
		throw new ParseException(
				"Unsupported input stream format! Cannot read the header.", 0);
	}

	/**
	 * returns the parser that can decode the input stream. If the file
	 * extension is known and provided the routine will try to inteligently
	 * parse the stream using the parser that is associated with the file
	 * extension. The "file extension", with_ext, could be the whole file name
	 * or null if the file name is not known or doesn't have an extension.
	 ...
	 * @throws IOException
	 *             if there was a problem reading from the input stream
	 * @return DataParser the parser that can decode the inputstream or null if
	 *         none can
	 */
	public static final DataParser getParser(final InputStream in,
			final String with_ext) throws IOException {
		final DataParser[] parsers = getParsersWithExt(with_ext);
	...
	/**
	 * returns the Summary created from the header information by parsing the
	 * input stream If the file extension is known and provided the routine will
	 * try to inteligently parse the stream using the parser that is associated
	 * with the file extension. The "file extension", with_ext, could be the
	 * whole file name or null if the file name is not known or doesn't have an
	 * extension.
	 * 
	 * @param in
	 *            the input stream
	 ...
	 */
	public static final SummaryInfo createSummary(final InputStream in,
			final String with_ext) throws IOException, ParseException {
		final DataParser[] parsers = getParsersWithExt(with_ext);
		if (DEBUG) {
			System.out.println("Found " + parsers.length
					+ " parsers with ext '" + with_ext + "'");
		}
		//start reading the data into growable buffer that is also an input
		// stream
		final BufferedInputStream b_in = new BufferedInputStream(in);
	...
	/**
	 * gets the parser associated with the file name, or extension if the
	 * extension is null, empty string, whitespace, or doesn't end in a
	 * recognizable extension then all the parsers are returned otherwise an
	 * array containing just the parser that is associated with the extension is
	 * returned.
	 * 
	 * @param file_name
	 *            the file name with file extension
	 * @return DataParser[] the data parsers that can handle the file with file
	 *         extension
	 */
	public static final DataParser[] getParsersWithExt(final String file_name) {
		...
		for (int i = 0; i < limit; i++) {
			if (ext.endsWith(FILE_EXTS[i])) {// could be a file name with an
											 // extension
				final DataParser parser = (DataParser) FILE_EXT_TO_PARSER
						.get(FILE_EXTS[i]);
				if (parser != null) {
	...
	/**
	 * gets the data parser that is associated with the file extension or null
	 * if none
	 * 
	 * @param file_name
	 *            the file name with file extension
	...
	public static final DataParser getParserWithExt(final String file_name) {
		...
		if (index != -1) {
			...
			if (java.util.Arrays.asList(htmlParser.getFileExtensions())
					.contains(extension)) {
				return htmlParser;
			}

			if (java.util.Arrays.asList(imageParser.getFileExtensions())
					.contains(extension)) {
				return imageParser;
			}

			if (java.util.Arrays.asList(textParser.getFileExtensions())
					.contains(extension)) {
				return textParser;
			}
	...
	//        if( is_dir && num > 1 )
	//            throw new IllegalArgumentException("Cannot both specify a directory and
	// files, either one dir or one or more files!");
	//        
	//        if( is_dir )
	...
	//                //System.out.println("RESULT ****** File ("+file_name+")
	// header=\n"+header);
	...
	//            System.out.println("START **** Processing File "+file_name+" *****");
	//            try {
	//                final java.io.FileInputStream in = new
	// java.io.FileInputStream(file_name);
	...
	//fields
	/**
	 * all the currently know parsers FIXME pick class that has this list of
	 * Parsers
	 */
	...
	/**
	 * associates the file extensions with the parser instance note that the
	 * file extensions are lower case and with out the file
	...
	/** static initializer */
	static {
		PARSERS = new DataParser[] { new FeaturesetPropertiesParser(),
				new GctParser(), new ResParser(), new ClsDataParser(),
				imageParser, htmlParser, textParser, excelParser };
		PARSER_LIST = java.util.Collections.unmodifiableList(java.util.Arrays
				.asList(PARSERS));
