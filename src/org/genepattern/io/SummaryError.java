...
/**
 * Reports that an error occured while getting summary information. FIXME
 * shouldn't this extend DefaultSummaryInfo?
 * 
 * @author kohm
 */
public class SummaryError implements SummaryInfo {
	...
	public SummaryError(final java.io.File file, final Exception ex,
			final DataParser parser) {
		this(file, ex, parser, null, null);
	}

	/** Creates a new instance of SummaryError */
	public SummaryError(final String file, final Exception ex,
			final DataParser parser) {
		this(file, ex, parser, null, null);
	}
	...
	/** Creates a new instance of SummaryError */
	public SummaryError(final java.io.File file, final Exception ex,
			final DataParser parser, final Map primary, final Map secondary) {
		this((Object) file, ex, parser, primary, secondary);
	}

	/** Creates a new instance of SummaryError */
	public SummaryError(final Object file, final Exception ex,
			final DataParser parser, final Map primary, final Map secondary) {
		final Map map = (primary == null || primary.size() == 0) ? new HashMap(
				4) : new HashMap(primary);
		if (file != null)
			map.put(FILE, file);
		if (ex == null)
			throw new NullPointerException(
					"The Exception cannot be null for reporting the problem!");
		if (parser == null)
		...
		this.secondary = (secondary == null || secondary.size() == 0) ? java.util.Collections.EMPTY_MAP
				: new HashMap(secondary);
	}
