...
public class IOUtil {
    ...
    * @exception  IOException     If an error occurs while reading from the
    *      file
    * @exception  ParseException  If there is a problem with the data
    */
   public static ClassVector readCls(String pathname) throws IOException, ParseException {
    ...
    * @exception  IOException  If an error occurs while reading from the file
    */
    ...
    * @param  expressionDataCreator  The expression data creator
    * @return                        An object containing the expression data
    * @exception  IOException        If an error occurs while reading from the
    *      file
    * @exception  ParseException     If there is a problem with the data
    */
   public static Object readExpressionData(String pathname, IExpressionDataCreator expressionDataCreator) throws IOException, ParseException {
    ...
    * @exception  IOException     If an error occurs while saving the data
    */
    ...
    * @exception  IOException     If an error occurs while saving the data
    */
   public static String write(IExpressionData data, String formatName, String pathname, boolean checkFileExtension) throws IOException {
      IExpressionDataWriter writer = (IExpressionDataWriter) formatNameToExpressionWriterMap.get(formatName);
      if(writer == null) {
   ...
   public static void readOdf(String pathname, IOdfHandler handler) throws ParseException, IOException {
   ...
   private static IExpressionDataReader tryAllReaders(Iterator it, String pathname) throws IOException {
      while(it.hasNext()) {
   ...
   private static Iterator lookupProviders(Class c, Iterator defaultIt) {
      try {
         ...
         java.lang.reflect.Method m = klass.getMethod("lookupProviders", new Class[]{Class.class});
   ...
   /*
       public static boolean isFeatureList(String pathname) {
       String suffix = pathname.substring(pathname.lastIndexOf(".") + 1, pathname.length()).toLowerCase();
       featureListReader.getFileSuffixes().contains(suffix);
       return pathname.endsWith(".grp");
       }
     */
   /**
   ...
   public static IExpressionDataReader getExpressionReader(String pathname) {
      ...
      if(dotIndex != -1) {// see if file has an extension
         ...
         reader = (IExpressionDataReader) suffix2ExpressionReaders.get(suffix);
      }
      if(reader == null) {
         try {
            reader = tryAllReaders(suffix2ExpressionReaders.values().iterator(), pathname);
         } catch(IOException ioe) {
            return null;
         }
      }
    ...
    *
    * @param  formatName  a String containing the informal name of a format
    *      (e.g., "res" or "gct".
    * @return             a IExpressionDataWriter that can encode in the
    *      specified format or <codee>null</code> if no such
    *      IExpressionDataWriter exists.
    */
   private static IExpressionDataWriter getWriterByFormatName(String formatName) {
      return (IExpressionDataWriter) formatNameToExpressionWriterMap.get(formatName);
   }

   static {
      List defaultWriters = Arrays.asList(new IExpressionDataWriter[]{new ResWriter(), new GctWriter(), new OdfDatasetWriter()});
      try {
         defaultWriters.add(new org.genepattern.io.expr.mage.MAGEMLWriter());// don't require magestk to be in classpath
      } catch(Throwable t) {
      }
      Iterator it = defaultWriters.iterator();//lookupProviders(edu.mit.broad.io.expr.IExpressionDataWriter.class, defaultWriters);
   ...
   static {
      Iterator defaultReaders = Arrays.asList(new IExpressionDataReader[]{new GctReader(), new ResReader(), new OdfDatasetReader()}).iterator();// defaults if using java 1.3
      Iterator it = defaultReaders;//lookupProviders(edu.mit.broad.io.expr.IExpressionDataReader.class, defaultReaders);
      ...
      while(it.hasNext()) {
         ...
         for(int j = 0, cnt = suffixes.size(); j < cnt; j++) {
            ...
            // XXX if > 1 reader can read a file with the same suffix, one of the readers will be 'lost'
