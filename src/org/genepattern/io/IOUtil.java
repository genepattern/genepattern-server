...
import org.genepattern.io.expr.snp.CnParser;
import org.genepattern.io.expr.snp.CnWriter;
...
public class IOUtil {
     ...
     */
    public static void writeFeatureList(String[] features, String pathname)
            throws IOException {
        PrintWriter pw = null;
        try {
     ...
     *                If there is a problem with the data
     */
    public static ExpressionData readExpressionData(String pathname)
            throws IOException, ParseException {
        return (ExpressionData) readExpressionData(pathname,
                new ExpressionDataCreator());
    }

    /**
     * Reads the expression data at the given input stream and returns a new
     * <tt>ExpressionData</tt> instance.
     * 
     * @param filename
     *            The filename. The filename extension is used to deduce the
     *            type of document at the given input stream
     * @param is
     *            The input stream. The stream should be closed by the caller.
     * @return An <tt>ExpressionData</tt> instance
     * @exception IOException
     *                If an error occurs while reading from the file
     * @exception ParseException
     ...
     *            A pathname string
     * @param whether
     *            to try all readers if the type of the document can not be
     ...
     */
    public static IExpressionDataWriter getExpressionWriterByFormatName(
            String formatName) {
    ...
    static {
        List defaultWriters = Arrays.asList(new IExpressionDataWriter[] {
                ...
                new SnpWriter(), new CnWriter("cn"), new CnWriter("loh") });
        try {
    ...
    static {
        Iterator defaultParsers = Arrays.asList(
                new IExpressionDataParser[] { new GctParser(), new ResParser(),
                        new OdfParserAdapter(), new SnpParser(), new CnParser(true), new CnParser(false) }).iterator();// defaults
        // if
        // using java 1.3
