...
public class FeaturesetPropertiesEncoder implements Encoder {
	...
	public void write(final DataObjector data, final Writer writer)
		...
		//final int h_cnt = ((remarks == null || remarks.length == 0) ?
		// attrs.size() : attrs.size() - 1) + getStdHeaderCount();
		final int h_cnt = attrs.size() + getStdHeaderCount(properties);
		try {
			final PrintWriter out = (writer instanceof PrintWriter) ? (PrintWriter) writer
			...
			final NameDescriptionIndices nd_inds = writeHeaders(out, attrs,
					remarks, properties);
			...
			throw new IOException(
					"Error: while writing featureset properties  "
							+ properties.getName() + ":\n" + ex.getMessage());
	 ...
	 *             if a problem arises durring an I/O operation
	 */
	protected void writeHeaderCount(final PrintWriter out, final int cnt)
			throws IOException {
	 ...
	 */
	protected NameDescriptionIndices writeHeaders(final PrintWriter out,
			final Map attrs, final Remark[] remarks,
			final FeaturesetProperties properties) throws IOException {
		...
		final NameDescriptionIndices nd_ind = //        which columns to put the names,
											  // desc
	 ...
	 *             if a problem arises durring an I/O operation
	 */
	protected void writeComment(final PrintWriter out, final String note,
			final FeaturesetProperties properties) throws IOException {
	 ...
	 *            the writer to write data to
	 * @param properties
	 *            he featurset properties object
	 * @throws IOException
	 *             if a problem arises durring an I/O operation
	 */
	protected void writeDataCount(final PrintWriter out,
			final FeaturesetProperties properties) throws IOException {
	 ...
	 *             if a problem arises durring an I/O operation
	 */
	protected void writeModel(final PrintWriter out,
			final FeaturesetProperties properties) throws IOException {
	 ...
	 *             if a problem arises durring an I/O operation
	 */
	protected void writeColumnTypes(final PrintWriter out,
			final FeaturesetProperties properties) throws IOException {
	 ...
	 *             if a problem arises durring an I/O operation
	 */
	protected void writeColumnHeaders(final PrintWriter out,
			final FeaturesetProperties properties) throws IOException {
	 ...
	 *             if a problem arises durring an I/O operation
	 */
	protected void writeColumnsDescriptions(final PrintWriter out,
			final FeaturesetProperties properties) throws IOException {
	 ...
	 * @return NameDescriptionIndices
	 */
	protected NameDescriptionIndices writeRowNameDescriptions(
			final PrintWriter out, final FeaturesetProperties properties)
			throws IOException {
		int row = -1;
	 ...
	 *             if a problem arises durring an I/O operation
	 */
	protected void writeAttributes(final PrintWriter out, final Map attrs,
			final FeaturesetProperties properties) throws IOException {
	 ...
	 *             if a problem arises durring an I/O operation
	 */
	protected void writeMainData(final PrintWriter out,
			final FeaturesetProperties properties,
			final NameDescriptionIndices nd_inds) throws IOException {
		//        final boolean has_row_names = (nd_inds != null && nd_inds.name_index
		// >= 0);
		//        final boolean has_row_descs = (nd_inds != null && nd_inds.descr_index
		// >= 0);
	 ...
	 *             if a problem arises durring an I/O operation
	 */
	private void writeLine(final String kw, final String val,
	 ...
	 * 
	 * @param properties
	 *            properties the featurset properties object
	...
	protected int getStdHeaderCount(final FeaturesetProperties properties) {
		/*
		 * KW_REMARK // not counted KW_COLUMN_DESCRIPTIONS // may not have
		 * KW_COLUMN_NAMES // yes KW_COLUMN_TYPES // maybe KW_DELIMITER // no
		 * just use the default KW_DATA_LINES // yes KW_ROW_DESCRIPTION_COLUMN //
		 * maybe KW_ROW_NAMES_COLUMN // maybe KW_MODEL // yes
		 */
		int total = 3;
		if (properties.hasColumnDescriptions())
			total++;
	 ...
	 *            the key word
	 * @param array
	 *            the array of primitives or objects
	 * @param out
	 *            the writer to record data to
	 * @throws IOException
	 *             if a problem arises durring an I/O operation
	 */
	protected final void writeArray(final String kw, final Object array,
			final PrintWriter out) throws IOException {
		out.print(kw);
		if (array instanceof Object[]) {
			...
			throw new UnsupportedOperationException(
					"Need to implement saving of arrays of type "
							+ array.getClass() + "!");
		out.println();
	}
	 ...
	 *             if a problem arises durring an I/O operation
	 */
	private void writeObjArray(final Object[] objs, final PrintWriter out)
			throws IOException {
	 ...
	 *             if a problem arises durring an I/O operation
	 */
	private void writeIntArray(final int[] vals, final PrintWriter out)
			throws IOException {
	 ...
	 *             if a problem arises durring an I/O operation
	 */
	private void writeFloatArray(final float[] vals, final PrintWriter out)
			throws IOException {
		final int limit = vals.length;
		for (int i = 0; i < limit; ++i) {
			out.print(delimiter);
	 ...
	 *             if a problem arises durring an I/O operation
	 */
	private void writeDoubleArray(final double[] vals, final PrintWriter out)
			throws IOException {
		final int limit = vals.length;
		for (int i = 0; i < limit; ++i) {
			out.print(delimiter);
	 ...
	 *             if a problem arises durring an I/O operation
	 */
	private void writeBooleanArray(final boolean[] vals, final PrintWriter out)
			throws IOException {
		final int limit = vals.length;
		for (int i = 0; i < limit; ++i) {
			out.print(delimiter);
	...
	/** the version tag */
	public static final String CURRENT_SDF_VERSION = SDF_VERSION_MAJOR + "."
			+ SDF_VERSION_MINOR;
	...
	//Key Words
	//FIXME this should be put in a super class or interface of this and
	// AbstractPropertiesDecoder
	/**
	 * Lines that begin with this character are considered comments or remark
	 * lines
	 */
	public static final String KW_REMARK = "#";

	/**
	 * Defines the number of header line (not counting remarks) REQUIRED 2nd
	 * line
	 */
	...
	static {
		...
		for (int i = 0; i < limit; i++) {
			CLASS_TO_TYPE.put(FeaturesetProperties.SUPPORTED_CLASSES[i],
					FeaturesetProperties.SUPPORTED_TYPES[i]);
		}
