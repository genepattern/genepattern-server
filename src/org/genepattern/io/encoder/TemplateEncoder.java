 ...
 * @author kohm
 */
public class TemplateEncoder extends FeaturesetPropertiesEncoder implements
		Encoder {
	...
	/**
	 * returns true if the data obejct is a <CODE>Template</CODE>
	 * 
	 * @param data
	 *            the data object
	 * @return true if the data obejct is a <CODE>Template</CODE>
	 */
	public boolean canEncode(DataObjector data) {
		return (data instanceof Template);
	}

	//    /** gets the file extension for the specified object or null if wrong
	// type */
	...
	/**
	 * Writes the <CODE>Template</CODE> to the output stream
	 * 
	 * @param data
	 *            the tempalte object
	 * @param out
	...
	public void write(final DataObjector data, final OutputStream out)
		...
		super.write(new TemplatePropertiesWrapper(template), out);
	}
	 ...
	 * gets the instance
	 * 
	 * @return Encoder, the singleton of this
	 */
	...
	//                final String name =
	// org.genepattern.io.parsers.AbstractDataParser.getFileNameNoExt(file_name);
	//                
	//                DataObjector data_object = org.genepattern.io.UniversalDecoder.parse(in,
	// file_name);
	//                in.close();
	//                System.out.println("RESULT ****** File ("+file_name+") ->"+data_object);
	//                if( data_object instanceof Template ) {
	//                    final java.io.File odf_file = new java.io.File(dir, name + ".odf");
	//                    try {
	//                        System.out.println("Writting "+odf_file);
	//                        final java.io.FileOutputStream out = new
	// java.io.FileOutputStream(odf_file);
	//                        INSTANCE.write(data_object, out);
	//                        System.out.println("worked!");
	//                        out.close();
	//                    } catch (IOException ex) {
	//                        System.err.println("Had a problem with I/O for file
	// \""+odf_file.getName()+"\"!");
	//                        ex.printStackTrace();
	//                    }
	//                }
	...
	// fields
	/** the instance */
	private static final TemplateEncoder INSTANCE = new TemplateEncoder();
}