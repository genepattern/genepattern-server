...
public class GctDatasetEncoder implements Encoder{
    ...
    // NOTE: see CSomAlg::SaveAll to optionally write class assignments and distances to file
    // To read this into splus (note row.names is still 1 even though the data begins on line 3.
    //    The reason is the skip=2.
    //
    //    gcm <- read.table("d:\\mprsets\\gcm.gct", header=T, row.names=1, skip=2, as.is=T, sep="\t")
...
//		fCurRow++;
            ...
            e = new IOException("Error: while writing dataset "+dataset.getName()+":\n"+e.getMessage());
            e.fillInStackTrace();
            throw e;
        }
     ...
     *
     */
    private String getDimensionLine(final StringBuffer buffer, final int aRows, final int aCols){
        buffer.setLength(0);
        buffer.append(aRows); 
    ...
    /** creates a data line */
    private String getDataLine(final int r, final StringBuffer buffer, final Dataset dataset) {
        final char delim = DELIMITER;
        buffer.setLength(0);
...
//        float[] theRow = dataset.getMatrix().getRow(r);
    ...
    /** the delimiter for tsv
     * FIXME this should be common for both the encoder and decoder */
