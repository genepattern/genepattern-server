 ...
 * @author  kohm
 */
public class DatasetPropertiesWrapper extends AbstractObject implements FeaturesetProperties {
    ...
    public String getColumnName(final int col) {
        switch( col ){
            ...
            case 1:
                return "Description";
            default:
                //System.out.println("("+getColumnCount()+") columns_panel has "+columns_panel.getCount()+" getting name "+(col-2));
    ...
    public void setValueAt(Object obj, int param, int param2) {
        throw new UnsupportedOperationException("Cannot modify the underlying Dataset!");
    }
    ...
    private void populateArrayWithNames(final NamesPanel panel, final String[] array) {
        final int limit = array.length;
        if( panel.getCount() != limit ) {
            final boolean too_small = ( limit < panel.getCount() );
            throw new ArrayIndexOutOfBoundsException("The size("+limit+") of the array is "
    ...
    private void populateArrayWithDescriptions(final NamesPanel panel, final String[] array) {
        final int limit = array.length;
        if( panel.getCount() != limit ) {
            final boolean too_small = ( limit < panel.getCount() );
            throw new ArrayIndexOutOfBoundsException("The size("+limit+") of the array is "
                +( (too_small) ? "smaller" : "larger" )
                +" than the number("+panel.getCount()+") of descriptions!");
        }
        for(int i = 0; i < limit; i++) {
    ...
    public class SimpleStringName extends AbstractSimpleObject{
        SimpleStringName(final int column) { super(column); }
    ...
    public class SimpleStringDescr extends AbstractSimpleObject{
        SimpleStringDescr(final int column) { super(column); }
        /** returns the String at the index
         * @parram row the row index
         */
        public final Object get(final int row) {
