...
public class OdfParser {
   ...
   LineReader reader;
   int dataLines;
   final static List VERSIONS = Arrays.asList(new String[]{"ODF 1.0", "SDF 1.0"});
    ...
    *      exception.
    * @throws  ParseException  - An IO exception from the parser, possibly from
    ...
    */
   public boolean canDecode(InputStream is) throws IOException {//delete this method??
      this.reader = new LineReader(is);
      reader.setComment("#");
   ...
   void parseData() throws ParseException, IOException {
      ...
      // read the 1st line to get the number of columns. Check to make sure all rows in data block have the same number of rows
      String line = reader.readLine().trim();// trim b/c old odf writer adds an extra tab at the end of the line
      ...
      for(; i < dataLines; i++) {
         line = reader.readLine().trim();// trim b/c old odf writer adds an extra tab at the end of the line
         if(line == null) {
            int dataLinesRead = i + 1;
            throw new ParseException("Unexpected end of file on line " + reader.getLineNumber() + ".");
         }
         tokens = line.split("\t");
         if(tokens.length != expectedColumns) {
            throw new ParseException("Expecting " + expectedColumns + " tokens on line " + reader.getLineNumber() + ", but found " + tokens.length + " tokens.");
         }
      ...
      for(String s = reader.readLine(); s != null; s = reader.readLine()) {
         if(!s.trim().equals("")) {
            throw new ParseException("Extra data rows on line " + reader.getLineNumber() + ".");
         }
      }
   }
   ...
   void parseHeader() throws ParseException, IOException {
      ...
      for(int i = 0; i < headerLines; i++) {
         ...
         if(equalsIndex > 0 && equalsIndex < semiIndex) {
            ...
            String value = line.substring(delimitterIndex + 1, line.length());
            key = key.trim();
            value = value.trim();
            ...
            key = key.trim();
            String values = line.substring(delimitterIndex + 1, line.length());
            values = values.trim();
    ...
    *  different handler in the middle of a parse, and the ODF parser must
