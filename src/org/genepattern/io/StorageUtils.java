...
public class StorageUtils {
     ...
     * normal JVM exit
     * @param suffix The name of the file
...
//                tmp_file = File.createTempFile(prefix + integer, suffix);
                //tmp_file = File.createTempFile(integer+TEMP_FILE_MARKER+"00", suffix);
                tmp_file = File.createTempFile(integer+TEMP_FILE_MARKER, suffix);
    ...
    /**
     * Creates a new file with the given name in a temp folder. If this file already exists
     * it's name is appended _1 or _2 etc.  This file is deleted upon
     * normal JVM exit
     */
    public static final File createTempFileNoMung(final String name) throws IOException{
    ...
    public static final String createStringFromContents(final File file) throws IOException{
    ...
    public static final String createStringFromReader(final Reader rdr) throws IOException{
     ...
     * @return String the file extension including the file extension separator character
     */
    public static final String getFileExtension(final String name) {
        final int index = name.lastIndexOf(AbstractDataParser.FILE_EXT_SEPARATOR);
        if( index < 0 )
            return "";
    ...
    public static final String getFileNameNoExt(final String base) {
        final int index = base.lastIndexOf(AbstractDataParser.FILE_EXT_SEPARATOR);
        if( index < 0 )
            return base;
     ...
     * @throws IOException if a problem arises durring an I/O operation
     */
    public static final void writeToFile(final File file, final InputStream in) throws IOException {
    ...
    /** writes the InputStream to the file
     * @param file he file to write to
     * @param reader the reader to read from
     * @throws IOException if a problem arises durring an I/O operation
     */
 /*   public static final void writeToFile(final File file, final Reader reader) throws IOException{
     ...
     * and closes the reader and writer when done
     * Note that this method uses Read and Writer which are Character centric
     * IO classes.  Use copyInputToOutputStream() for binary transactions
     * @param reader he reader to read from
     * @param writer the write to write to
     * @throws IOException if a problem arises durring an I/O operation
     */
    public static final void copyReaderToWriter(final Reader reader, final Writer writer) throws IOException{
        final int size = 8096;
        final char[] buff = new char[size];
        int i = 0, count = 0;
        final BufferedReader input  = (reader instanceof BufferedReader ?
                (BufferedReader)reader : new BufferedReader(reader, size));
        final BufferedWriter output = (writer instanceof BufferedWriter ?
                (BufferedWriter)writer : new BufferedWriter(writer, size));
    ...
    /** writes the contents of an InputStream to the OutputStream using buffered data streams
     * and closes the input and output when done
     * @param input he input to read from
     * @param output the output stream to read from
     * @throws IOException if a problem arises durring an I/O operation
     */
    public static final void copyInputToOutputStream(final InputStream input, final OutputStream output) throws IOException{
        final int size = 8096;
        final byte[] buff = new byte[size];
        int i = 0, count = 0;
        final BufferedInputStream in  = (input instanceof BufferedInputStream ?
                (BufferedInputStream)input : new BufferedInputStream(input, size));
        final BufferedOutputStream out = (output instanceof BufferedOutputStream ?
                (BufferedOutputStream)output : new BufferedOutputStream(output, size));
        while((i = in.read(buff, 0, size)) >= 0 ) { // if i == -1 (EOF) then done
            out.write(buff, 0, i);
            //System.out.print(++count+". ");
        }
        //System.out.println();
