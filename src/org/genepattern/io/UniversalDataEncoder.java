...
/**
 * This class has many static methods for saving a data object (DataObjector) to
 * an output stream. An example code fragment for saving a Dataset to a file is:
 * 
 * <pre>
 * 
 ...
 * data object's <CODE>DataModel</CODE> was mapped to a different <CODE>
 * Encoder</CODE> with <CODE>mapModelToEncoder(DataModel, Encoder)</CODE>.
 * 
 ...
 * </pre>
 * 
 * The above code will save the transposed_dataset as a gct file instead of the
 * default which is an odf file.
 * 
 * The following keys are defined for various <CODE>Encoder</CODE>s: "odf
 * dataset" - save a Dataset as a odf file "gct dataset" - save a Dataset as a
 * gct file "odf template" - save a Template (Class Vector) as a odf file
 * 
 * Note that res and cls <CODE>Encoder</CODE> s are not yet implemented. Also
 * if a data object is represented as a FeaturesetProperties object or subset of
 * FeaturesetProperties then it is saved in odf format without the need for any
 * special <CODE>Encoder</CODE> besides the default one that handles <CODE>
 * FeaturesetProperties</CODE>.
 * 
 * 
 * FIXME problem is that DataModels are mapped to encoders. But it should be
 * that classes are mapped to encoders. Reason is that more than one class can
 * represent a DataModel. For example a Dataset can be represented in memory by
 * a DefaultDataset or a DefaultFeaturesetProperties objects. These two classes
...
public class UniversalDataEncoder implements Encoder {
	...
	private UniversalDataEncoder() {
		...
		mapTextToEncoder("odf_dataset", DatasetEncoder.instance()); // default
																	// for
																	// dataset
	 ...
	 * determines this by using querying all encoders with
	 * canEncode(DataObjector)
	 * 
	 * @param data
	 ...
	 * UnsupportedEncodingException if the data object is not supported or an
	 * IOException if there are I/O problems.
	 * 
	 * @param data
	 *            the data object to encode
	 * @param out
	 *            the out put stream to save to
	 * @throws IOException
	 *             if an error occures during an IO operation
	 * @throws UnsupportedEncodingException
	 *             if the data object is not supported
	 * @return <CODE>Encoder</CODE> the encoder used to save the data object
	 */
	public static final Encoder encode(final DataObjector data,
			final OutputStream out) throws IOException,
			UnsupportedEncodingException {
		final Encoder encoder = getEncoderFor(data);

		if (encoder == null) {
			throw new UnsupportedEncodingException("The data object, " + data
	...
	/**
	 * gets the file extension for the specified data object or null if there
	 * are no Encoders known.
	 * 
	 * @param data
	 *            the data object
	...
	public static final String getFileExtensionFor(final DataObjector data) {
		final Encoder encoder = getEncoderFor(data);
		if (encoder == null) {
			System.err.println("Couldn't get encoder for: "
					+ data.getDataModel());
			return null;
		}
	...
	public static final Encoder getEncoderFor(final DataObjector data) {
		...
		final Encoder encoder = (Encoder) MODEL_ENCODER
				.get(data.getDataModel());
	 ...
	 * @param data
	 *            the data object
	 * @return boolean true if the <CODE>DataObjector</CODE> has an encoder
	 *         that can save it
	 */
	 ...
	 *            the data object
	 * @param out
	 *            the output stream where the data object will be saved
	 * @throws IOException
	 ...
	 * 
	 * @param data_model
	 *            the data model to associate with the encoder
	 * @param encoder
	 *            the encoder that is associated with the data model
	 * @return true if the data model was not entered previously
	 */
	public static final boolean mapModelToEncoder(final DataModel data_model,
			final Encoder encoder) {
		//        if( MODEL_ENCODER.containsKey(data_model) )
		//            throw new DuplicateKeyException("Cannot have a DataModel mapped to
		// more than one Encoder!");
	 ...
	 * 
	 * @param key
	 *            the key that will be used to retrieve the encoder
	 * @param encoder
	 *            the encoder to retrieve
	 * @return true if it worked otherwise it replaced an old encoder that was
	 *         mapped to the same text
	 */
	public static final boolean mapTextToEncoder(final String key,
			final Encoder encoder) {
		//        if( TEXT_TO_ENCODER.containsKey(key) )
		//            throw new DuplicateKeyException("Cannot have a text key mapped to
		// more than one Encoder!");
	...
	private static final Encoder[] ENCODERS = new Encoder[] {
			FeaturesetPropertiesEncoder.instance(), DatasetEncoder.instance(),
			GctDatasetEncoder.instance(),
