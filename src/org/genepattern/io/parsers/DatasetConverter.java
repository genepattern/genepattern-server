...
/**
 * Converts a FeaturesetProperties object with DataModel == Dataset into a
 * Dataset object.
 * 
 * @author kohm
 */
public class DatasetConverter implements FeaturesetPropertiesConverter {
	...
	public DataObjector createDataObject(final FeaturesetProperties fs) {
		if (!Dataset.DATA_MODEL.equals(DataModel.findModel(fs.getModel()))) {
			throw new IllegalArgumentException(
					"The FeaturesetProperties represents a " + fs.getModel()
							+ " model. It should be a " + Dataset.DATA_MODEL
							+ " model!");
		}
		...
		for (int r = 0, i = 0; r < row_cnt; r++) {
			for (int c = 2; c < real_col_cnt; c++) {
				data_array[i++] = ((DefaultFeaturesetProperties.SimpleFloat) fs
						.getValueAt(r, c)).getFloat();
			}
		}
		...
		AnnotationFactory rfactory = PrimeAnnotationFactory
				.createAnnotationFactory(row_names, fs.getRowDescriptions(null));
		AnnotationFactory cfactory = PrimeAnnotationFactory
				.createAnnotationFactory(col_names, createArrayTwoLess(fs
						.getColumnDescriptions(null)));
		return new DefaultDataset(fs.getName(), new DefaultMatrix(row_cnt,
				col_cnt, data_array),
	...
	// java.io.FileInputStream("/Users/kohm/data/test_ds_encoder/Prostate_BiopsyTrial_ams.res");
	//        final Dataset res_dataset = (Dataset)new ResParser().parse(in, "From
	// res");
	//        in.close();
	//        
	//        System.out.println("\nreading the odf file...");
	//        final java.io.FileInputStream in2 = new
	// java.io.FileInputStream("/Users/kohm/data/test_ds_encoder/Prostate_BiopsyTrial_ams.odf");
	//        final Dataset odf_dataset = (Dataset)new
	// FeaturesetPropertiesParser().parse(in2, "From odf");
	//        in2.close();
	//        
	//        System.out.println("\n\n");
	...
	//        System.out.println("\ndoes the res object equal the odf object
	// "+res_dataset.equals(odf_dataset));
	//    }
