...
/**
 * Converts a FeaturesetProperties object with DataModel == Template into a
 * Template object.
 * 
 * @author kohm
 */
public class TemplateConverter implements FeaturesetPropertiesConverter {
	...
	public DataObjector createDataObject(final FeaturesetProperties fs) {
		if (!Template.DATA_MODEL.equals(DataModel.findModel(fs.getModel()))) {
			throw new IllegalArgumentException(
					"The FeaturesetProperties represents a " + fs.getModel()
							+ " model. It should be a " + Template.DATA_MODEL
							+ " model!");
		}
		...
		if (is_numeric) {
			...
			for (int r = 0; r < row_cnt; r++) {
				...
				items[r] = new Template.Item(SampleLabel.create(sample_name),
						fs.getValueAt(r, 1).toString());
			}

			//create Klass objects
			final Template.Klass[] klasses = createKlasses(klass_names);
			//            final Template.Klass[] klasses = new
			// Template.Klass[klass_names.length];
	 ...
	 * 
	 * @param attrs
	 *            the attributes of the featureset properties object
	...
	protected static final String[] getKlasseNames(final FeaturesetProperties fs) {
		...
		if (value != null) {
			if (value instanceof String[]) {
				...
				final java.util.StringTokenizer toke = new java.util.StringTokenizer(
						(String) value, "\t");
				...
				throw new IllegalArgumentException(
						"Unknown 'klasses' class value " + value);
		}
		return null;
	}

	private static final Template.Klass[] createKlasses(
			final String[] klass_names) {
	...
	// java.io.FileInputStream("/Users/kohm/data/test_template_encoder/Test_small.cls");
	//        final Template cls_template = (Template)new ClsDataParser().parse(in,
	// "cls_template");
	//        in.close();
	//        
	//        System.out.println("\nreading the odf file...");
	//        final java.io.FileInputStream in2 = new
	// java.io.FileInputStream("/Users/kohm/data/test_template_encoder/Test_small.odf");
	//        final Template odf_template = (Template)new
	// FeaturesetPropertiesParser().parse(in2, "odf_template");
	//        in2.close();
	//        
	//        System.out.println("\n\n");
	...
	//        System.out.println("\ndoes the cls object equal the odf object
	// "+cls_template.equals(odf_template));
	//    }
	//fields
