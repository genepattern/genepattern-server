...
public class ClsDataParser extends AbstractDataParser {
	...
	//    /** Creates a new instance of ClsDataParser */
	//    public ClsDataParser(final InputStream input, final String cls_name)
	...
	//    }
	//    /** Creates a new instance of ClsDataParser */
	//    public ClsDataParser() throws IOException,
	...
	public boolean canDecode(final InputStream in) throws IOException {
		...
		try {
			...
			//System.out.println(getClass()+" cannot decode stream
			// "+ex.getMessage());
	 ...
	 * @param name
	 *            the name of the resulting Template object
	 * @param in
	 *            the input stream
	 * @throws IOException
	 *             if a problem occurs due to an I/O operation
	...
	//    private final void saveClassifer (String first, String second, String
	// third, File file) throws IOException{
	 ...
	 *            input stream
	 * @throws IOException
	 *             if error durring I/O operation
	 * @throws org.genepattern.io.ParseException
	 *             if format problem with data
	 * @return Summaryinfo
	 */
	...
	// fields
	/**
	 * scratch inner parser that should be useable for canDecode() and
	 * getSummaryInfo()
	 */
	...
	protected static class InnerParser {
		...
		//            return template;
		//        }
		Template parse(final String cls_name) throws IOException,
				java.text.ParseException, RuntimeException {
			String header_line, classifier_line, data_line;
			int[] classIndices = null;
			String[] names = null;
			int[] values = null;
			try {
				// header line: <num_data> <tab> <num_classes> <tab>
				// <numAssignments>*
				header_line = reader.readLine();

				final Integer[] header_values = processHeader(header_line);
				final int num_items = header_values[0].intValue(), num_classes = header_values[1]
						.intValue();
				...
				} else { // assume classifier line was skipped (second line) so
						 // try it as data
					data_line = classifier_line;
					try {
						classIndices = processData(data_line, num_classes,
								values, num_items);
						names = createNames(classIndices);
						// the rest is done after the three main catch
						// statements below
					} catch (NumberFormatException ex) {
						// It was discovered that the data line didn't have
						// numbers
						// so it's assumed that each datum is a class label
						names = processDataAsArbitraryStrings(data_line,
								num_classes, values, num_items);
					}
				}
			...
			//            try {
			//                getClassifiersFromUser (classIndices);// and get the classifiers
			// from the user
			//                classifier_line = createClassifiersLine ();
			//                File new_file = createNewName (fFile);
			//                saveClassifer (header_line, classifier_line, data_line,
			// new_file);
			//            } catch (IOException e) { // don't rethrow exception because main
			// operation completed
		...
		private final Integer[] processHeader(final String header_line)
				throws IOException, java.text.ParseException {
			return processHeader(header_line, null);
		}

		private final Integer[] processHeader(final String header_line,
				final Map primary) throws IOException, java.text.ParseException {
			if (header_line == null)
			...
			try {
				...
				throw new java.text.ParseException(
						"CLS Format error Header line element '"
								+ e.getMessage() + "' is not a number!", 0);
			} finally {
				if (primary != null) {
			...
			if (value > 0) {
				...
				throw new java.text.ParseException(
						"CLS Format error\nHeader line missing first number, num data",
						0);
			...
			if (value > 0) {
				...
				throw new java.text.ParseException(
						"CLS Format error\nHeader line missing second number, num classes",
						0);
			...
			if (value > 0) {
				...
				if (num_assignments > 1)
					REPORTER
							.logWarning("Classifiers file format: Header line third number assigns the number of assignments.\n"
									+ "Current value is "
									+ num_assignments
									+ " but currently program only supports one assignment.\n"
									+ "Ignoring value...");
			} else
				REPORTER
						.logWarning("Classifiers file format: Header line missing third number, num assignments");
			//System.out.println("CClassAssignmentsFile::run NumData: " +
			// num_data + " NumClasses: " + num_classes);
			return hdrInts;
		}
		...
		private Object interpretInteger(final Integer integer) {
			if (integer == null)
				return null;
			return ((integer.intValue() > 0) ? (Object) integer
					: (Object) ("Illegal value (" + integer + ")"));
		}

		private final boolean isClassifier(String classifier_line) {
			return (classifier_line != null && classifier_line.length() > 2 && classifier_line
					.startsWith("#"));
		}

		private final String[] processClassifier(String classifier_line,
				final int num_classes) throws java.text.ParseException {
			// optional: class label line, otherwise the data line
			// # Breast Colon Pancreas ...

			// remove the # because it could be "# CLASS1" or "#CLASS1"
			classifier_line = classifier_line.substring(classifier_line
					.indexOf('#') + 1);
			StringTokenizer st = new StringTokenizer(classifier_line);
			if (st.countTokens() != num_classes)
				throw new java.text.ParseException("first line specifies "
						+ num_classes + " classes,\nbut found "
						+ (st.countTokens()) + "!", 0);
		...
		private final int[] processData(String data_line,
				final int num_classes, final int[] values, final int num_data)
				throws IOException, java.text.ParseException {
			...
			try {
				...
				if (dataInd != num_data)
					throw new java.text.ParseException("Header specifies "
							+ num_data + " datapoints. But file contains "
							+ dataInd + " datapoints.", 0);
		...
		/**
		 * processes the data not as integers but as arbitrary strings problem
		 * is that the strings are not associated with the classifiers
		 */
		private final String[] processDataAsArbitraryStrings(String data_line,
				final int num_classes, final int[] values, final int num_data)
				throws IOException, java.text.ParseException {
			...
			// Allow the class assignments line to have arbitrary strings as
			// class names
			...
			do {
				...
				while (st.hasMoreTokens()) {
					...
					Integer class_index = (Integer) fClassNameMap
							.get(className);
					if (class_index == null) {
						if (classCount >= num_classes)
							throw new java.text.ParseException(
		...
		// fields
		/**
		 * wrapper for the current line being read. Used when throwing
		 * exceptions to indicate what was being read at the time of the problem
		 */
