...
public class FeaturesetPropertiesParser extends AbstractDataParser {
	...
	// "/Users/kohm/data/res_files/ucGCM_9_15000_All_SOM_2.gct__.gpp";
	...
	//            //FeaturesetPropertiesParser decoder = new FeaturesetPropertiesParser(in,
	// "Test_props");
	...
	//        }
	//        //display it
	//// final edu.mit.genome.gp.ui.odfviewer.AttributesViewer viewer = new
	// edu.mit.genome.gp.ui.odfviewer.AttributesViewer();
	...
	public boolean canDecode(final InputStream in) throws IOException {
		...
		try {
			...
			//  System.out.println("canDecode(InputStream): "+getClass()+" cannot
			// decode stream "+ex.getMessage());
		}
		return false;
	}
	 ...
	 * @param name
	 *            the name of the resulting data object
	 * @param in
	 *            the input stream
	 * @throws IOException
	 *             if a problem occurs due to an I/O operation
	...
	public SummaryInfo createSummary(final InputStream in) throws IOException,
		...
		if (exception != null)
			return new SummaryError(null, exception, this, primary,
					new HashMap());
	 ...
	 */
	protected static final FeaturesetPropertiesConverter getConverter(
			final DataModel model) {
		final FeaturesetPropertiesConverter converter = (FeaturesetPropertiesConverter) MODEL_TO_CONVERTER
				.get(model);
		return ((converter != null) ? converter
				: FeaturesetPropertiesConverter.PASSTHOUGH_CONVERTER);
	}

	/**
	 * Helper that fills the list with tokens. If a delimiter is found and the
	 * next token is also a delimter returns the empty String
	 * 
	 * @param list
	 *            the list to populate with strings
	 * @param toke
	 *            the reusable string tokenizer that has a string to parse
	 * @return a List the same one as in the params
	 */
	protected static List populateWithTokens(final List list,
			final ReusableStringTokenizer toke) {
	...
	static {
		...
		MODEL_TO_CONVERTER.put(Template.DATA_MODEL, TemplateConverter
				.instance());
		...
		if (limit != FeaturesetProperties.SUPPORTED_TYPES.length)
			throw new ArrayIndexOutOfBoundsException(
					"Internal error: unequal array lengths!\n"
							+ "SUPPORTED_CLASSES[" + limit + "] "
							+ "SUPPORTED_TYPES["
							+ FeaturesetProperties.SUPPORTED_TYPES.length + "]");
		TYPE_TO_CLASS = new java.util.HashMap(limit * 3 / 2);
		for (int i = 0; i < limit; i++) {
			TYPE_TO_CLASS.put(FeaturesetProperties.SUPPORTED_TYPES[i],
					FeaturesetProperties.SUPPORTED_CLASSES[i]);
		}

		// Create and map the instances of AbstractKeywordParser with the
		// lowercase
		// keywords minus their delimiters
		// CONSIDER: Could change this to:
		// parser = StringAssignmentParser(FeaturesetPropertiesEncoder.DW_MODEL,
		// getFieldFor("model_type"));
		// PARSERS.put(parser.getLowerCaseKeyword(), parser);
		//
		// parser =
		// StringListParser(FeaturesetPropertiesEncoder.KW_COLUMN_NAMES,
		// getFieldFor("column_names"));
		// PARSERS.put(parser.getLowerCaseKeyword(), parser);
		//
		// parser =
		// IntAssignmentParser(FeaturesetPropertiesEncoder.KW_DATA_LINES,
		// getFieldFor("data_rows_count"));
		//...
		// parser =
		// ObjectListParser(FeaturesetPropertiesEncoder.KW_COLUMN_TYPES,
		// getFieldFor("column_classes")) {
		//    protected Object transformToken(final String token, final InnerParser
		// fpp) {
	...
	protected static class InnerParser {
		...
		static {
			// odf files: SOMCluster, PredictionResults, GeneList,
			// PredictionFeatures
			keywords.add("model");
			keywords.add("model=");
			keywords.add("Model");
		 ...
		 *             if a problem occurs due to an I/O operation
		 * @throws ParseException
		 *             if there is a problem with the format or content of the
		 *             data
		 */
		protected void parseStream() throws IOException,
			...
			try {
				...
				//FIXME may not have the data_rows_count and will have to do
				// something
				// beside ArrayOArrays...
				//FIXME may not have defined the column_classes should default
				// to
				// an array of all String classes
				//FIXME should issue error if havn't defined any of the column
				// keyword
				// lists that determine the column_count (i.e. column_count ==
				// 0)
				...
				props = createFeaturesetProperties(the_name, getModel(),
						column_labels, column_descriptions, column_classes,
						arrays, attributes);
			} catch (IOException ex) {
				ex.printStackTrace();
				throw new IOException(
						"Error: while reading featureset properties \""
								+ fsp_name + "\":\n" + ex.getMessage());
			}
		}
		...
		protected void readVersion() throws IOException,
			...
			//FIXME digest the line to see if it is a odf with version, no
			// space i.e. odf1.0
			if (!magic.equalsIgnoreCase(FeaturesetPropertiesEncoder.MARKER_TAG)
					&& !magic.equalsIgnoreCase("sdf"))
				throw new java.text.ParseException(
						"Not a SDF/ODF data stream!\nFirst line: \"" + line
								+ "\"", 0);
			// Version number
			// FIXME digest the line to see if it is a lesser version that could
			// still be read
			if (!toke.hasMoreTokens())
				throw new java.text.ParseException(
						"Not a supported SDF/ODF version!\nFirst line: \""
								+ line + "\"", 0);
			final String version = toke.nextToken().trim();
			if (!version
					.equals(FeaturesetPropertiesEncoder.CURRENT_SDF_VERSION)) {
				throw new java.text.ParseException(
						"Not a supported SDF/ODF version!\n"
								+ "Current version = "
								+ FeaturesetPropertiesEncoder.CURRENT_SDF_VERSION
								+ "\nFirst line: \"" + line + "\"", 0);
			}
		}
		 ...
		 */
		protected int readHeaderCount() throws IOException,
				java.text.ParseException {
			reader.readLine(); // goes into current.line
			line_parser.parseLine(EQUALS_COLON);
			line_parser
					.checkSameKeyword(FeaturesetPropertiesEncoder.KW_HEADER_LINES);
			//System.out.println("header count line=
			// "+reader.getCurrentLine());

			try {
				header_lines_count = Integer.parseInt(line_parser.value.trim());
			} catch (NumberFormatException ex) {
				throw new java.text.ParseException("Cannot convert \""
						+ line_parser.value + "\" to a number!", reader
						.getTotalLineCount());
			}
		 ...
		 *             data
		 */
		protected void readHeaders(final int header_cnt) throws IOException,
				java.text.ParseException {
		 ...
		 *            the number of header lines
		 * @param primary
		 *            where the primary attributes will be stored
		 * @throws IOException
		 *             if a problem occurs due to an I/O operation
		 * @throws ParseException
		 *             if there is a problem with the format or content of the
		 *             data
		 */
		protected void readHeaders(final int header_cnt, final Map primary)
				throws IOException, java.text.ParseException {
			...
			try {
				if (primary == null) {
					...
					for (int i = 0; i < header_cnt; i++) {
						final String kw = parseKeyword();
						final AbstractKeywordParser parser = getParser(kw);
						final Object value = parser.parse(line_parser.value,
								this);
						if (line_parser.value.length() < 1000) // don't keep big
															   // stuff for
															   // summary
		 ...
		 *             data
		 */
		protected void getSummary(final int header_cnt, final Map map)
				throws IOException, java.text.ParseException {
			reader.setExpectEOF(false);
			try {
				for (int i = 0; i < header_cnt; i++) {
					...
					if (kw.equals("COLUMN_NAMES")) {
						if (value instanceof String[]) {
							...
							int dataColumns = s.length - 2; // assume file has
															// row names and row
															// descriptions
															// columns
		 ...
		 */
		protected String parseKeyword() throws IOException,
				java.text.ParseException {
			reader.readLine();
			if (line_parser.isRemark(FeaturesetPropertiesEncoder.KW_REMARK)) {
				// need to store intermediate results in this method for
				// recursion to work
		 ...
		 *            the remark's text
		 * @param kw
		 *            the keyword this remark is just after
		 */
		...
		/**
		 * calculates the real number of columns given the row names and
		 * descriptions may be in the main mbody
		 */
		private int calcColumnCount() {
			...
			final boolean has_row_names = (attributes.containsKey(rnc
					.substring(0, rnc.length() - 1)));
			final boolean has_row_descs = (attributes.containsKey(rnd
					.substring(0, rnd.length() - 1)));
		 ...
		 *             data
		 */
		protected void readAllRows() throws IOException,
				java.text.ParseException {
			...
			for (int r = 0; r < num_rows; r++) {
				...
				final List list = populateWithTokens(new ArrayList(toke
						.countTokens()), toke);
				...
				if (cnt != num_cols) {
					...
					throw new java.text.ParseException("Expecting " + num_cols
							+ " data tokens found " + cnt + " on line "
							+ reader.getLineCount() + "!\n"
		...
		// helper methods
		/**
		 * creates the name if possible otherwise use specified name from
		 * parse(...)
		 */
		private String createName(final Map attrs) {
			final String[] NAME_KEYS = new String[] { "name", "name=", "Name",
					"Name=", "NAME", "NAME=" };
		...
		//         *
		//         */
		//        protected final String readLine(final BufferedReader reader) throws
		// IOException {
		//            final String line = reader.readLine();
		//            current.line = line;
		//            current.total_num++;
		...
		//        }
		/**
		 * parses the String representations of the supported classes and
		 * returns the corresponding Class
		 * 
		 * @param rep
		 *            the class represention
		 * @return Class
		 */
		protected final Class parseClassRep(final String rep) {
			...
			if (clss == null)
				throw new java.util.NoSuchElementException(
						"No type defined for \"" + rep + "\"!");
			return clss;
		}

		/**
		 * returns a parser for the values associated with this keyword This
		 * could be overriden by subclass to expand the number of
		 * AbstractKeywordParser classes available for handling new Key Words
		 * that this doesn't know about
		 * 
		 * The overriding method should finaly call super.getParser(kw) if it
		 * couldn't find the keyword. Example code for subclass: protected
		 * AbstractKeywordParser getParser(final String kw) { final
		 * AbstractKeywordParser parser =
		 * (AbstractKeywordParser)ADDITIONAL_PARSERS.get(kw.toLowerCase()); if(
		 * parser == null ) return super.getParser(kw); return parser; }
		 * 
		 * @param kw
		 *            the key word
		...
		protected AbstractKeywordParser getParser(final String kw) {
			final AbstractKeywordParser parser = (AbstractKeywordParser) PARSERS
					.get(kw.toLowerCase());
			if (parser == null)
		 ...
		 * subclasses can override this to produce more specific DataObject
		 * instances
		 * 
		 * @param name
		 *            name of the data object
		 * @param model
		 *            the string representation of the data model
		 * @param column_labels
		 ...
		 * @param arrays
		 *            the array of arrays of the main data
		 * @param attributes
		 *            the attributes
		 * @return FeaturesetProperties
		 */
		protected FeaturesetProperties createFeaturesetProperties(
				final String name, final String model,
				final String[] column_labels,
				final String[] column_descriptions,
				final Class[] column_classes, final ArrayOArrays arrays,
				final Map attributes) {
			//            PropertiesFactory factory =
			// (PropertiesFactory)model_factory.get(model);
			//            if( model != null ) {
			//                return factory.createDataObjectProperties(name, model,
			// column_labels,
			...
			if (model.equals(SomProperties.DATA_MODEL.value))
				return new SomProperties(name, column_labels,
						column_descriptions, row_labels, row_descriptions,
						attributes);
			// default
			return new DefaultFeaturesetProperties(name, model, column_labels,
					column_descriptions, row_labels, row_descriptions,
					column_classes, arrays, attributes);
		}
		...
		/**
		 * ArrayOArrays is needed to construct a valid
		 * DefaultFeaturesetProperties
		 */
		private ArrayOArrays arrays;

		/**
		 * the type of model which could be "Dataset", "Prediction Results",
		 * etc.
		 */
	...
	static class LineParser {
		...
		/**
		 * parses the line for the key word using the delimiters to separate kw
		 * from value
		 * 
		 * @throws ParseException
		 *             if there is a problem with the format or content of the
		 *             data
		 */
		protected void parseLine(final char[] dels)
			...
			if (index == -1) {
				throw new java.text.ParseException(
						"Could not identify the key word using '"
								+ new String(dels) + "' delimiters!\n"
								+ "line (" + current.getTotalLineCount()
								+ "): " + line, current.getTotalLineCount());
			}
			this.delimiter = del;
			this.keyword = line.substring(0, index).trim();
			// don't trim the value it might have whitespace delimiters at the
			// beginning
		 ...
		 *             data
		 */
		protected void checkSameKeyword(final String kw)
				throws java.text.ParseException {
			final int len = kw.length() - 1;
			final String minus_del = kw.substring(0, len);
			if (!(minus_del.equalsIgnoreCase(keyword))) { // Error if not
														  // similar
				throw new java.text.ParseException("Error: Keyword \""
						+ keyword + "\" is not similar to the expected \""
						+ minus_del + "\"!", current.getTotalLineCount());
			}
			final char best = kw.charAt(len);
			if (!(delimiter == best)) { // has same delimiter
				addWarning("Keyword \"" + keyword
						+ "\" should be delimited with a '" + best
						+ "' i.e. \"" + kw + "\"");
			}
			if (!(minus_del.equals(keyword))) { // exact match
				addWarning("Case does not match: \"" + keyword
						+ "\" should be \"" + minus_del + "\"!");
			}
		}
		...
		protected boolean isRemark(final String remark_kw) {
			...
			if (line.startsWith(remark_kw)) {
				...
				this.delimiter = 0;
				this.value = line.substring(remark_kw.length() - 1,
						line.length()).trim();
				return true;
			}
			return false;
	...
	abstract static protected class AbstractKeywordParser {
		 ...
		 * 
		 * @param kw
		 *            the key word associated with this parser
		 */
		 ...
		 */
		abstract public Object parse(String value, InnerParser fpp)
				throws java.text.ParseException;
	...
	/**
	 * This should be subclasses by those classes that need to parse multiple
	 * values
	 */
	abstract protected static class MultiValueParser extends
		 ...
		 * 
		 * @param cnt
		 *            what the wrong count was
		 * @param fpp
		 *            inner parser
		 * @throws ParseException
		 *             to indicate that there is a problem with the format or
		 *             content of the data
		 */
		abstract protected void createException(final int cnt,
				final InnerParser fpp) throws java.text.ParseException;
		 ...
		 *            the initial count
		 * @param setter_kw
		 *            the key word that set the count
		 * @param fpp
		 *            the inner parser
		 * @throws ParseException
		 *             if there is a problem with the format or content of the
		 *             data
		 */
		abstract protected void setCount(final int cnt, final String setter_kw,
				final InnerParser fpp) throws java.text.ParseException;
		 ...
		 * 
		 * @param value
		 *            the text that represents an array
		 * @param fpp
		 *            the inner parser
		 * @param limit
		 *            the expected number of elements in array
		 * @throws ParseException
		 *             if there is a problem with the format or content of the
		 *             data
		 * @return Object[] the array
		 */
		protected Object[] breakString(final String value,
				final InnerParser fpp, final int limit)
				throws java.text.ParseException {
			final ReusableStringTokenizer toke = fpp.toke;
			//toke.resetTo(value, DELIMITERS, true);// get both tokens and
			// delimiters
			toke.resetTo(value, "\t", true);// get both tokens and delimiters
			//System.out.println("Initial number of
			// tokens+delims="+toke.countTokens());
			final List list = populateWithTokens(new ArrayList(toke
					.countTokens()), toke);
			//System.out.println("The list:");
			//edu.mit.genome.debug.FieldDumper.printArray(list.toArray(),
			// System.out, "' '");
			final int cnt = list.size();
			if (cnt != 0 && limit != 0 && cnt != limit) {
				//System.out.println("tab
				// count="+StringUtils.getNumOccurances(value, "\t"));
		...
		/**
		 * transforms the token into another Object (just returns the String
		 * here) can be overriden by subclasses to return a Class, Integer, etc
		 * 
		 * @param token
		 *            the string value
		 * @param fpp
		 ...
		 */
		protected Object transformToken(final String token,
				final InnerParser fpp) {
			return token;
		}
		 ...
		 * @return Object
		 */
		public Object parse(final String value, final InnerParser fpp)
				throws java.text.ParseException {
			final String name = getClass().getName();
			//System.out.println(name.substring(name.lastIndexOf('.'))+"'s
			// value \""+value+"\"");
	...
	abstract protected static class MultiValueCParser extends MultiValueParser {
		...
		/**
		 * throw the Exception with the proper information Default is to throw
		 * an exception if the cnt is not equal to the column_count this
		 * behavoiur can be overriden by subclasses if needed
		 * 
		 * @param cnt
		 *            the count
		 * @param fpp
		 ...
		 */
		protected void createException(int cnt, InnerParser fpp)
				throws java.text.ParseException {
			throw new java.text.ParseException(
					"There are not the expected number (" + fpp.column_count
							+ ") of " + getKeyword() + " but " + cnt
							+ "! Line:\n" + fpp.reader.getCurrentLine()
							+ "\nNote that the original count was set by "
							+ fpp.column_count_setter, fpp.reader
							.getTotalLineCount());
		}

		/**
		 * gets what the expected count should be or 0 if not set yet Currently
		 * gets the column_count but this could be overrideen by subclasses
		 * 
		 * @param fpp
		 *            the inner parser
		 * @return int the expected count
		...
		/**
		 * default implementation checks and/or sets the column count. Should be
		 * overridden if different behavior is desired
		 * 
		 * @param cnt
		 *            the initial count
		 * @param setter_kw
		 ...
		 *             data
		 */
		protected void setCount(final int cnt, final String setter_kw,
				final InnerParser fpp) throws java.text.ParseException {
			if (fpp.column_count <= 0) { // unset so set the value
				fpp.column_count = cnt;
				fpp.column_count_setter = setter_kw;
			} else if (cnt != 0 && fpp.column_count != cnt) { // error something
															  // is wrong
				throw new java.text.ParseException(
						"There are not the expected number ("
								+ fpp.column_count + ") of column headers but "
								+ setter_kw + "=" + cnt + "!\n"
	...
	abstract protected static class MultiValueRParser extends MultiValueParser {
		...
		/**
		 * throw the Exception with the proper information Default is to throw
		 * an exception if the cnt is not equal to the data_rows_count this
		 * behavoiur can be overriden by subclasses if needed
		 * 
		 * @param cnt
		 *            the wrong count
		...
		protected void createException(int cnt, InnerParser fpp)
				throws java.text.ParseException {
			throw new java.text.ParseException(
					"There are not the expected number (" + fpp.data_rows_count
							+ ") of " + getKeyword() + " but " + cnt
							+ "! Line:\n" + fpp.reader.getCurrentLine()
							+ "\nNote that the original count was set by "
							+ fpp.data_rows_count_setter, fpp.reader
							.getTotalLineCount());
		}

		/**
		 * gets what the expected count should be or 0 if not set yet Currently
		 * gets the data_rows_count but this could be overriden by subclasses
		 * 
		 * @param fpp
		 *            the inner parser
		 * @return int the expected count
		...
		/**
		 * default implementation checks and/or sets the row count. Should be
		 * overridden if different behavior is desired
		 * 
		 * @param cnt
		 *            the initial count
		 * @param setter_kw
		 *            the key word that caused the setting of the count
		 * @param fpp
		 *            the inner parser
		 * @throws ParseException
		 *             if there is a problem with the format or content of the
	...
	private static class RowCountParser extends AbstractKeywordParser {
		 ...
		 *             data
		 */
		public Object parse(String value, InnerParser fpp)
				throws java.text.ParseException {
			//System.out.println("RowCountParser value=("+value+")");
			try {
				fpp.data_rows_count = Integer.parseInt(value.trim());
			} catch (NumberFormatException ex) {
				throw new java.text.ParseException("Cannot convert \"" + value
						+ "\" to a number!", fpp.reader.getTotalLineCount());
			}
	...
	private static class ColumnDescriptionsParser extends MultiValueCParser {
		...
		/** sets the appropriet array in FeaturesetPropertiesParser */
		protected void setArray(Object[] values, InnerParser fpp) {
			fpp.column_descriptions = (values != null && values.length > 0) ? (String[]) values
					: (String[]) null;
		}
	...
	private static class RowDescriptionsParser extends MultiValueRParser {
		...
		/** sets the appropriet array in FeaturesetPropertiesParser */
		protected void setArray(Object[] values, InnerParser fpp) {
			fpp.row_descriptions = (values != null && values.length > 0) ? (String[]) values
					: (String[]) null;
		}
	...
	abstract protected static class AssignmentParser extends
			AbstractKeywordParser {
		AssignmentParser(final String kw) {
			...
			if (kw.charAt(kw.length() - 1) != del)
				throw new IllegalStateException(
						"Wrong key word for an assignment!\n Should end in a"
								+ del + ".");
		}
	...
	/**
	 * Assigns the parsed String value subclasses must implement
	 * setValue(String);
	 */
	abstract protected static class StringAssignmentParser extends
			AssignmentParser {
		 ...
		 *             data
		 */
		protected void setValue(String value, InnerParser fpp)
				throws java.text.ParseException {
			setValue(value);
		}
	...
	abstract protected static class IntAssignmentParser extends
			AssignmentParser {
		 ...
		 *             data
		 */
		protected void setValue(final String value, final InnerParser fpp)
				throws java.text.ParseException {
			try {
				final int i = Integer.parseInt(value.trim());
				setValue(i);
			} catch (NumberFormatException ex) {
				throw new java.text.ParseException("Could not set the "
						+ getKeyword() + " value!" + "\"" + value
						+ "\" is not an integer.", fpp.reader
						.getTotalLineCount());
			}
		}
	...
	abstract protected static class ListRowParser extends MultiValueRParser {
		...
		/** sets the array */
		//abstract protected void setArray(final String value, final
		// InnerParser fpp) throws ParseException;
	} // end AssignmentParser
	...
	private static final class DefaultParser extends MultiValueParser {
		...
		/**
		 * default implementation checks and/or sets the column count. This is
		...
		/**
		 * gets what the expected count should be or 0 if not set yet Overridden
		 * to return 0.
		 */
		protected int getCount(final InnerParser fpp) {
			return 0;
		...
		/**
		 * parses the value overrides the super Since the keyword is undefined
		 * cannot verify that the proper delimiter was used; nor correct for the
		 * improper use of '=' when ':' should have been used to specify a list.
		 * 
		 * @throws ParseException
		 *             if there is a problem with the format or content of the
		 *             data
		...
		public Object parse(final String value, final InnerParser fpp)
			...
			//System.out.println("DefaultParser's key \""+key+"\" del '"+del+"'
			// value \""+value+"\"");
		...
		/**
		 * throw the Exception with the proper information This is overridden to
		 * do nothing
		 */
		protected void createException(int cnt, InnerParser fpp)
				throws java.text.ParseException {
		...
		/**
		 * sets the appropriet array in FeaturesetPropertiesParser This is
		 * overridden to do nothing
		 */
		protected void setArray(Object[] values, InnerParser fpp) {
			// not implemented
	...
	//        
	//        /** parses the value */
	//        public void parse(final String value, final InnerParser fpp) throws
	// ParseException {
	//
	//        }
	//        
