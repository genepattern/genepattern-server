...
public class LineReader {
		...
		if (current == null) {
			if (expect_eof) {
				...
				throw new EOFException("Error: Not expecting to reach end"
						...
						+ ((expected_row_count < 0) ? "!" : "should be " //FIXME
																		 // count
																		 // is
																		 // wrong!
	...
	/**
	 * sets the current line This can be overridden by supclasses but must be
	 * called to set the line variable
	 * 
	 * protected String setCurrentLine(final String current) { // do somthing
	 * with it final String something = processString(current); return
	 * super.setCurrentLine(something); }
	 * 
	 * @param current
	 *            the new line
	 ...
	 * @return BufferedReader
	 */
	public static final BufferedReader createReader(final File file)
			throws FileNotFoundException {
	 ...
	 * 
	 * @param expect_eof
	 *            if false then readLine method throws end of file exception if
	 *            reached
	 */
