...
public abstract class AbstractDatasetParser extends AbstractDataParser {
     ...
     * @param lenient true then this will parse leniently
     * @param extensions supported file extensions
     */
    protected AbstractDatasetParser(final boolean lenient, final String[] extensions) {
        super(extensions);
    }
//    /** Creates new AbstractDatasetParser using the lieniency defined in the properties*/
//    protected AbstractDatasetParser(final InputStream input, final String ds_name, final String[] extensions) throws IOException, org.genepattern.io.ParseException{
//        this(input, ds_name, IS_LENIENT, extensions);
//    }
//    /** Creates new AbstractDatasetParser using the lieniency defined in the properties*/
//    protected AbstractDatasetParser(final String ds_name, final String[] extensions) throws IOException, org.genepattern.io.ParseException{
...
//    }
     ...
     */
    abstract protected AbstractInnerParser createInnerParser(final LineReader reader);
    
    // DataParser interface signature methods
    
    /** Parses the InputStream  to create a DataObjector instance with the specified name
     *
     * FIXME not thread safe.  Need to eliminate all the variables that store state
     * should have no instance fields
     * @return the resulting data object
     * @param name the name for the DataObjector
     ...
     * Note that this assumes that its reading from the beginning of the stream.
     * @param in  the input stream
     * @throws IOException if a problem occurs due to an I/O operation
    ...
    public SummaryInfo createSummary(final InputStream in) throws IOException, java.text.ParseException {
        ...
        try {
            //total = 0; // reset
				secondary.put("Size=", (long)(Math.ceil(in.available()/1024.0)) + " KB");
     ...
     * @throws IOException if an error occures durring an i/o operation
     * @throws org.genepattern.io.ParseException if a problem with the data format
     * @return String the header
     */
    public String getFullHeader(final InputStream in) throws IOException, java.text.ParseException {
        final StringBuffer buf = new StringBuffer();
        final AbstractInnerParser parser = createInnerParser(new LineRecorder(in, buf));
        try {
            parser.readHeader();
        } catch (java.text.ParseException ex) {
            REPORTER.logWarning(getClass()+" cannot decode stream "+ex.getMessage());
     ...
     * The tokens are seperated by commas ',' and the tokens are case-insensitive.
     */
     ...
     * and don't need to have gene names etc.
     * The tokens are case-insensitive: "true", "True", "TRUE" are the same.
     *
    ...
    static {
        ...
        try {
            strict = GPpropertiesManager.getBooleanProperty(STRICT_DATASET_PARSING_KEY);
        } catch (java.text.ParseException ex) {REPORTER.logWarning(ex.toString());}
        IS_LENIENT = !strict;
     
        final String infinities = GPpropertiesManager.getProperty(POSITIVE_INFINITYS_KEY, "Infinity");
        POSITIVE_INFINITYS = StringUtils.splitStrings(infinities, delim);
        
        final String neginfinities = GPpropertiesManager.getProperty(NEGATIVE_INFINITYS_KEY, "-Infinity");
        NEGATIVE_INFINITYS = StringUtils.splitStrings(neginfinities, delim);

        final String nans = GPpropertiesManager.getProperty(POSITIVE_INFINITYS_KEY, "NaN");
    ...
    protected abstract static class AbstractInnerParser {
         ...
         * @throws org.genepattern.io.ParseException if problem with format of data
         * @throws NumberFormatException if problem with reading some of the numberic data
         * @throws IOException if problem durring I/O operations
         * @return Dataset the dataset object
         */        
        protected final Dataset readStream() throws java.text.ParseException, NumberFormatException, IOException {
            //total = 0; // reset
            // read header... if created tmp_file then read from that
            final File tmp_file = readHeader();
            
            //FIXME must set the LineReader.num and total to reflect the numnber
            // of lines read by readHeader
            final LineReader reader = ( tmp_file == null) ? this.reader : new LineReader(tmp_file);
            this.reader = reader;
            parseDataLines();
            reader.close();
            fixDuplicateLabels();
            ...
            AnnotationFactory rfactory = PrimeAnnotationFactory.createAnnotationFactory(this.row_labels, this.row_descriptions);
            AnnotationFactory cfactory = PrimeAnnotationFactory.createAnnotationFactory(this.column_labels, this.column_descriptions);
            return  new DefaultDataset(this.dataset_name,
                new DefaultMatrix(this.row_count, this.column_count, this.data_array),
                new DefaultNamesPanel(this.row_labels, rfactory),
                new DefaultNamesPanel(this.column_labels, cfactory));
        }
        
        //abstract methods
        /** reader method  may return a file if the rest of the input stream was dumped
         * @param primary were primary summary info goes
         * @param secondary were secondary summary info goes
         * @throws org.genepattern.io.ParseException if problem with the data's format
         * @throws NumberFormatException problem with the format of a number
         * @throws IOException problem durring I/O operation
         * @return File if a temp file was created, otherwise null
         */
        abstract protected File readHeader(final Map primary, final Map secondary) throws java.text.ParseException, NumberFormatException, IOException;
        /** get the max number of tokens exzpected on a line */
        abstract int getMaxNumEntriesPerLine();
        /** subclass processes each line */
        abstract void processDataTokens(int row, String[] tokens, int start, int end) throws NumberFormatException;
         ...
         * @param row the row were the EOF was found
         */
        abstract protected void processEOF(int row);
        
        // helper methods
        /** like the abstract method of the same name but does not expect to record the attributes
         * @throws org.genepattern.io.ParseException if problem with data's format
         * @throws NumberFormatException if expected number is not one
         * @throws IOException if problem durring I/O operation
         * @return File the temp file to read from or null
         */
        protected File readHeader() throws java.text.ParseException, NumberFormatException, IOException  {
            return readHeader(null, null);
        }
        /** Parses the token into a float
         * If is_lenient is true then converts anomalous values into their
         * primative representations
         *
         * This method is intended to be used in processDataTokens(...)
         * @param value text version of the value
         * @param r its' row
         * @param c its' column position
        ...
        /**
         * helper method that determines if any element in the array equals
         * the target ignoring case
         */
        private static final boolean containsString(final String target, final String[] array) {
        ...
        /** reads a line
         * @throws IOException if an error happens while I/O operation
         * @throws org.genepattern.io.ParseException if a problem with the data's format is detected
         */
        protected void parseDataLines() throws IOException, java.text.ParseException{
            ...
            try {
                for(int r = row - 1; curLine != null; curLine = reader.readLine()) {
                    this.lines_read++;
                    ...
                    } else if(tok_count == (max_num_per_line - 1)) {// assume no description
                        labels[r] = tokenizer.nextToken();
                        descs[r] = empty;
                        no_desc  = true;
                        no_label = false;
                    } else if(this.is_lenient && tok_count == (max_num_per_line - 2)) {
                        // assume no name nor description
                        labels[r] = gene+no_label_count; // gene0 gene1 ...
                        descs[r] = empty;
                        no_desc = no_label = true;
                    } else {
                        throw new java.text.ParseException((tok_count > max_num_per_line ? "More" : "Less")
                        +" data elements  ("+tok_count
                        +") than expected ("+max_num_per_line+") on data row "+row
                ...
                if(row > this.row_count)
                    throw new java.text.ParseException("Too many lines of data! Read "+(row)
                    +" expected " + this.row_count
                    + getExceptionMessageTrailer(), 0);
                int c = count - start + 1;
                if((c + 1)  > this.column_count)
                    throw new java.text.ParseException("Too many data value(s)at line "
                    +this.lines_read +": expected "+this.column_count+" got "+(c + 1)
                    + getExceptionMessageTrailer(), 0);
                REPORTER.logWarning("row="+row+" NUM_ROWS="+this.row_count+" c="+c+" NUM_COLS="+this.column_count);
                throw ex; // other internal problem...
            } catch (NumberFormatException ex) {
                if(no_desc || no_label)
                    throw new java.text.ParseException("Less data elements ("+tok_count
                    +") than expected ("+max_num_per_line+")\n"
                    +"Failed at: data element "+(this.current_column)
                    +", token:\""+tokens[this.current_column]+"\""
                    + getExceptionMessageTrailer(), 0);
                else {
                    throw new java.text.ParseException("Expected a number got text at line "
                    +this.lines_read+" data element "+(this.current_column)+" token:\n\""+tokens[this.current_column]+"\"\n"
                    +" with Label: \""+labels[row - 1]+"\"\n"
                    +"Description: \""+descs[row - 1]+"\""
                    + getExceptionMessageTrailer(), 0);
                }
            }
            row--;
            processEOF(row);
            if(row < this.row_count)
                throw new java.text.ParseException("Missing data rows! Read "+(row)
                +" expected "+this.row_count + getExceptionMessageTrailer(), 0);
            else if(row > this.row_count)
                throw new java.text.ParseException("Too many data rows! Read "+(row)
                +" expected "+this.row_count + getExceptionMessageTrailer(), 0);
        }
            ...
            for(int i = 0; i < len; i++) {
                ...
                do {
                    ...
                    if(value != null) {
                        ...
                        REPORTER.logWarning(v+" duplicates found for ("+labels[i]+")");
                        labels[i] +="_"+v;
                        do_over = true;
                    } else do_over = false;
        ...
        /** changes the NaN values to be something else i.e. Infinity or -Infinity */
        ...
        protected final void setRowColumnCounts(final int r, final int c) {
            if(total != 0)
                throw new IllegalArgumentException("the total number of things to do has been already calculated");
            total = (r * 2) + c;
            row_count    = r;
            column_count = c;
        ...
        /** the total number of lines to read + lines to check for duplicate labels */
        private   int total = 0;
        /** the line tokenizer */
        ...
        /**
         * if this is true then the parser will try to ignore certain irregularities
         * but could cause problems with the integrety of the data
         */
