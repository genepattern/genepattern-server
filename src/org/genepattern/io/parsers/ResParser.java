...
public class ResParser extends AbstractDatasetParser {
    ...
    protected final static class InnerParser extends AbstractInnerParser {
         ...
         * @param primary where the main summary information is stored
         * @param secondary where the less important summary information is stored
         * @throws NumberFormatException if couldn't parse a number where a number was expected
         * @throws ParseException if a format problem was detected in the data
         * @throws IOException if error occurs during an I/O operation
         * @return a temp file if needed or null
         */
        protected void getSummary(Map map) throws IOException, java.text.ParseException {
            ...
            if(curLine.trim().length() > 0) {
                ...
                try {
                    ...
                    throw new java.text.ParseException("Parser expecting number of rows on third line", 0);
					 }
				} else {
					throw new java.text.ParseException("Parser expecting number of rows on third line", 0);
				}
         ...
         * @return a temp file if needed or null
         */
        protected final File readHeader(final Map primary, final Map secondary) throws java.text.ParseException, NumberFormatException, IOException {
            final LineReader reader = this.reader;
            reader.setExpectEOF(false);
            ...
            if (curLine.length() > 0) {
                ...
                if (st.hasMoreTokens()) {
                    ...
                    if( !ResParser.DESCRIPTION.equalsIgnoreCase(tk) ) {
                        throw new java.text.ParseException("Not a 'res' file - first line "
                        +"does not start with \""+DESCRIPTION+"\"!", 0);
                    }
                }
                if (st.hasMoreTokens()) {
                    final String tk = st.nextToken().trim();  // Accession
                    if( !ACCESSION.equalsIgnoreCase(tk) ) {
                        throw new java.text.ParseException("Not a 'res' file - first line second token"
                        +"is not \""+ACCESSION+"\"!", 0);
                    }
                }
            ...
            if (curLine.length() > 0) {
               ...
               // if( primary != null ) primary.put("COLUMN_DESCRIPTIONS:", descArr);
            }
            ...
            //if( primary != null ) primary.put("COLUMN_DESCRIPTIONS:", col_descs);
            ...
            if(curLine.trim().length() > 0) {
                ...
                try {
                    ...
                    haveNumRows = true;
                    // exception here means ("ResParser::no or invalid numRows specified");
                } catch (NumberFormatException e) {
                    REPORTER.logWarning("ResParser::Expecting a number - the number of rows\nNo or invalid number of rows specified:\n"+curLine);
                    if( secondary != null ) secondary.put("Warning=", "Parser expecting number of rows on third line");
                    // rereading is safer than using reader.mark(some_buffer_size_too_small)
                    // before reading this line
                    // an then in here (caught Exception) doing: reader.reset();
                    
                    // so count em...
                    
                    tmp_file = org.genepattern.io.StorageUtils.createTempFile("res-like");
            ...
            //        System.out.println("ResParser::Starting to read the data portion");
            return tmp_file;
         ...
         * @throws NumberFormatException if couldn't parse a number where a number was expected
         */
        void processDataTokens(final int row, final String[] tokens, final int start, final int end) throws NumberFormatException {
            final int r = row -1;
         ...
         * @param row the row where end of file was found
         */
