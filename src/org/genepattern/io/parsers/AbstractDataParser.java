...
abstract public class AbstractDataParser implements DataParser {
     ...
     * @param extensions the file extensions that this parser supports
     */
    protected AbstractDataParser(final String[] extensions) {
        final String[] exts = (String[])extensions.clone(); // just to be safe-should be small arrays
    ...
    public static final String getLowercaseFileExtension(final java.io.File file) {
        if( !file.isFile() )
            return null; // should this be an error condition throws SomekindaException
     ...
     * extension seperator wich is usually a dot, '.', a.k.a. a period character.
     *
     * Note that the file extension strings are expected be in lower case for convenience
     *      and the array sorted!
     ...
     *FIXME this could be system dependent
     */
    ...
    static {
        ...
        final String fes = org.genepattern.util.GPpropertiesManager.getProperty("gp.dataparser.file.ext.separator", ".");
    ...
    static class LineRecorder extends LineReader {
        ...
        public LineRecorder(final InputStream input, final StringBuffer buf) throws FileNotFoundException {
            super(input);
            this.buffer = buf;
        }
        /** Creates a new instance of LineReader */
        public LineRecorder(final File file, final StringBuffer buf) throws FileNotFoundException {
            super(file);
            this.buffer = buf;
        }
