...
public class GctParser extends AbstractDatasetParser {
	...
	//    public GctParser(final InputStream input, final String name,final boolean
	// lenient) throws IOException, org.genepattern.io.ParseException{
	//        super(input, name, lenient, FILE_EXTENSIONS);
	//    }
	//    /** Creates a new instance of GctParser with system defined leniency */
	//    public GctParser(final InputStream input, final String name) throws
	// IOException, org.genepattern.io.ParseException{
	...
	protected final static class InnerParser extends AbstractInnerParser {
		...
		protected void getSummary(Map map) throws IOException,
			...
			lines_read = 0;
			// this is trimmed so eliminates problems of whitespace preventing
			// equality
			...
			if (curLine.equals(VERSION_1_TOKEN))
				...
				// e.g. if there is no version line
				fileFormat = 0;
			...
			//   put this first so we can allocate arrays for rows and columns
			// before
			//   reading them in
			...
			if (hdrInts[1] > 0) {
				...
				throw new java.text.ParseException("Missing <columns> value "
						+ getExceptionMessageTrailer(), 0);
		...
		/**
		 * reads the header information and if the primary or seconday maps ar
		 * not null saves the attributes
		 * 
		 * @param primary
		 *            where the primary summay info is stored
		 * @param secondary
		 *            where the summay info of secondary importance is stored
		 * @throws org.genepattern.io.ParseException
		 ...
		 */
		protected File readHeader(final Map primary, final Map secondary)
			...
			if (curLine.equals(VERSION_1_TOKEN))
				...
				fileFormat = 0;
			if (secondary != null)
				secondary.put("GCT version=", new Integer(fileFormat));
			...
			if (fileFormat >= 2) {
				...
				// '#')
				// # <"Name"> <tab> <"Desc"> <tab> <col 0 name> <tab> <col 1
				// name> <tab> <...>
				...
				if (st.nextToken().trim().equals("#")) // pop the "Name" column
													   // label or "#"
					st.nextToken(); // pop "name"
				String desc = st.nextToken(); // pop the "Description" column
											  // label
				...
				if (tok_count != strArr.length) {
					throw new java.text.ParseException(
							(tok_count > strArr.length ? "More" : "Less")
									+ " column names("
									+ st.countTokens()
									+ ") than number of columns specified("
									+ strArr.length
									+ ")\n"
				...
				java.util.Arrays.fill(column_labels, "C");// they must not be
														  // null for BPOG
			}
		 ...
		 *            the row index
		 * @param tokens
		 *            array of tokens from the line
		 * @param start
		 *            start column
		 * @param end
		 *            end column
		 * @throws NumberFormatException
		 *             if a float was not parsable
		 */
		protected final void processDataTokens(final int row,
				final String[] tokens, final int start, final int end)
				throws NumberFormatException {
			final int r = row - 1;
			int c = 0;
			int i = start;
		 ...
		 * 
		 * @param row
		 *            the row index where the end of file occured
		 */
