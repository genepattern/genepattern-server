...
public class MAGEMLWriter implements IExpressionDataWriter {
	...
	public void write(IExpressionData expressionData, OutputStream os) throws IOException {
		...
		avgDiffQuantitationType.setIdentifier("Affymetrix:QuantitationType:CHPAvgDiff");
		avgDiffQuantitationType.setName("Avg Diff");
		avgDiffQuantitationType.setIsBackground(Boolean.FALSE);

		quantitationTypeDimension.addToQuantitationTypes(avgDiffQuantitationType);
		quantitationType_package.addToQuantitationType_list(avgDiffQuantitationType);

		if(expressionData instanceof IResExpressionData) {
			PresentAbsent callQuantitationType = new PresentAbsent();
			callQuantitationType.setIdentifier("Affymetrix:QuantitationType:CHPAbsCall");
			...
			d.setText("Indication of the confidence in the measured expression value");
			descriptions_list.add(d);
			callQuantitationType.setDescriptions(descriptions_list);

			quantitationTypeDimension.addToQuantitationTypes(callQuantitationType);
			quantitationType_package.addToQuantitationType_list(callQuantitationType);
		}
		...
		for(int i = 0; i < expressionData.getRowCount(); i++) {
			...
			/*
			    ...
			    descriptions_list.add(d);
			    reporter.setDescriptions(descriptions_list);
		...
		for(int j = 0; j < expressionData.getColumnCount(); j++) {
			...
			/*
			    String desc = expressionData.getColumnDescription(j);
			    if(desc != null) {
			    Descriptions_list descriptions_list = new Descriptions_list();
			    Description d = new Description();
			    d.setText(desc);
		...
		if(storeDataExternally) {
			...
			for(int j = 0; j < expressionData.getColumnCount(); j++) {
				for(int i = 0; i < expressionData.getRowCount(); i++) {
					buf.append(String.valueOf(expressionData.getValueAsString(i, j)));
					buf.append(" ");
					if(resExpressionData != null) {
						String call = resExpressionData.getCallAsString(i, j);
		...
		bioAssayData_package.addToDesignElementDimension_list(designElementDimension);
		bioAssayData_package.addToQuantitationTypeDimension_list(quantitationTypeDimension);
