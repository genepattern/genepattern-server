 ...
 *  GeneCluster. These files use spaces to separate the fields.</P>
 *  <UL>
 *    <LI> The first line of a CLS file contains numbers indicating the number
 *    of samples and number of classes. The number of samples should correspond
 *    to the number of samples in the associated RES or GCT data file. </LI>
 *
 *    <UL>
 *      <LI> Line format: (number of samples) (space) (number of classes)
 *      (space) 1 </LI>
 *      <LI> For example: 58 2 1</LI>
 *    </UL>
 *
 *    <LI> The second line in a CLS file contains names for the class numbers.
 *    The line should begin with a pound sign (#) followed by a space. </LI>
 *
 *    <UL>
...
public class ClsParser {
   ...
   List suffixes = Collections.unmodifiableList(Arrays.asList(new String[]{"cls"}));
   ...
   /**
    *  Parses the <CODE>InputStream</CODE> to create a <CODE>ClassVector</CODE>
    *  instance
    *
    * @param  is                  The input stream
    * @exception  org.genepattern.io.ParseException  If there is a problem with the data
    * @throws  IOException        if an I/O error occurs while reading the
    *      stream.
    */
   public void parse(InputStream is) throws IOException, org.genepattern.io.ParseException {
   ...
   void read() throws IOException, org.genepattern.io.ParseException {
      ...
      if(hasClassNames(classifierLine)) {
         ...
         assignments = processData(dataLine, numClasses, numItems, classNumber2NameMap);
   ...
   private void processHeader() throws IOException, org.genepattern.io.ParseException {
      ...
      if(tok.countTokens() != 3) {
         throw new org.genepattern.io.ParseException("Header line needs three numbers!\n"
                + "\"" + headerLine + "\"");
      }
      try {
         for(int i = 0; i < 3; i++) {
         ...
         throw new org.genepattern.io.ParseException("Header line element '" + e.getMessage() + "' is not a number!");
      }

      if(hdrInts[0] <= 0) {
         throw new org.genepattern.io.ParseException("Header line missing first number, number of data points");
      }
      if(hdrInts[1] <= 0) {
         throw new org.genepattern.io.ParseException("Header line missing second number, number of classes");
      }

     /* if(hdrInts[2] != 1) {
         throw new org.genepattern.io.ParseException("Third number on 1st line must '1'.");
      }
      */
   ...
   private String[] processClassifier(String classifierLine, int num_classes) throws IOException {
      // optional: class label line, otherwise the data line
      // # Breast Colon Pancreas ...

      // remove the # because it could be "# CLASS1" or "#CLASS1"
      classifierLine = classifierLine.substring(classifierLine.indexOf('#') + 1);
    ...
    * @exception  org.genepattern.io.ParseException  Description of the Exception
    */
   private String[] processData(String data_line, int num_classes, int num_data, Map classNumber2ClassNameMap) throws IOException, org.genepattern.io.ParseException {
      // data line <int0> <space> <int1> <space> <int2> <space> ...
      if(data_line == null) {
         throw new org.genepattern.io.ParseException("Missing data (numbers seperated by spaces) in 3rd line");
      }
      try {
         ...
         do {
            ...
            while(st.hasMoreTokens()) {
               ...
               if(classNumber >= num_classes || classNumber < 0) {
                  throw new org.genepattern.io.ParseException("Header specifies " + num_classes
                         + " classes, but data line contains a " + classNumber + ", a value "
                         + "that is too " + (classNumber < 0 ? "small" : "large") + "."
                         + "All data x for this file must be in the range 0-"
                         + (num_classes - 1) + ".");
               }
               String name = (String) classNumber2ClassNameMap.get(new Integer(classNumber));
         ...
         if(dataInd != num_data) {
            throw new org.genepattern.io.ParseException("Header specifies " + num_data + " datapoints. But file contains " + dataInd + " datapoints.");
         }

         if(numClasses != num_classes) {
            throw new org.genepattern.io.ParseException("Header specifies " + num_classes + " classes. But file contains " + numClasses + " classes.");
         }
         return assignments;
