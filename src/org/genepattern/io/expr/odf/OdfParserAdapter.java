...
class OdfParserAdapter implements IExpressionDataParser {
   ...
   class MyHandler implements IOdfHandler {
      ...
      public void data(int i, int j, String s) throws org.genepattern.io.ParseException {
         if(j == rowNamesColumn) {
            ...
            try {
               if(expressionHandler != null) {
                  expressionHandler.data(i, j - dataOffset, Double.parseDouble(s));
               }
            } catch(NumberFormatException nfe) {
               throw new org.genepattern.io.ParseException("Data at row " + i + " and column " + j + " is not a number.");
            }
         }
      }
      ...
      public void endHeader() throws org.genepattern.io.ParseException {
         if(rowNamesColumnSpecified && rowNames != null) {
            throw new org.genepattern.io.ParseException("Row names specifed in header and in data block.");
         }

         if(rowDescriptionsColumnSpecified && rowDescriptions != null) {
            throw new org.genepattern.io.ParseException("Row descriptions specifed in header and in data block.");
         }
         if(rowNames == null) {
         ...
         if(columnNames == null) {
            throw new org.genepattern.io.ParseException("No column names found.");
         }
         if(columnDescriptions == null) {
            throw new org.genepattern.io.ParseException("No column descriptions found.");
         }
         if(columnTypes == null) {
            throw new org.genepattern.io.ParseException("No column types found.");
         }
         if(columnNames.length != columnDescriptions.length) {
            String[] temp = new String[columnNames.length];
            int stop = Math.min(columnNames.length, columnDescriptions.length);
          ...
          //  throw new org.genepattern.io.ParseException("Length of column names(" + columnNames.length + ") not equal to length of column descriptions(" + columnDescriptions.length + ").");
         }

         if(columnNames.length != columnTypes.length) {
            throw new org.genepattern.io.ParseException("Length of column names not equal to length of column types.");
         }

         if(!rowDescriptionsColumnSpecified && rowDescriptions == null) {
            throw new org.genepattern.io.ParseException("No row descriptions found.");
         }

         if(!rowNamesColumnSpecified && rowNames == null) {
            throw new org.genepattern.io.ParseException("No row names found.");
         }

         if(rowNamesColumnSpecified && !columnTypes[0].equals("String")) {// backward compatibilty
            throw new org.genepattern.io.ParseException("0th column type must be String.");
         }
         if(rowDescriptionsColumnSpecified && !columnTypes[1].equals("String")) {// backward compatibilty
            throw new org.genepattern.io.ParseException("1st column type must be String.");
         }

         if(rowNamesColumnSpecified && rowNamesColumn != 0) {
            throw new org.genepattern.io.ParseException("RowNamesColumn must be  column 0.");
         }

         if(rowDescriptionsColumnSpecified && rowDescriptionsColumn != 1) {
            throw new org.genepattern.io.ParseException("RowNamesColumn must be column 1.");
         }
         ...
         for(int i = dataOffset; i < columnTypes.length; i++) {// for compatibilty with other odf parser
            if(!columnTypes[i].equals("float")) {
               throw new ParseException("The column type of column " + i + " must be float.");
            }
         }
         ...
         if(expressionHandler != null) {
            expressionHandler.init(rows, actualColumns, rowDescriptionsColumnSpecified, true, false);
         }
         if(expressionHandler != null) {
            for(int j = 0; j < actualColumns; j++) {
               expressionHandler.columnName(j, columnNames[j + dataOffset]);
               if(columnDescriptions!=null) {
                  expressionHandler.columnDescription(j, columnDescriptions[j + dataOffset]);
               } else {
      ...
      private int parsePositiveInt(String key, String value) throws ParseException {
         try {
