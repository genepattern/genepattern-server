...
public class GctParser implements IExpressionDataParser {
   ...
   void readData() throws org.genepattern.io.ParseException, IOException {
      ...
      for(String s = reader.readLine(); s != null; s = reader.readLine(), rowIndex++) {
         if(rowIndex >= rows) {
            ...
            throw new org.genepattern.io.ParseException("More data rows than expected on line " + reader.getLineNumber() + ". Read " + rowsRead + ", expected " + rows + ".");
         }
         String[] tokens = s.split("\t");
         if(tokens.length != expectedColumns) {
            throw new org.genepattern.io.ParseException(tokens.length + getColumnString(tokens.length) + " on line " + reader.getLineNumber() + ". Expected " + expectedColumns + ".");
         }
         String rowName = tokens[0];
         if(rowNamesSet.contains(rowName)) {
            throw new org.genepattern.io.ParseException("Duplicate row name " + rowName + " on line " + reader.getLineNumber());
         }
         ...
         if(handler != null) {
            for(int columnIndex = 0; columnIndex < columns; columnIndex++) {
               try {
                  handler.data(rowIndex, columnIndex, Double.parseDouble(tokens[columnIndex + 2]));
               } catch(NumberFormatException nfe) {
                  throw new org.genepattern.io.ParseException("Data at line number " + reader.getLineNumber() + " and column " + columnIndex + " is not a number.");
               }
            }
         }
      }
      if(rowIndex != rows) {
         throw new org.genepattern.io.ParseException("Missing data rows. Read " + rowIndex + " " + getRowString(rowIndex) + ", expected " + rows);
      }
   }
   ...
   void readHeader() throws org.genepattern.io.ParseException, IOException {
      ...
      if(!versionLine.equals(VERSION)) {
         throw new org.genepattern.io.ParseException("Unknown version on line 1.");
      }
      String dimensionsLine = reader.readLine().trim();// 2nd header line: <numRows> <tab> <numCols>

      String[] dimensions = dimensionsLine.split("\t");
      if(dimensions.length != 2) {
         throw new org.genepattern.io.ParseException("Line number " + reader.getLineNumber() + " should contain the number of rows and the number of columns separated by a tab.");
      }
      ...
      if(rows <= 0 || columns <= 0) {
         throw new org.genepattern.io.ParseException("Number of rows and columns must be greater than 0.");
      }
      if(handler != null) {
      ...
      if(columnNames.length != expectedColumns) {
         throw new org.genepattern.io.ParseException("Expected " + expectedColumns + " tokens got " + columnNames.length + " tokens on line " + reader.getLineNumber() + ".");
      }
      Collection columnNamesSet = new HashSet(columns);
      for(int j = 0; j < columns; j++) {
         String columnName = columnNames[j + 2];
         if(columnNamesSet.contains(columnName)) {
            throw new org.genepattern.io.ParseException("Duplicate column name " + columnName);
         }
