 ...
 * 
 * Only requires that the genes and their associated annotations be known
...
//    }
    ...
    FileAnnotation(final String[] features, final String[] annotations) {
        ...
        if(count != annotations.length)
            throw new IllegalArgumentException("The number of features does not"
        ...
        for ( int i = 0; i < count; i++) {
            ...
            final Object obj = map.put((feature != null) ? feature : empty, // key, value
                                       (annot != null) ? annot : empty);
            if( obj != null )
                throw new DuplicateException("Feature "+obj+" has a duplicate!");
        }
    }
    
    /** returns the named gene's description based on the current capability
     * @param gene the feature whose description is to be gotten
    ...
    public String toString() {
        return "[FileAnnotation "+this.map.toString()+" genes/annotations "+capability+"]";
    }
    /** gets the name
