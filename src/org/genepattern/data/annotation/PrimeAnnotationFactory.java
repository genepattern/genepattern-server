...
public class PrimeAnnotationFactory {
    ...
    /** Creates a new instance of PrimeAnnotationFactory 
    ...
    /** this factory method creates an annotation factory from two arrays of strings
     * it is most likely to be created from a res or gct file
     * @param names the array of features
     * @param descriptions the corresponding array of descriptions
     * @return AnnotationFactory, the annotation factory that has the names and descriptions
     */
    public static final AnnotationFactory createAnnotationFactory(final String[] names, final String[] descriptions) {
        if(descriptions == null || descriptions.length == 0) {
            return EMPTY_ANNOTATION_FACTORY;
        }
        Annotator annot = add(new FileAnnotation(names, descriptions));
        final AnnotationFactory factory = new AnnotationFactory((Annotator[])annotators.toArray(new Annotator[size()]), annot);
        return factory;
    }
    /**
    ...
    private static final Annotator add(final Annotator annot) {
        ...
        if(index < 0) { // it's unique so add it and return it
            ...
            return annot;
        } else         // already have one like it so return the old one
    ...
    // fields
    private static final AnnotationFactory EMPTY_ANNOTATION_FACTORY = new AnnotationFactory(null, null);
