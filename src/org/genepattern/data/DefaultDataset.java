...
public class DefaultDataset extends AbstractDataset {
	 ...
	 *            the matrix object
	 * @param rows
	 *            the rows names panel
	 * @param columns
	 *            the columns names panel
	 */
	 ...
	 *            the column count
	 */
	public DefaultDataset(final String name, final float[] data,
			final NamesPanel rows, final NamesPanel columns) {
		super(name,
				new DefaultMatrix(rows.getCount(), columns.getCount(), data),
				false/* not mutable */, rows, columns);
	}
	...
	public DefaultDataset(final Dataset ds, final String name) {
		this(name, check(ds).getMatrix(),
				(NamesPanel) ds.getRowPanel().clone(), (NamesPanel) ds
						.getColumnPanel().clone());
	}
	 ...
	 */
	protected final void checkInit() { // does nothing all checking has been
									   // done
	} // and the class is immutable

	/**
	 * Creates a DefaultMatrix if the input matrix is mutable otherwise returns
	 ...
	 * 
	 * @param panel
	 *            The names panel to process
	 * @return NamesPanel
	 */
	protected NamesPanel processNamesPanel(final NamesPanel panel) {
		return (panel.isMutable()) ? new DefaultNamesPanel(
				(AbstractNamesPanel) panel) : panel;
	}
