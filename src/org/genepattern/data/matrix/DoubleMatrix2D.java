...
public class DoubleMatrix2D {
    ...
    * @param  rowNames                    The row names.
    * @param  columnNames                 The column names.
    * @param  rowNameToRowIndexMap        The row name to row index map.
    * @param  columnNameToColumnIndexMap  The column name to row index map.
    */
   private DoubleMatrix2D(Matrix matrix, String[] rowNames, String[] columnNames, ObjectIntMap rowNameToRowIndexMap, ObjectIntMap columnNameToColumnIndexMap) {
      this.matrix = matrix;
    ...
    * @see               #print(java.io.PrintStream, String,
   ...
   public void print(java.io.PrintStream ps, String delimiter, java.text.NumberFormat nf) {
      ...
      for(int i = 0, rows = getRowCount(); i < rows; i++) {
         ...
         ps.print(nf.format(matrix.get(i, columns - 1)));// don't print the delimmiter after the last column
      }
   }
   ...
   public DoubleMatrix2D transpose() {
      ...
      transpose.columnNameToColumnIndexMap = (ObjectIntMap) rowNameToRowIndexMap.clone();
      transpose.rowNameToRowIndexMap = (ObjectIntMap) columnNameToColumnIndexMap.clone();
    ...
    *
    * @param  rowNames                   The row names of the cells in the new
    *      matrix. To indicate that the new matrix should contain all rows,
    *      set this parameter to null.
    * @param  columnNames                The column names of the cells in the
    *      new matrix. To indicate that the new matrix should contain all
   ...
   public DoubleMatrix2D slice(String[] rowNames, String[] columnNames) {
      ...
      if(columnNames != null) {
         ...
         for(int i = 0, length = columnNames.length; i < length; i++) {
            columnIndices[i] = columnNameToColumnIndexMap.get(columnNames[i]);
         }
      }
    ...
    *
    * @param  rowIndices                 The rows of the cells in the new
    *      matrix. To indicate that the new matrix should contain all rows,
    *      set this parameter to null.
    * @param  columnIndices              The columns of the cells in the new
    *      matrix. To indicate that the new matrix should contain all columns,
    *      set this parameter to null.
    * @return                            the new matrix
    * @throws  IllegalArgumentException  if an index occcurs more than once.
   ...
   public DoubleMatrix2D slice(int[] rowIndices, int[] columnIndices) {
      ...
      for(int i = 0, length = rowIndices.length; i < length; i++) {
         ...
         if(d.rowNameToRowIndexMap.containsKey(d.rowNames[i])) {
            throw new IllegalArgumentException("Duplicate indices are not allowed. Row name: " + d.rowNames[i] + ", Row index: " + rowIndices[i]);
         }
      ...
      for(int j = 0, length = columnIndices.length; j < length; j++) {
         ...
         if(d.columnNameToColumnIndexMap.containsKey(d.columnNames[j])) {
            throw new IllegalArgumentException("Duplicate indices are not allowed. Column name: " + d.columnNames[j] + ", Column index: " + columnIndices[j]);
         }
   ...
   public DoubleMatrix2D times(DoubleMatrix2D B) {
      ...
      C.columnNameToColumnIndexMap = (ObjectIntMap) B.columnNameToColumnIndexMap.clone();
      C.rowNameToRowIndexMap = (ObjectIntMap) this.rowNameToRowIndexMap.clone();
      return C;
   }
   ...
   public DoubleMatrix2D times(double d) {
      ...
      C.columnNameToColumnIndexMap = (ObjectIntMap) this.columnNameToColumnIndexMap.clone();
      C.rowNameToRowIndexMap = (ObjectIntMap) this.rowNameToRowIndexMap.clone();
      return C;
   }
   ...
   public DoubleMatrix2D copy() {
      ...
      doubleMatrixCopy.columnNameToColumnIndexMap = (ObjectIntMap) this.columnNameToColumnIndexMap.clone();
      doubleMatrixCopy.rowNameToRowIndexMap = (ObjectIntMap) this.rowNameToRowIndexMap.clone();
   ...
   private void fillInColumns(int columnIndex) {
      for(int i = columnIndex, columns = getColumnCount(); i < columns; i++) {
         ...
         if(columnNameToColumnIndexMap.containsKey(name)) {
            throw new IllegalArgumentException("Duplicate names are not allowed:" + name);
         }
   ...
   public void setColumnName(int columnIndex, String name) {
      ...
      if(columnNameToColumnIndexMap.containsKey(name) && columnNameToColumnIndexMap.get(name) != columnIndex) {
    ...
    *
    * @param  rowIndex     The new rowName value
    * @param  name         The new rowName value
    */
   public void setRowName(int rowIndex, String name) {
      ...
      if(rowNameToRowIndexMap.containsKey(name) && rowNameToRowIndexMap.get(name) != rowIndex) {
    ...
    *  allowed. If the length of s is less than getRowCount(), the remaining
    *  row names will be filled in automatically
    *
    * @param  s  The list containing the row names
    */
   private void setRowNames(List s) {
      if(s.size() > getRowCount()) {
         throw new IllegalArgumentException("Invalid row names length. getRowCount():" + getRowCount() + " row names length:" + s.size());
      }
      for(int i = 0, size = s.size(); i < size; i++) {
         String rowName = (String) s.get(i);

         if(rowNameToRowIndexMap.containsKey(rowName) && rowNameToRowIndexMap.get(rowName) != i) {
            throw new IllegalArgumentException("Duplicate row names are not allowed:" + rowName);
         }
   ...
   private void setColumnNames(List s) {
      if(s.size() > getColumnCount()) {
         throw new IllegalArgumentException("Invalid column names length. getColumnCount():" + getColumnCount() + " column names length:" + s.size());
      }
      for(int i = 0; i < s.size(); i++) {
         String name = (String) s.get(i);
         if(columnNameToColumnIndexMap.containsKey(name) && columnNameToColumnIndexMap.get(name) != i) {
            throw new IllegalArgumentException("Duplicate column names are not allowed:" + name);
         }
         if(name == null) {
            throw new IllegalArgumentException("Null column names are not allowed.");
    ...
    *
    * @param  rowName  the row name.
    * @return          the row index, or -1 if the row name is not contained in
    *      this matrix
    */
    ...
    *
    * @param  columnName  the column name.
    * @return             the column index, or -1 if the column name is not
