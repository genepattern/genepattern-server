...
public class ClassVector {
	...
	public ClassVector(String[] x) {
		...
		for(int i = 0; i < x.length; i++) {
			...
			List indices = (List) this.classNumber2IndicesMap.get(classNumberInteger);
	...
	private ClassVector(Map classNumber2IndicesMap, int[] assignments, Map classNumber2LabelMap, int classCount) {
	...
	public ClassVector union(ClassVector classVector) {
		...
		for(int i = 0; i < size(); i++) {
			assignments[i] = "Class " + lookup[getAssignment(i)][classVector.getAssignment(i)];
		}
	...
	public ClassVector slice(int[] order) {
		...
		for(int i = 0, length = order.length; i < length; i++) {
			...
			List newIndices = (List) newClassNumber2IndicesMap.get(new Integer(newAssignments[i]));
			if(newIndices == null) {
				newIndices = new ArrayList();
				newClassNumber2IndicesMap.put(new Integer(newAssignments[i]), newIndices);
				numClasses++;
				newClassNumber2LabelMap.put(new Integer(newAssignments[i]), getClassName(this.assignments[order[i]]));
			}
			newIndices.add(new Integer(i));
		}
		return new ClassVector(newClassNumber2IndicesMap, newAssignments, newClassNumber2LabelMap, numClasses);
	}
   ...
   /**
	...
	public int[] getIndices(int classNumber) {
		List indices = (List) classNumber2IndicesMap.get(new Integer(classNumber));
		if(indices == null) {
			return new int[0];
		}
