 ...
 * @author kohm
 */
public class DefaultFeaturesetProperties extends AbstractObject implements
		FeaturesetProperties {

	/**
	 * Creates a new instance of DefaultFeaturesetProperties. Note the column
	 * classes if representing a primitive should use the class defined in the
	 * corresponding wrapper class TYPE variable (i.e. float should be
	 * represented by Float.TYPE not Float.class same goes for the other
	 * primitives - int should have class Integer.TYPE etc.)
	 */
	public DefaultFeaturesetProperties(final String name, final String model,
			final String[] column_names, final String[] column_descriptions,
			final String[] row_names, final String[] row_descs,
			final Class[] column_classes, final ArrayOArrays arrays,
			final Map attributes) {
		super(name, false);
		//        if( name == null )
		//            throw new NullPointerException("The name of this object cannot be
		// null!");
		//        this.name = name.trim();
		if (model == null)
		...
		if (column_classes == null)
			throw new NullPointerException(
					"column classes array cannot be null!");
		...
		if (check_cnt && column_names.length != cnt) {
			throw new ArrayIndexOutOfBoundsException(
					"Column names array wrong size " + column_names.length
							+ " != " + cnt);
		}

		if (check_cnt && column_descriptions != null
		...
		if (arrays.getArrayCount() != cnt) {
			throw new ArrayIndexOutOfBoundsException(
					"Number of Column arrays should be " + cnt + " not "
							+ arrays.getArrayCount());
		}
		this.column_descriptions = (ArrayUtils
				.arrayIsEmpty(column_descriptions)) ? null
				: column_descriptions;
		...
		this.attributes = (attributes != null) ? new java.util.HashMap(
				attributes) : new java.util.HashMap(1);
		...
		if (value != null) {
			try {
				...
				System.err
						.println("Couldn't parse the row_names_column value: "
								+ ex);
			}
		}
		if (r_names == null)
			r_names = (ArrayUtils.arrayIsEmpty(row_names)) ? null
					: (String[]) row_names.clone();
		...
		if (value != null) {
			try {
				...
				System.err
						.println("Couldn't parse the row_descriptions_column value: "
								+ ex);
			}
		}
		if (r_descrs == null)
			r_descrs = (ArrayUtils.arrayIsEmpty(row_descs)) ? null
					: (String[]) row_descs.clone();
		...
		//        if(row_descs != null && row_names != null && row_names.length !=
		// row_descs.length
	...
	/**
	 * Creates a new instance of DefaultFeaturesetProperties This has no body or
	 * main table of data.
	 */
	public DefaultFeaturesetProperties(final String name, final String model,
			final String[] col_names, final String[] col_descs,
			final String[] row_names, final String[] row_descs,
			final Map attributes) {
		this(name, model, col_names, col_descs, row_names, row_descs,
				new Class[0], new ArrayOArrays(0, new Class[0]), attributes);
	}
	 ...
	 * data model occurs. Not implemented since no TableModelListener objects
	 * are stored.
	 * 
	 * @param l
	 ...
	 * <code>rowIndex</code>. For columns that store primitives, a reused
	 * simple object will be returned. The SimpleObject will contain the value
	 * of the primitive that is stored at that row.
	 * 
	 * @param rowIndex
	 ...
	 * the data model occurs. Note there will be no changes to the data since
	 * this is an immutable data class thus no listeners will ever be notified
	 * of anything.
	 * 
	 * @param l
	 *            the TableModelListener
	...
	//    }
	/** dumps the data */
	public void dumpTo(final java.io.PrintStream out)
	...
	public String toString() {
		return "DefaultFeaturesetProperties " + getName() + " " + row_count
				+ "x" + column_count;
	}
	...
	public boolean equals(Object obj) {
		...
		return row_count == other.getRowCount()
				&& column_count == other.getColumnCount();
		//FIXME
		//&& column_classes.equals(other.column_classes) &&
		// column_names.equals(other.column_names)
	...
	/**
	 * sets the array values to those of the Row Descriptions if the specified
	 * array is null creates a new String[]
	 */
	...
	//fields
	/**
	 * each element is an array of some supported type i.e. int[] String[]
	 * boolean[] float[]
	 */
	...
	public class SimpleInteger extends AbstractSimpleObject {
		SimpleInteger(final int column) {
			super(column);
		}
	...
	public class SimpleBoolean extends AbstractSimpleObject {
		SimpleBoolean(final int column) {
			super(column);
		}
