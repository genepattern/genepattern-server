...
public class Range {
     ...
     */
      ...
      * For example createRanges(5, 0, 100, 1) will create 20 <CODE>Range</CODE>
      * objects 0-4, 5-9, etc al with a fuzz of 1.
      * min and max are Inclusive
      * @param spread the spread for each Range
      * @param min min value
      * @param max max value
      * @param fuzz the fuzz for each <CODE>Range</CODE>
      * @return Range[] the array of ranges
      */
    public static Range[] createRanges(final float spread, final float min, final float max, final float fuzz) {
     ...
     * @param min min value for first range
     * @param max max value for last range
     * @param fuzz the fuzz for all ranges
     * @return Range[] and array of ranges
     */
    public static Range[] createRanges(final int nranges, final float min, final float max, final float fuzz) {
     ...
     * @param density desnsity values to assign to each <CODE>Range</CODE>
     * @param values used to create each <CODE>Range</CODE>
     ...
     */
     public static Range[] createRanges(final int density, final float[] values) {
        ...
        for (int i=0; i < ranges.length; i+= density) {
            ...
            ranges[cnt++] = new Range(v.min(), v.max());
        }
        */
        ...
        ranges[ranges.length -1] = new Range(ranges[ranges.length-1].min(), values[values.length-1]);
