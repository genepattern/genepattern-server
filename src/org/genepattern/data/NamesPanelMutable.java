...
public class NamesPanelMutable extends AbstractNamesPanel implements Cloneable {
	...
	public NamesPanelMutable(final List names,
			final AnnotationFactory annotation) {
	...
	public NamesPanelMutable(final AbstractNamesPanel panel) {
		super((AnnotationFactory) panel.internalGetAnnotationFactory().clone(),
				true);
	...
	public NamesPanel createSubSet(final int[] indices) {
		...
		return new NamesPanelMutable(new_names,
	...
	public final void removeEntries(final int[] inds) {
		...
		// start with the largest indicies first so the smaller ones are always
		// valid
