 ...
 *
 * Notes:
 * - before adding methods might want to check if already impl in GVector
...
public class FloatVector implements org.genepattern.data.DataObjector, Serializable, FloatsSource {
    ...
    /**
      * Constructs a new Vector by copying length elements from the
    ...
    public float median() {
        ...
        Arrays.sort(v1);
        float median;
        if (XMath.isEven(elementCount)) median = elementData[elementCount/2];
        else median = elementData[ (elementCount-1) / 2];
        ...
        System.arraycopy(aVec, 0, v1, 0, aLen);
        Arrays.sort(v1);
        int ind = (aLen - 1) / 2;
        if (CMath.isEven(aLen))
     ...
     *
     {* @link http://davidmlane.com/hyperstat/A16252.html}
    ...
    public float max() {
        ...
        for (int i=0; i < elementCount; i++) {
            if (elementData[i] > max) max = elementData[i];
        }
        computeset.max = max;
        return max;
    }
    ...
    public float min() {
        ...
        for (int i=0; i < elementCount; i++) {
            if (elementData[i] < min) min = elementData[i];
        }
        computeset.min = min;
        return min;
    }
    ...
    /** 
     * These set the unbiased variance of the vector to var.
     *    (They do not set the sigma to var)
     * E.g. to change the variance from oldVar to newVar, multiply each component of
     *   the vector by sqrt(newVar / oldVar).
     */
    public void setRowNormalization(float mean, float var) {
        final float oldVar = var(false); // unbiased or biased ???
        if (oldVar == 0)
            throw new IllegalStateException("Cannot divide by a variance of zero!");
...
//        }
    ...
    public final void sum(final float [] v1data) {
        if (elementCount != v1data.length)
            throw new ArrayIndexOutOfBoundsException("this.size:"+elementCount+" != v2's size:"+v1data.length);
...
//        return buf.toString();
    ...
    public static String getSummaryStatistics(FloatVector[] vectors) {
        StringBuffer buf = new StringBuffer("Number of Vectors : ").append(vectors.length).append('\n');

        float min = Float.MAX_VALUE;
        float max = Float.MIN_VALUE;

        for (int i=0; i < vectors.length; i++) {
            if (min > vectors[i].min()) min = vectors[i].computeset.min;
            if (max < vectors[i].max()) max = vectors[i].computeset.max;
