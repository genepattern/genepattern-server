 ...
 * <code>Dataset</code> objects or creating new ones from old.
 *
 * @author  kohm
 */
public class Datasets {
     ...
     *
     * @param dataset is the original (this method could have been nonstatic and this would not be needed)
     * @param labels has the new order defined by the order of the array of labels
     * @param panels some lists of data that needs the same ordering
     */
...
//        final boolean use_rows = labels.usingDatasetRows();
    public static final Dataset getSortedDataset (final Dataset dataset,
    final String[] labs, final boolean use_rows, final java.util.List panels,
    final String name) {
        System.out.println("Original dataset:");
        datasetDump(dataset);
//        System.out.println("labels: use_rows="+use_rows+" row="+labels.row+" transposed="+labels.transposed);
...
//        final int[] rows_cols2include = genome.createIndices (labs);
        ...
        if(dataset.isMutable()) // FIXME should create a new DefaultDataset 
            AbstractReporter.getInstance().logWarning("Creating a DatasetView from Dataset "
                                    +dataset.getName()+" which is muttable!");
        DatasetView newdataset;
        if(use_rows) {
            newdataset = new DatasetView(dataset, name, rows_cols2include, null/*all cols*/);
//            newdataset.matchParentsColumns();
//            newdataset.useOnlyRows(rows_cols2include);
//            newdataset = CDataset.buildDatasetWithRows (dataset, rows_cols2include,
//            dataset.getName ()+labels.getLabelExtension());
        } else {
            newdataset = new DatasetView(dataset, name, null/**all rows*/, rows_cols2include);
//            newdataset.matchParentsRows();
//            newdataset.useOnlyColumns(rows_cols2include);
////            newdataset = CDataset.buildDatasetWithColumns (dataset, rows_cols2include,
////                                dataset.getName ()+labels.getLabelExtension());
        }
//        newdataset.setName(dataset.getName ()+labels.getLabelExtension());
    ...
    public static final void datasetDump(final Dataset dataset) {
        ...
        System.out.println("Matrix #rows="+matrix.getRowCount()+" #columns="+matrix.getColumnCount());
        String[] rows = dataset.getRowNames(), cols = dataset.getColumnNames();
        System.out.println("NamesPanels #rows="+rows.length+" #columns="+cols.length);
        System.out.println("is mutable="+dataset.isMutable()+" Id="+dataset.getId());
        System.out.println(" ***  *** ");
//        System.out.println(dataset.getInfo ());
    ...
    public static final int[] createIndices(final Dataset dataset, final String[] labels, final boolean use_rows) {
	...
	final int[] inds = new int[num_labels];
        final int rc_count = (use_rows ? dataset.getRowCount() : dataset.getColumnCount());
        // assumes that all names are unique
        for (int i = 0; i < rc_count; i++) {
            map.put((use_rows ? dataset.getRowName(i) : dataset.getColumnName(i)), i);
        }
...
//     * If the input dataset is mutable creates a new DefaultDataset otherwise creates
//     * an immutable DatasetView of the original
//     */
//    public static final Dataset buildDatasetWithColumns (final Dataset original, final int[] cols2include, final String new_name) {
