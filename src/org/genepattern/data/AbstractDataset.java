...
//        final String[] prop_names = new String [] {"name", "matrix", "rowNames", "colNames"};
...
//        
//        // tried to fix a problem in Java 1.4.0 where the java.util.Arrays$ArrayList
...
//    }
    ...
    final boolean mutable, final NamesPanel rows, final NamesPanel columns) {
        if(!mutable)
     ...
     * @param matrix matrix from the constructor
     * @return Matrix
     */
     ...
     * @param panel the names panel from the constructor
     * @return NamesPanel
     */
    ...
    /** checks the name and matrix for problems and
     * checks the row and column NamesPanels for problems too
     * can be overriden by subclasses
     * @param name the name to check
     * @param matrix the matrix to check
     * @param rows the rows names panel to check
     * @param columns the columns names panel to check
     */
    protected void check(String name, Matrix matrix, NamesPanel rows, NamesPanel columns) {
        if (name == null)
    ...
    protected void finalCheck(final int rows, final int cols) {
        ...
        if (getColumnCount() != cols)
            throw new IllegalArgumentException("Matrix: column " + getColumnCount()
                + " and colNames: " + cols + " do not match in size");
    }
    /** Must be called by all public instance methods of this abstract class. */
    abstract protected void checkInit();
        
    
    // these methods are only needed by subclasses for getting (and setting) private vars
    ...
    protected final void internalSetName(final String name) {
        if(is_mutable)
            ...
            throw new UnsupportedOperationException("Cannot set the name! Class is not mutable!");
    }
    ...
    protected final void internalSetMatrix(final Matrix matrix) {
        if(is_mutable)
            ...
            throw new UnsupportedOperationException("Cannot set the matrix! Class is not mutable!");
    }
    /** sets the columns NamesPanel
     * @param new_cols the new columns
     */
    protected final void internalSetColumns(final NamesPanel new_cols) {
        if(is_mutable)
            this.columns = new_cols;
        else
            throw new UnsupportedOperationException("Cannot set the column names! Class is not mutable!");
    }
    /** sets the columns NamesPanel
    ...
    protected final void internalSetRows(final NamesPanel new_rows) {
        if(is_mutable)
            ...
            throw new UnsupportedOperationException("Cannot set the rows names! Class is not mutable!");
    }
    ...
    //gets the column panel, which is where the column names, descriptions/annotation are...();
    public NamesPanel getColumnPanel() {
        checkInit();
        return columns;
    }
    
    // gets the row panel, which is where the row names, descriptions/annotation are...
    ...
    public boolean equals(Object obj) {
        ...
        return ( this.getColumnPanel().equals(dataset.getColumnPanel())
                && this.getRowPanel().equals(dataset.getRowPanel())
                && this.getMatrix().equals(dataset.getMatrix()) );
    }
    /**
    ...
    private String name;
    /**
     * false means that this classe's variables cannot have their values
     * changed once set in the constructor
     */
