...
public class DatasetView extends org.genepattern.data.AbstractDataset {
	...
	public DatasetView(Dataset dataset, String name, int[] row_index,
			int[] col_index) {
		this(dataset, name, dataset.getMatrix(), false, row_index, col_index);
	}

	/**
	 * the master constructor that should be used by all other constructors of
	 * this class and its' subclasses
	 */
	protected DatasetView(final Dataset dataset, final String name,
			final Matrix matrix, final boolean mutable, int[] row_index,
			int[] col_index) {
		// perhaps a helper constructor class would simplify this?!?
		super(createName(name, dataset), matrix, mutable, createNamesPanel(
				dataset.getRowPanel(), row_index = getIndices(row_index,
						dataset.getRowCount(), true)), createNamesPanel(dataset
				.getColumnPanel(), col_index = getIndices(col_index, dataset
				.getColumnCount(), false)));
		// Note may have to preprocess the matrix using static helper method see
	...
	/** helper */
	private static final String createName(final String name,
	...
	/** helper */
	private static final NamesPanel createNamesPanel(final NamesPanel old,
			final int[] indices) {
		return (indices == null) ? old : old.createSubSet(indices);
	}

	/** helper for constructor */
	private static final int[] getIndices(final int[] indices, final int max,
			final boolean is_row) {
		if (indices != null) {
			final int cnt = indices.length;
			if (cnt == 0)
				throw new IllegalArgumentException("zero length "
						+ (is_row ? "row" : "column") + " index array");
	...
	protected static final void checkIndicies(final int len,
			final int[] indices, final int max_parent, final boolean is_row) {
		for (int i = len - 1; i >= 0; i--) { //rev. loop
			final int val = indices[i];
			if (val >= max_parent || val < 0) {
				final String rc = (is_row ? "row" : "column");
				throw new IndexOutOfBoundsException("Index array: number of "
						+ rc + "s in parent (" + max_parent
						+ ")  index is out of bounds - " + rc + "[" + i + "]="
						+ val);
			}
		}
	}
	...
	public Object clone() {
		final DatasetView dataset = new DatasetView(this.parent, this.getName()
				+ "_clone", this.row_index, this.col_index);
		return dataset;
	}
	 ...
	 */
	protected final static String[] createMappedStringArray(
			final String[] names, final int[] indices) {
	...
	protected final void internalSetRowIndices(final int[] indices) {
		if (isMutable())
			...
			throw new UnsupportedOperationException(
					"Not mutable! Cannot set the row indices.");
	}
	...
	/**
	 * generally should return immuted matrix the Matrix is where the view
