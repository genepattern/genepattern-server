 ...
 * 
 * Object is auto assigned a unique programmatic name on construction. The alg
 * used for this is edu.mit.genome.util.UUID and the object name is guranteed to
 * be uniquie in time and space. This is final so, program name of an object
 * never changes. The display name is settable/changeable.
 * 
 * 
 * Sub-classes must implement: always dynamically (or at least transiently) make
 * the summary so as not to affect any serialization.
 * 
...
// theres a bit of redundance here - Dataset for instance extends AbstractObject
// and implements
// ProjectObjetct which in turn extends PersistentObject
public abstract class AbstractObject implements DataObjector {//implements
															  // PersistentObject
															  // {
	/**
	 * Class constructor. Makes a new object with specified name and not
	 * mutable.
	 * 
	 * @param name
	 *            the name of this
	...
	/**
	 * Class constructor. Makes a new object with specified name and mutability.
	 * 
	 * @param name
	 *            the name of this
	 * @param ismutable
	 *            is this a mutable data object
	 */
	...
	/**
	 * Class constructor. Makes a new object with specified name.
	 * 
	 * @param id
	 *            the id
	 * @param name
	 ...
	 *            is this a mutable data object
	 */
	protected AbstractObject(final Id id, final String name,
		...
		if (name == null)
			throw new NullPointerException(
					"Parameter display name cannot be null");
	...
	/**
	 * returns the index of the last bad character (not suitable for File names
	 * etc) or -1 if none
	 * 
	 * @param name
	 *            a name
	 ...
	 * 
	 * @param prop
	 *            the featureset properties object
	 */
	protected final void deserializeFromProperties(
			final FeaturesetProperties prop) {
		...
		try {
			...
			for (int i = 0; i < num; i++) {
				...
				if (type.isPrimitive() && value instanceof String) {
					final AbstractPrimitiveParser parser = (AbstractPrimitiveParser) TYPE_PRIMITIVE_PARSER
							.get(type);
					if (parser == null)
						throw new UnsupportedOperationException(
								"primitive type parser for " + type
										+ " is not yet implemented!");
					parser.parseToVar(field, (String) value, this);
				} else {
					final AbstractObjectParser ob_parser = (AbstractObjectParser) type_objectsetter
							.get(type);
					if (ob_parser == null) {
			...
			throw new IllegalArgumentException(
					"Exception occured while accessing the fields\n" + ex);
		}
	}
	...
	private void parseToVar(final Class type, final Field field,
			final Object value) throws IllegalAccessException {
		if (value instanceof String) {
			//if( value.getClass() == String.class ) {
			final AbstractPrimitiveParser parser = (AbstractPrimitiveParser) TYPE_PRIMITIVE_ARRAY_PARSER
					.get(type);
			if (parser == null)
				throw new UnsupportedOperationException(
						"primitive array type parser for " + type
								+ " is not yet implemented!");
			parser.parseToVar(field, (String) value, this);
		} else if (value instanceof String[]) {
			final AbstractArrayParser parser = (AbstractArrayParser) TYPE_PRIMITIVE_ARRAY_PARSER
					.get(type);
			if (parser == null)
				throw new UnsupportedOperationException(
						"primitive array type parser for " + type
			...
			System.out.println("Not implemented yet: converting a "
					+ value.getClass() + " to a type=" + type + " field="
					+ field + " value=" + value);
	 ...
	 */
	public Id getId() {
		//checkInit(); // uncloometn me if the ko serialization idea didnt oan
		// out IMP IMp IMp
		return id;
	}
	...
	public String getSummary() {
		if (log.isDebugEnabled())
			log.debug("Using summary from AbstractObject");
		//checkInit(); // intentionally dont call! some summary helps when
		// debugging
	...
	/**
	 * Convenience method (for developers) to pretty print out the contents of
	 * the IData. Mostly for use when debugging. <br>
	 ...
	 *            the array to copy
	 * @param array
	 *            the array to copy to or null
	 * @return String[]
	 */
	public static final String[] arrayCopy(final String[] source,
			final String[] array) {
	...
	static {
		...
		AbstractPrimitiveParser parser = new AbstractPrimitiveParser() {
			protected void parseToVar(final Field field, final String value,
					final Object owner) throws IllegalAccessException {
		...
		parser = new AbstractArrayParser() {
			/**
			 * implementations should create a primitive array of the specified
			 * size
			 */
			 ...
			 */
			protected void setValue(final String token, final Object array,
					final int i) {
		...
		//             * value at the indicated index
		//             */
		//            protected void setValue(final String token, final Object array, final
		// int i) {
		//                ((int[])array)[i] = Integer.parseInt(token.trim());
		//            }
		//        };
		...
		TYPE_PRIMITIVE_ARRAY_PARSER = java.util.Collections
				.unmodifiableMap(clss_array);
	...
	abstract static class AbstractPrimitiveParser {
		abstract protected void parseToVar(final Field field,
				final String string_value, final Object owner)
				throws IllegalAccessException;
		//abstract protected void parseToVar(final Field field, final String[]
		// string_value, final Object owner) throws IllegalAccessException;
	}
	...
	protected abstract static class AbstractObjectParser {
		 ...
		 * @param owner
		 *            the object/instance of the field
		 * @throws IllegalAccessException
		 *             if there is a problem accessing the specified field
		 */
		abstract protected void setVar(final Field field, final Object value,
				final Object owner) throws IllegalAccessException;
	}

	/**
	 * Defines a method to parse a string to an array of values and set the
	 * variable to the array
	 */
	protected abstract static class AbstractArrayParser extends
			AbstractPrimitiveParser {
		/**
		 ...
		 *            the variable to set
		 * @param value
		 *            the text that represents an array of values
		 * @param owner
		 *            the instance whos field should be set
		 * @throws IllegalAccessException
		 *             if it is not possible to set the field
		 */
		protected void parseToVar(final Field field, final String value,
				final Object owner) throws IllegalAccessException {
		...
		/**
		 * Parses the individual string values into their true types and
		 * populates the array elements and sets the field to the resulting
		 * array.
		 * 
		 * @param field
		 *            the variable to set
		 * @param string_values
		 *            the array of strings that represents an array of possibly
		 *            other values
		 * @param owner
		 *            the instance whos field should be set
		 * @throws IllegalAccessException
		 *             if it is not possible to set the field
		 */
		protected void parseToVar(final Field field,
				final String[] string_values, final Object owner)
				throws IllegalAccessException {
		...
		/**
		 * implementations should parse the value to some type (a number, for
		 * example) and set the indicated index in the array
		 * 
		 * @param token
		 *            the text to parse to some other type
		 * @param array
		 *            the array to set the element at a particular index
		 * @param i
		 *            the index in the array
		 */
		abstract protected void setValue(final String token,
				final Object array, final int i);
	}
