 ...
 * See MatrixMutable for a subclass that is mutable.
 * Note that all variables are private.  There are protected getInternalXXX(...)
 * that are for use by subclasses only.  Classes within this package should
 * not be using them.
 *
...
//        String [] prop_names = new String[] {"rowCount", "columnCount", "elementData"};
//        encoder.setPersistenceDelegate(DefaultMatrix.class, 
//                        new java.beans.DefaultPersistenceDelegate(prop_names));
//    }
    
    /**
     * Constructs an rowCount by columnCount all zero matrix. (as change)
     * Note that even though row and column numbering begins with
     * zero, rowCount and columnCount will be one larger than the maximum
     * possible matrix index values.
     * @param rowCount number of rows in this matrix.
     * @param columnCount number of columns in this matrix.
     */
    protected DefaultMatrix(final int nrows, final int ncols, final boolean mutable) {
        this(mutable);
        if (nrows < 0)
    ...
    /**
     * Constructs an RowCnt by ColCnt matrix initialized to the values
     * in the matrix array.  The array values are copied in one row at
     * a time in row major fashion.  The array should be at least
     * RowCnt*ColCnt in length.
     * Note that even though row and column numbering begins with
     * zero, RowCnt and ColCnt will be one larger than the maximum
     * possible matrix index values.
     * @param rowCount number of rows in this matrix.
     * @param columnCount number of columns in this matrix.
    ...
    protected DefaultMatrix(int nrows, int ncols, float[] matrix, boolean mutable) {
    ...
    /**
     * data is not shared
     * matrices must be of same number of columns
     * (nrows can be arb - thats the point!)
     */
     ...
     *
     * as added: if shared values, then this matrix shares the elements with the specified matrix
    ...
    /** parses the string to produce a 2D array of data assumes data is from the output of dump()
     * puts the row count and column count in the int array 
     * row_col_cnt[0] is the row count
     * row_col_cnt[1] is the column count
     *
     *@throws NumberFormatException  if there is a problem with converting 
     *        the tokens to floats
     */
//    public static final float[][] parseString(final String data, final int[] row_col_cnt) throws NumberFormatException {
//       final ReusableStringTokenizer r_tok = new ReusableStringTokenizer();
//       final ReusableStringTokenizer val_tok = new ReusableStringTokenizer(); // whitespace including tab
...
//       return array;
//    }
    public static final float[] parseString(final String data_string, final int[] row_col_cnt) throws NumberFormatException {
...
//        //final String data = "[  [359.8975	301.44925	371.68536	296.83972	284.50644	196.84131	383.2755	400.70026	324.9408	482.81595	270.24112	268.4501	638.277	280.66357	236.82712	360.41672	297.84247	310.44394	210.5185	378.10822	397.7781	270.57297	479.28885	293.8416	327.8654	345.71387	247.01103	314.5929	224.64565	472.78412	384.7093	321.7087	364.28226	253.39578	410.72562	296.64578	373.73352	423.47232]  [13474.908	14258.262	14715.771	13695.667	13647.065	13979.262	13960.111	14615.777	14289.196	14841.32	13659.738	13796.405	15352.412	14118.692	13648.837	14420.393	14274.621	13959.17	13757.032	14922.111	14389.235	13979.908	14931.529	14548.869	14508.117	14459.163	13722.745	14503.085	13729.333	14617.974	14565.457	14536.078	14758.634	14378.568	14761.889	14366.202	14495.026	14384.444] ]";
//        final String data = "   [  [286.2857	226.95763	312.97974	201.29184	194.98112	187.00543	345.4358	366.5634	246.33171	415.59534	202.4267	216.21548	570.5884	275.50323	232.04291	308.34488	319.07904	261.2104	90.92851	390.09662	327.3447	238.77219	422.84995	303.76337	287.00333	278.3014	192.82605	305.4419	187.56792	507.79538	395.29858	332.05597	373.69162	215.31554	402.3468	282.17657	364.83157	431.0131]  [12352.847	12973.49	13482.676	12335.109	12385.29	12693.725	12848.918	13464.493	13046.984	13690.954	12494.255	12570.944	14318.386	12825.974	12395.598	13147.79	13050.247	12742.468	12415.714	13696.25	13173.715	12765.154	13863.793	13370.125	13324.183	13184.99	12455.911	13233.171	12358.426	13528.876	13444.697	13248.735	13569.03	13113.938	13663.237	13187.144	13354.49	13244.352] ]";
...
//    }
    ...
    protected final float[] getInternalElementData() {
        if(!subclass_can_change)
            throw new IllegalAccessError("Cannot get access to the elementData. Matrix is immutable!");
        return elementData;
    }
    /** sets the elementData */
    protected final void setInternalElementData(final float[] elementData) {
        if(!subclass_can_change)
            throw new IllegalAccessError("Cannot change the elementData to another object. Matrix is immutable!");
        this.elementData = elementData;
    }
    /** sets the number of rows */
    protected final void setInternalRowCount(final int rowcnt) {
        if(!subclass_can_change)
            throw new IllegalAccessError("Cannot set rowCount value. Matrix is immutable! ");
        this.rowCount = rowcnt;
    }
    /** sets the number of columns */
    protected final void setInternalColCount(final int colcnt) {
        if(!subclass_can_change)
            throw new IllegalAccessError("Cannot set columnCount value. Matrix is immutable!");
        this.columnCount = colcnt;
    }
    /** returns the index into the data array given the row and column index (zero based) */
    protected final int getIndex(final int row, final int column) {
        return getIndex(row, column, rowCount, columnCount);
    }
    /** returns the index into the data array given the row and column index (zero based) and row and column counts*/
    public static final int getIndex(final int row, final int column, final int row_cnt, final int col_cnt) {
    ...
    public final Matrix createSubMatrix(int[] rows, int[] cols) {
        ...
        final DefaultMatrix matrix = new DefaultMatrix(num_rows, num_cols, false);
    ...
    /**
     * gets the value at the specified index into the elementData array
     * This should only be used by subclasses not intended for general use.
     */
    ...
    public final FloatVector getRow(final int row, final FloatVector vector) {
        ...
        if (vector.getSize() < columnCount)
            ...
            //throw new ArrayIndexOutOfBoundsException("vector size:"+vector.getSize()+" smaller than matrix's columnCount:"+columnCount);

        // if may use package friendly accessibility, would do;
        System.arraycopy(elementData, row*columnCount, vector.elementData, 0, columnCount);
        return vector;
    }
    ...
    public final  FloatVector getRowv(final int row) {
        if (rowCount <= row) throw new ArrayIndexOutOfBoundsException("row:"+row+" > matrix's rowCount:"+rowCount);
        if (row < 0) throw new ArrayIndexOutOfBoundsException("row:"+row+" < 0");
    ...
    public final  FloatVector getColumnv(final int col) {
        ...
        System.arraycopy(elementData, col*rowCount ,vector.elementData, 0, rowCount);
    ...
    public final Matrix get(final MatrixMutable m1) {
        // need error check.
        final int m1_row_cnt = m1.getRowCount(), m1_col_cnt = m1.getColumnCount(); 
        if (m1_row_cnt < rowCount || m1_col_cnt < columnCount)
            throw new IllegalArgumentException(
                "m1 matrix is smaller than this matrix.");

        if (m1_col_cnt == columnCount) {
            System.arraycopy(elementData, 0, m1.getElementData(), 0, rowCount*columnCount);
        } else {
            for (int i = 0; i < rowCount; i++) {
                System.arraycopy(elementData, i*columnCount , m1.getElementData(), i*m1_col_cnt, columnCount);
            }
        }
        return m1;
    }
    /** dumps the contents of the values to a Sting on a single line
     * (no line seperators) */
    public String dump() {
        return dump("");
    }
    ...
    /**
     * Returns a string that contains the values of this Matrix in a 
     * human readable form.
...
//    }
    ...
    public final  boolean equals(final Matrix m1) {
        ...
        if( m1 instanceof DefaultMatrix )
            return java.util.Arrays.equals(elementData, ((DefaultMatrix)m1).elementData);
        else { // this case could violate the transitive property of equality
            // is equavalent to being equals(Matrix) and the "if" part would be equals(DefaultMatrix)
...
//    }
    ...
    public final  void bin(final Range[] ranges) {
        for(int i = 0; i < rowCount*columnCount; i++) {
            for (int j=0; j < ranges.length; j++) {
                if (ranges[j].isMember(elementData[i])) ranges[j].increment();
            }
        }
    }
    ...
    public final  void bin(final java.util.List ranges) {
        for(int i = 0; i < rowCount*columnCount; i++) {
            for (int j=0; j < ranges.size() ; j++) {
                ...
                if (range.isMember(elementData[i])) range.increment();
            }
        }
    }
    ...
    // equal the numb of elements in the matrix.
     public final  Matrix discretize(final Range[] ranges) {
        ...
        DefaultMatrix newmatrix = new DefaultMatrix(getRowCount(), getColumnCount(), false);
        for(int i = 0; i < rowCount*columnCount; i++) {
            for (int j=0; j < ranges.length; j++) {
    ...
    /** IMP IMP: if any fields are added to this class, make sure to update the constructors and the set(Matrix method */

    /** The data of the Matrix.(1-D array. The (i,j) element is stored in elementData[i*columnCount + j]) */
    ...
    protected static final transient XLogger log = XLogger.getLogger(Matrix.class);
