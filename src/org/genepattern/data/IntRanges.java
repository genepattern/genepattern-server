...
public class IntRanges {
	...
	/**
	 * Creates a new instance of IntRanges that will be condensed into fewer
	 * ones
	 */
	public IntRanges(final RangeInt[] ranges) {
		...
		do { // repeat a few times until no more mergers
			old_len = list.size();
			for (int j = list.size() - 1, i = j - 1; i >= 0; i--, j--) {// rev
																		// loop
	...
	public static final IntRanges parse(final String string) {
		...
		for (int i = 0; commaTok.hasMoreTokens(); i++) {
			final StringTokenizer dashTok = new StringTokenizer(commaTok
					.nextToken().trim(), "-");
			...
			if (num_tokes == 2) {
				...
				throw new IllegalArgumentException(
						"Wrong format in the number element " + i + " ("
								+ dashTok.nextToken() + ")");
			}
		}
		//        } catch (NumberFormatException ex) {
		//            // rethrow a Warning that can display a Warning message so the user
		//            // can try again (same thing for above:
		// IllegalArgumentException->Warning)
		//        }
		return new IntRanges((RangeInt[]) ranges.toArray(new RangeInt[ranges
				.size()]));
	}

	/**
	 * creates the complement of the given min-max range from this IntRanges For
	 * example: if this instance defines a range of (10-20) and the min & max
	 * are 0, 50; the complement would be a new IntRanges (0-9, 21-50)
	 * 
	 * Note that the specified IntRanges has to be contained within the min and
	 * max values. Thus if this defines a range (45-55) and the min max values
	 * are 0, 50; then the complement would produce an error because the max
	 * value is 55 which is greater than this range's max value of 50.
	 * 
	 * Result is undefined for when the range is a single IntRanges that spans
	 * the minimum and maximum values specified. for example this IntRanges has
	 * only one RangeInt that defines the range 0-50 and the parameters are also
	...
	public IntRanges createComplement(final int minimum, final int maximum) {
		...
		//    noop();
		return new IntRanges((RangeInt[]) list
				.toArray(new RangeInt[list.size()]));
	}
	...
	//    /** gets the RangeInt iterator that will return all RangeInt in sort
	// order */
	//    public final RangeIterator getRangeIterator() {
	//        return new RangeIterator(ranges);
	//    }
	/**
	 * gets the RangeInt iterator that will return all RangeInt in reverse sort
	 * order
	 */
