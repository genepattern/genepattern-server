...
//    }
     ...
     * Class constructor
     */
    public DatasetMutable(final String name, final Matrix matrix, final NamesPanel rows, final NamesPanel columns) {
        super(name, matrix, true/*is mutable*/, rows, columns);
        // note no finalCheck(); since the matrix and row and column names can be 
        // changed
    }
//    /**
//     * Class constructor
//     */
//    public DatasetMutable(final String name, final Matrix matrix, final String[] row_names, final String[] col_names) {
...
//    }
    ...
    public DatasetMutable(Dataset ds) {
        this(check(ds).getName(), ds.getMatrix(), ds.getRowPanel(), ds.getColumnPanel());
    }

//    /** processes the panel to make it mutable */
//    protected static final NamesPanelMutable processPanel(NamesPanel panel) {
//        return new NamesPanelMutable(panel.getListOfNames(), ((AbstractNamesPanel)panel).internalGetAnnotationFactory());
//    }
    
    /** creates a clone of this */
    public Object clone() {
        final DatasetMutable dataset = new DatasetMutable(getName()+"_clone", internalGetMatrix(), getRowPanel(), getColumnPanel());
        return dataset;
    }
    ...
    protected Matrix processMatrix(Matrix matrix) {
        if(matrix != null) // could keep the old one if it was immutable until
            return new MatrixMutable(matrix); // some operation would cause a change
        else
            return null;
    }
...
//    }
    ...
    // helper methods
    /** test if the data is consistent i.e. matrix.getRowCount() == rowNames.length etc. */
    public boolean isDataConsistant() {
        try {
            check(internalGetName(), internalGetMatrix(), getRowPanel(), getColumnPanel());
