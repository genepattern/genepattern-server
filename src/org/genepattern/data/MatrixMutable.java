...
public class MatrixMutable extends DefaultMatrix {
     ...
     * elementData[i*getColumnCount() + j])
     * Note this is just a copy of the super's variable done for convience
     */
    protected float elementData[];
    
    private static final transient XLogger log = XLogger.getLogger(MatrixMutable.class);

    /**
     * Class constructor
     * Must call set() after this form of the constructor.
     */
    ...
    /**
     * Constructs an getRowCount() by getColumnCount() all zero matrix. (as change)
     * Note that even though row and column numbering begins with
     * zero, getRowCount() and getColumnCount() will be one larger than the maximum
     * possible matrix index values.
     * @param getRowCount() number of rows in this matrix.
     * @param getColumnCount() number of columns in this matrix.
     */
     ...
     *
     * @param getRowCount() number of desired rows in this matrix
     * @param getColumnCount() number of desired columns in this matrix
     */
    public final void setSize(final int nrows, final int ncols)  {
        ...
        if (oldColCnt == ncols) {
            ...
            for (int i = 0; i < oldRowCnt; i++)
                System.arraycopy(oldData, i*oldColCnt, newData, i*ncols, oldColCnt);
        }
        setData(newData);
    }
    /**
    ...
    public final void removeRows(final IntRanges ranges) {
        // reverse loop is be less work for removeRow(int) because it removes later elements first
    ...
    public final void removeRow(final int row) {
        ...
        if(row + 1 < row_cnt) { // not last row
            ...
            //System.out.println("removing row "+row+" dim(r,c)=("+row_cnt+", "+col_cnt+") start="+start+" start next row="+end+" len="+len);
...
//    }
    ...
    public final void removeColumns(final IntRanges ranges) {
        // reverse loop is less work for removeColumns(RangeInt) because it removes later elements first
        for(IntRanges.ReverseRangeIterator iter = ranges.getReverseRangeIterator(); iter.hasMore(); ) { 
            removeColumns(iter.next());
        }
    }
    /** removes the columns defined by the RangeInt without creating another array */
    public final void removeColumns(final RangeInt range) {
        ...
        for(int r = row_cnt - 1; r >= 0; r--) { // rev loop
            ...
            //System.out.println("removing columns "+range+" start="+start+" end="+end+" len="+len);
            System.arraycopy(elementData, end, elementData, start, len);
        }
     ...
     * The lengths of all the FloatVectors should be equal to the number of columns
     * since each FloatVector is assumed to be a row.
     */
    ...
    public final void set(Matrix m1, boolean sharedvalues ) {
        ...
        if (m1.getRowCount() < getRowCount() || m1.getColumnCount() < getColumnCount())
...
//        }
//        else {
//            m1.get(this);//System.arraycopy(m1.elementData, 0, elementData, 0, getRowCount()*getColumnCount());
//        }
    ...
    public final void setElement(int row, int column, float value) {
        if (getRowCount() <= row)
            throw new ArrayIndexOutOfBoundsException("row:"+row+" > matrix's getRowCount():"+getRowCount());
        if (row < 0)
            throw new ArrayIndexOutOfBoundsException("row:"+row+" < 0");
        if (getColumnCount() <= column)
            throw new ArrayIndexOutOfBoundsException("column:"+column+" > matrix's getColumnCount():"+getColumnCount());
        if (column < 0)
    ...
    public final void setRow(int row,  float array[]) {
        ...
        if (array.length < col_cnt)
            throw new ArrayIndexOutOfBoundsException("array length:"+array.length+" < matrix's getColumnCount()="+getColumnCount());
    ...
    public final void setRow(final int row,  final FloatVector vector) {
        ...
        if (vecSize < col_cnt)
            throw new ArrayIndexOutOfBoundsException("vector's size:"+vecSize+" < matrix's getColumnCount()="+getColumnCount());
            
        System.arraycopy(vector.elementData, 0, elementData, row * col_cnt, col_cnt);
//        for (int i = 0; i < getColumnCount(); i++) {
//            elementData[row*getColumnCount() + i] = vector.getElement(i);
//            // if may use package friendly accessibility, would do;
//            // System.arraycopy(vector.elementData, 0, elementData, row*getColumnCount(), getColumnCount());
//        }
    }
    ...
    public final void setColumn(int col,  float array[]) {
        ...
        if (getColumnCount() <= col)
            throw new ArrayIndexOutOfBoundsException("col:"+col+" > matrix's getColumnCount()="+getColumnCount());
        if (col < 0)
            throw new ArrayIndexOutOfBoundsException("col:"+col+" < 0");

        if (array.length < getRowCount())
            throw new ArrayIndexOutOfBoundsException("array length:"+array.length+" < matrix's getRowCount():"+getRowCount());
    ...
    public final void setColumn(int col, FloatVector vector) {
        ...
        if (vecSize < getRowCount())
            throw new ArrayIndexOutOfBoundsException("vector size:"+vecSize+" < matrix's getRowCount()="+getRowCount());
        for (int i = 0; i < getRowCount(); i++)
    ...
    public final void mulTransposeRight(Matrix m1, Matrix m2) {
        if (m1.getColumnCount() != m2.getColumnCount() || getRowCount() != m1.getRowCount() || getColumnCount() != m2.getRowCount())
            throw new ArrayIndexOutOfBoundsException("matrices mismatch");
        final int row_cnt = getRowCount(), col_cnt = getColumnCount();  
        final int m2_col_cnt = m2.getColumnCount(), m1_col_cnt = m1.getColumnCount();  
        
        for (int r = 0; r < row_cnt; r++) {
            for (int c = 0; c < col_cnt; c++) {
                float sum = (float)0.0;
                for (int k = 0; k < m1_col_cnt; k++)
                    sum += m1.getElementAtIndex(r * m1_col_cnt + k) * m2.getElementAtIndex(c * m2_col_cnt + k);
    ...
    public final void transpose() {
        ...
        for(int r = 0; r < row_cnt; r++) {
            for(int c = 0; c < col_cnt; c++) {
                ...
                //final int index_a = getIndex(r, c), index_b = getIndex(c, r); // cannot do it this way
    ...
    public final void setScale(final float scale) {
        ...
        final int min = getRowCount() < getColumnCount() ? getRowCount() : getColumnCount();
        for (int i = 0; i < min; i++)
    ...
    protected final void swapRows(int i, int j) {
        ...
        for (int k = 0; k < getColumnCount(); k++) {
            ...
            elementData[i*getColumnCount() + k] = elementData[j*getColumnCount() + k];
    ...
    public final void mul(Matrix m1, Matrix m2) {
        ...
        // for alias-safety, decided to new double [getColumnCount()*getRowCount()].
        // Is there any good way to avoid this big new ?
        if( getRowCount() != m1.getRowCount() )
            throw new ArrayIndexOutOfBoundsException(
                "getRowCount():" + getRowCount() + " != m1.getRowCount():" + m1.getRowCount());

        if( getColumnCount() != m2.getColumnCount() )
            throw new ArrayIndexOutOfBoundsException(
                "getColumnCount():" + getColumnCount() + " != m2.getColumnCount():" + m2.getColumnCount());

        if( m1.getColumnCount() != m2.getRowCount() )
            throw new ArrayIndexOutOfBoundsException(
                "m1.getColumnCount():" + m1.getColumnCount() + " != m2.getRowCount():" + m2.getRowCount());
        ...
        for(int i = 0; i < getRowCount(); i++) {
            for(int j = 0; j < getColumnCount(); j++) {
                ...
                for(int k = 0; k < m1.getColumnCount(); k++)
                    sum += m1.getElementAtIndex(i*m1.getColumnCount() + k) * m2.getElementAtIndex(k*m2.getColumnCount() + j);
    ...
    public final void mul(FloatVector v1, FloatVector v2) {
        if (getRowCount() < v1.getSize())
            throw new IllegalArgumentException(
                "getRowCount():"+getRowCount()+" < v1.getSize():" +v1.getSize());
        if (getColumnCount() < v2.getSize())
            throw new IllegalArgumentException(
                "getColumnCount():"+getColumnCount()+" < v2.getSize():" +v2.getSize());

        for (int i = 0; i < getRowCount(); i++)
            for (int j = 0; j < getColumnCount(); j++)
                elementData[i*getColumnCount() + j] = v1.getElement(i)*v2.getElement(j);
    }
    ...
    public final void add(Matrix m1) {
        if (getRowCount() != m1.getRowCount() || getColumnCount() != m1.getColumnCount())
            throw new IllegalArgumentException("this:(" +getRowCount()+"x"+getColumnCount()+") != m1:("+m1.getRowCount()+"x"+m1.getColumnCount()+").");
    ...
    public final void add(Matrix m1, Matrix m2) {
        ...
        if (getRowCount() != m1.getRowCount() || getColumnCount() != m1.getColumnCount())
            throw new IllegalArgumentException("this:(" +getRowCount()+"x"+getColumnCount()+") != m1:("+m1.getRowCount()+"x"+m1.getColumnCount()+").");
        if (getRowCount() != m2.getRowCount() || getColumnCount() != m2.getColumnCount())
            throw new IllegalArgumentException("this:(" +getRowCount()+"x"+getColumnCount()+") != m2:("+m2.getRowCount()+"x"+m2.getColumnCount()+").");
    ...
    public final void identityMinus() {
        negate();
        int min = getRowCount() < getColumnCount() ? getRowCount() : getColumnCount();
        for (int i = 0; i < min; i++)
