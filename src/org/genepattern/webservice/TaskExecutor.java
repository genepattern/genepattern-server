...
public abstract class TaskExecutor {
	...
	public final static String[] PARAM_INFO_OPTIONAL = {"optional", PARAM_INFO_CHECKBOX};
	public final static String[] PARAM_INFO_DEFAULT_VALUE = {"default_value", PARAM_INFO_STRING};
	...
	/**
	 *@param  taskInfo            Description of the Parameter
	 *@param  paramName2ValueMap  Description of the Parameter
	 *@return                     the new command line
	 *@exception  IOException     Description of the Exception
	 */
	protected String[] doCommandLineSubstitutions(TaskInfo taskInfo, Map paramName2ValueMap) throws IOException {
		...
		// replace variables in the command line from System.properties and the substitutions HashMap
		for(c = 0; c < cmd.length; c++) {
			for(start = cmd[c].indexOf(leftDelimiter, 0); start != -1; start = cmd[c].indexOf(leftDelimiter, start)) {
				end = cmd[c].indexOf(rightDelimiter, start);
				if(end == -1) {
					break;
				}

				variableName = cmd[c].substring(start + leftDelimiter.length(), end);
				argValue = System.getProperty(variableName);
				if(argValue == null) {
					argValue = (String) substitutions.get(variableName);
				}
				if(argValue != null) {
					cmd[c] = replace(cmd[c], cmd[c].substring(start, end + rightDelimiter.length()), argValue);
				} else {
	 ...
	 *@author           Jim Lerner
	 */
	protected final static String replace(String original, String find, String replace) {
	...
	protected static boolean validateCPU(String expected) {
		...
		return false;
//		if(System.getProperty(GPConstants.COMMAND_PREFIX, null) != null) { FIXME
//			return true;
	...
	private void doExec(String[] commandLine) throws IOException, InterruptedException {
	...
	static {
		if("true".equals(System.getProperty("edu.mit.genome.gp.ui.analysis.debug"))) {
			DEBUG = true;
		}
	}
}
