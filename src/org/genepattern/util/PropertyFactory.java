...
public class PropertyFactory implements FilenameFilter {
	...
	/**
	 * the path to the property file will be located in the environment variable
	 * omnigene.conf
	 */
	...
	/**
	 * loads the props file they must either have a .properties extention or
	 * .props extention
	 * 
	 * @exception if
	 *                the PropertyFile cannot be found
	 */
	...
	/**
	 * @exception if
	 *                Properyfile cannot be found
	 * @exception if
	 *                an IO error occurs
	 */
	private void getPropertiesFiles() throws IOException,
			PropertyNotFoundException {

		propsHash = new Hashtable();
		String[] list = new File(System.getProperty("omnigene.conf"))
				.list(this);
		for (int i = 0; i < list.length; i++) {
			String filePath = System.getProperty("omnigene.conf")
	...
	public Properties getPropertiesOnClasspath(String propsFileName)
			throws PropertyNotFoundException {

		Properties p = null;
		/**
		 * First try and find the properties files on the classpath This is
		 * dependant on jdk 1.3.1 which all major platform support We'll check
		 * for this first and pass over it if we're not >=jdk1.3.1 If not >= 1.3
		 * then look for omnigene.conf property
		 */
		...
		if ((Float.parseFloat(System.getProperty("java.specification.version"))) >= 1.3) {
			...
			try {
				...
				//URL path =
				// ClassLoader.getSystemClassLoader().getResource(propsFileName);
				...
				System.out.println("Pros file cannot be found: "
						+ ioe.getMessage());
			...
			throw new UnsupportedOperationException(
					"Cannot get classpath with Java version less than 1.3");
		}
	 ...
	 * @exception if
	 *                can't find property file or property object
	 */
	...
	public Properties getProperties(String propertyFileName)
		...
		if (props == null) {
			throw new NullPointerException(
					"Property File can't be found or does not exist");
		}
