...
public class XMath {
	...
	//            if(! (flip == isEven(i)) )
	//                System.out.println("Error: "+i+" was determined to be "+ ((isEven(i)) ?
	// "even" : "odd") );
	//             flip = !flip;
	//        }
	//    }
	 ...
	 */
	public static final boolean equals(final int fuzz, final int n1,
			final int n2) {

		if ((Math.abs(n1 - n2)) <= fuzz)
			return true;
		return false;
	}

	/** sumOfSquares is x1^2 + x2^2 + ... */
	public static float biasedStd(final float sumOfSquares, final float mean,
			final int num) {
	 ...
	 * 
	 * First, we make the variance at least a fixed percent of the mean If the
	 * mean is too small, then we use an absolute variance
	 * 
	 * However, we don't want to bias our algs for affy data, e.g. we may get
	 * data in 0..1 and in that case it is not appropriate to use an absolute
	 * standard deviation of 10 - will kill the signal.
	 */
	...
	/**
	 * Creates an array whose elemnts are randomly arranged between 0 and num -
	 * 1. Example: randomize(5) could yeild: 0, 3, 1, 2, 4
	 * 
	 * The same random number generator is employed (in this instance of the
	 * jvm) so safe to call multiple times. But as it uses the same seed each
	...
	public static final double normalizedEntropy(final double[] array) {
		// normFactor is the entropy of a uniform distribution of the
		// appropriate length
	...
	//        //System.out.println("getTotalVariance() numRows="+numRows+"
	// numCols="+numCols
	...
	//        for (int r = 0; r < numRows; r++) {
	//            matrix.getRow(r, row);
	//            System.out.println("row"+r+" init 10="+ArrayUtils.toString(row,
	// Math.min(10, row.length)));
	//        }
	//	float rowVariance = 0;
	//        final edu.mit.genome.gp.analytics.distance.FloatArraysDFInput input =
	//            new edu.mit.genome.gp.analytics.distance.FloatArraysDFInput(rowMean,
	// row);
	...
	//            rowVariance +=
	// edu.mit.genome.gp.analytics.distance.EuclideanDistance.calculateEuclideanDistanceSquaredAsFloat(input);
