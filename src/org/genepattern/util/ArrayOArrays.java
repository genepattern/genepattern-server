 ...
 * each of a different Class.  It enforces that all arrays be populated individully
 * and row-wise in sequence.
 * @author  kohm
 */
public class ArrayOArrays {
    ...
    public ArrayOArrays(final int depth, final Class[] classes) {
        ...
        for(int i = 0; i < cnt; i++) {
            ...
            final Object array = java.lang.reflect.Array.newInstance(clss, depth);
    ...
    /** gets the specified array checks the status first
     * and if not ready throws an Exception*/
    ...
    /** sets the short throws a ClassCastException if shouldn't be setting an short*/
    ...
    /** sets the float 
     * throws a ClassCastException if shouldn't be setting a float */
    public void setFloat(final float  fl) {
        ((float[])arrays[current_column])[current_row] = fl;
        advance();
    }
    /** sets the double 
     * throws a ClassCastException if shouldn't be setting a double */
    public void setDouble(final double value) {
        ((double[])arrays[current_column])[current_row] = value;
        advance();
    }
    /** sets the boolean from the String representation of a boolean
     * throws a ClassCastException if shouldn't be setting a boolean */
    public void setBoolean(final String text) {
        ...
        if(string.equals("true") || string.equals("t") || string.equals("yes") )
            value = true;
        else if(string.equals("false") || string.equals("f") || string.equals("no"))
            value = false;
        else 
            throw new IllegalArgumentException(
                 "Not a valid string representation ("+text+") of a boolean value!");
        setBoolean(value);
    }
    /** sets the boolean 
     * throws a ClassCastException if shouldn't be setting a boolean */
    ...
    /** sets the character 
     * throws a ClassCastException if shouldn't be setting a char */
    public void setChar(final char value) {
        ((char[])arrays[current_column])[current_row] = value;
        advance();
    }
    /** sets the String 
     * throws a ClassCastException if shouldn't be setting a String */
...
//    }
    ...
    private void advance() {
        current_column++;
        if(current_column == array_count) {//if EOR goto next row and reset column 
            current_column = 0;
            current_row++;
    ...
    private void checkStatus() {
        if(!isComplete()) {
            throw new IllegalStateException("Arrays are not yet full: rows "
                + current_row+" of "+depth
                + "; columns "+current_column+" of "+array_count);
        }
    }
    // fields
    /** the value that the current_column and current_row are set to when 
