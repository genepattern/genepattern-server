...
public abstract class SafeRun implements java.lang.Runnable {
	...
	public final void run() {
		...
		//        } catch (IllegalStateException ex) { // these are errors but the
		// program may still be runnable
		//            showError ("Error: ", ex);
		//            ex.printStackTrace ();
		//// } catch (AssertionError ex) {// these should not occur unless
		// something is wrong with program
		...
		//        } catch (Throwable t) { // don't know how serious the problem is but
		// still shouldn't have happened
		//            showError ("Internal error of unknown severity!\nAssume severe error
		// - Save work and restart program:", t);
	 ...
	 * 
	 * @param thw
	 *            the exception or error - the throwable
	 */
	public static void handleException(final Throwable thw) {
		try {
			...
			showWarning("Warning: ", w);
		} catch (IllegalStateException ex) { // these are errors but the program
			...
			showError(
					"Internal Error! Should save work and restart program:\n",
					ex);
			ex.printStackTrace(); // so assume something is wrong with program
		} catch (java.lang.Error er) {
			showError(
			...
			er.printStackTrace(); // so assume something is wrong with program
								...
								// still shouldn't have happened
			showError(
					"Internal error of unknown severity!\nAssume severe error - Save work and restart program:",
					t);
			t.printStackTrace(); // so assume something is wrong with program
		}
	}
	...
	protected static final Reporter REPORTER = org.genepattern.util.AbstractReporter
			.getInstance();
}