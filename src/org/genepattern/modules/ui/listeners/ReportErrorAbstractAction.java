...
public abstract class ReportErrorAbstractAction extends AbstractAction {
	...
	/**
	 * Creates new ReportErrorAbstractAction with a name and an accelerator
	 * based an the specified character. The resulting accelerator will be a
	 * combination stroke of the char and the platform dependent modifier key.
	 * For example on windows platforms the standard way to copy a selection is
	 * to simultainiously press control+C. The modifier control is automatically
	 * determined on a per-platform basis.
	 *  
	 */
	...
	/**
	 * Creates new ReportErrorAbstractAction with a name, icon and an
	 * accelerator based an the specified character. The resulting accelerator
	 * will be a combination stroke of the char and the platform dependent
	 * modifier key. For example on windows platforms the standard way to copy a
	 * selection is to simultainiously press control+C. The modifier control is
	 * automatically determined on a per-platform basis.
	 *  
	 */
	protected ReportErrorAbstractAction(final String name, final Icon icon,
			final char accel) {
	...
	/**
	 * helper method sets the accelerator key Note the key should originate from
