...
/**
 * Stolen from the expresso project... Perhaps not needed for Java 1.4? JTable
 * now supports cut copy and paste.
 * 
 * @author kohm
 * @version
 */
public class BaseTable extends javax.swing.JTable {
	...
	/**
	 * helper method for the constructor Note should probably check to see if
	 * this Class has the command in the InputMap before replacing it. This
	 * would allow future versions of Java's JTable to work as advertised
	 */
	...
	public static final boolean isModelNotEditable(final TableModel tm) {
		for (int col = tm.getColumnCount() - 1; col >= 0; col--) { //reverse
																   // loop
	...
	/**
	 * method for descovering the KeyStrokes bound to a command Could not find a
	 * general method to this in the java/javax packages
	 */
	private static final KeyStroke[] getKeyStrokesBoundToCommand(
		...
		final KeyStroke[] bound_strokes = new KeyStroke[command_bound_strokes_list
				.size()];
	 ...
	 *            the table model
	 * @param longValues
	 *            an array of objects whos string size will set the individual
	...
	protected final void initColumnSizes(final Object[] longValues) {
		...
		for (int i = 0, limit = model.getColumnCount(); i < limit; i++) {
			final TableCellRenderer tcr = getDefaultRenderer(model
					.getColumnClass(i));
			//System.out.println("i="+i+" longValues[i]="+longValues[i]+"
			// class="+model.getColumnClass(i)+" Default renderer="+tcr);
			...
			final java.awt.Component comp = tcr.getTableCellRendererComponent(
					this, longValues[i], false, false, 0, i);
	 ...
	 * tooltips of its renderers <code>JTable</code> must be a registered
	 * component with the <code>ToolTipManager</code>. This is done
	 * automatically in <code>initializeLocalVars</code>, but if at a later
	 * point <code>JTable</code> is told <code>setToolTipText(null)</code>
	 * it will unregister the table component, and no tips from renderers will
	 * display anymore.
	 * 
		...
		if ((hitColumnIndex != -1) && (hitRowIndex != -1)) {
			...
			if (obj != null) {
				if (obj instanceof String[])
					tip = ((DefaultTableCellRenderer) this
							.getDefaultRenderer(obj.getClass())).getText();
				else
					tip = obj.toString();
			}
		}
	...
	// paste functionality
	/**
	 * pastes the array elements into the corresponding cells The first array
	 * dimension is the row and the second is the column
	 * 
	 * @param text
	 *            the array of strings to paste text[row]column]
	 * @return a boolean, true if success
	 */
	public final void paste(final String[][] text) throws PasteException,
			PasteWarning {
	 ...
	 * 
	 * @param labels
	 *            the array of strings to paste into the table
	 * @param col
	 *            the column to paste the labels into starting at row zero
	 */
	public final void paste(final String[] labels, final int col)
			throws PasteException, PasteWarning {
	...
	/**
	 * Another helper method for paste(String) It does nothing but rethrows the
	 * exception. Subclasses could override this method to display messages,
	 * conditionaly rethrow the exception, etc.
	 * 
	 * @param e
	 *            the paste exception
	 * @exception PasteException
	 *                is thrown if the user cancels
	 */
	 ...
	 * what can go into which column or row Currently just adds the text to the
	 * cell without throwing an exception.
	 * 
	 * @param name
	 *            the name of the sample (col = 0) or scan (col >= 1)
	 * @param row
	 *            the number of the row
	 * @param col
	 *            the number of the column
	 */
	protected void addCell(final String name, final int row, final int col)
			throws PasteException, PasteWarning {
		setValueAt(name, row, col);
	}

	/**
	 * This is called before pasting begins which allows for stuff to be done
	 * before pasting
	 */
	...
	class PasteAction extends AbstractAction {
		...
		public final void actionPerformed(ActionEvent e) {
			...
			//            try {
			//                Object data = transf.getTransferData
