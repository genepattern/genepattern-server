 ...
 * @version
 */
public class PeriodicProgressObserver implements java.awt.event.ActionListener,
		java.awt.event.WindowListener {
	/** constructor */
	public PeriodicProgressObserver(final Component comp, final String message) {
		this(comp, message, 1, true/* one progress bar */, 100, false/*
																    * don't show
																    * when the
																    * parent
																    * does
	 ...
	 * 
	 * @param comp
	 *            the component that is attached to a frame
	 * @param message
	 *            the text for the dialog's title
	 * @param num
	 *            the number of ProgressObservables
	 * @param one_bar
	 *            show progress with one bar not two
	 * @param delay
	 *            the time in milliseconds between checking on and displaying
	 *            the progress
	 * @param show
	 *            if true then the dialog will appear when the parent becomes
	 *            visible
	 */
	public PeriodicProgressObserver(final Component comp, final String message,
			final int num, final boolean one_bar, final int delay,
			final boolean show) {
		this.component = comp;
		...
		monitor
				.setString("This is just a long sentence to help the packing or anything else");
		monitor.setValue(0);
		timer = new javax.swing.Timer(delay /* 500ms = 1/2 sec */, this);
		timer.setInitialDelay(0);

		final Frame frame = (comp instanceof Frame) ? (Frame) comp
	...
	public void addProgressObservable(final ProgressObservable observable) {
		...
		Thread.yield();
		//System.out.println("PeriodicProgressObserver adding
		// "+observer_index+"th ProgressObservable: "+ observable);
	}
	...
	public final void actionPerformed(final java.awt.event.ActionEvent evt) {
		//label.setText("total="+total+" observable="+observable+"
		// observer_index="+observer_index+" max_observer="+max_observer);
	...
	/**
	 * returns true if the overall progress bar is complete or the last
	 * observable is being monitored
	 */
	...
	public static final Component getTopParent(Component comp) {
		Component c = comp;
		while (c != null
				&& !(c instanceof Window || c instanceof java.applet.Applet))
			c = c.getParent();
		if (c == null)
			return comp;
		return c;
	...
	/**
	 * Max value for an observable if only one observable then 100 two 50 three
	 * 33 etc
	 */
