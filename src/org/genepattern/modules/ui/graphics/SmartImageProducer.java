 ...
 * the current amount of free memory taken into consideration (graphics ram
...
public final class SmartImageProducer implements java.awt.image.ImageProducer {
	...
	public SmartImageProducer(Component comp, int width, int height,
			ImageDrawer drawer) {
		this.width = width;
		this.height = height;
		this.component = comp;
		...
		//this.height /= num_parts; this.height *= 2; this.num_parts = 2;//
		// testing

		System.out.println("SmartImageProducer: Free memory = " + free_mem
				+ " total memory=" + total_mem + " number of parts="
				+ num_parts + " total width=" + width + " height=" + height);
	}
	...
	public void addConsumer(java.awt.image.ImageConsumer imageConsumer) {
		if (consumer == null) {
			...
			System.err
					.println("Error: trying to add an image consumer, but alread have a consumer");
		}
	}
	...
	public void requestTopDownLeftRightResend(
			java.awt.image.ImageConsumer imageConsumer) {
	 ...
	 * already been registered with the producer.
	 */
	public void startProduction(final java.awt.image.ImageConsumer imageConsumer) {
		...
		BufferedImage image = (BufferedImage) component.createImage(width,
				increment);
		...
		for (int i = 0, h = 0, limit = height; h < limit;) {
			...
			if (whats_left < increment) { // no blank stuff at the bottom of
										  // image
				System.out.println("limit=" + limit + " h=" + h + " increment="
						+ increment + " whats_left=" + whats_left);
			...
			System.out.println("loop " + (++i) + " h=" + h + " increment="
					+ increment);
	...
	/**
	 * this will get it's chance to draw something before the image is sent to
	 * the ImageConsumer
	 */
	...
	private final class MyImageConsumer implements ImageConsumer {
		...
		public void setPixels(int x, int y, int w, int h, ColorModel model,
			...
			//System.out.println("setPixels (ints): x="+x+" y="+y+" w="+w+"
			// h="+h+" model="+model+" num pixels="+pixels.length+" off="+off+"
			// scansize="+scansize);
		...
		public void setPixels(int x, int y, int w, int h, ColorModel model,
			...
			//System.out.println("setPixels (bytes): x="+x+" y="+y+" w="+w+"
			// h="+h+" model="+model+" num pixels="+pixels.length+" off="+off+"
			// scansize="+scansize);
			consumer.setPixels(x, y, w, h, model, pixels, off, scansize);
			if (y > max_y)
		...
		public void setDimensions(int width, int height) {
			System.out.println("dimension: width=" + width + " height="
					+ height);
		}
