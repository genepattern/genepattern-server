...
/**
 * This class together with the LegendTableModel allows the column headers to be
 * self documenting. Thus if the table is too big for the space allocated to it
 * the column header labels become unreadable. If the user's mouse lingers over
 * the header a tool tip will appear indicating the label. Likewise, the cells
 * in the table have description tool tips that are displayed when the user's
 * mouse lingers over them.
 * 
 * @author KOhm
 * @version 0.1
...
public class LegendTable extends AutoResizeTable {
	...
	//            throw new IllegalArgumentException("The model must be a subclass of
	// LegendTableModel:\n"+model);
	//        LegendTableModel ltm = (LegendTableModel)model;
	//        setModel(ltm);
	//    }
	/**
	 * Sets the header tool tips The index of the array of text corresponds to
	 * each column
	 *  
	 */
	private final void setHeaderToolTips(final LegendTableModel model) {
		...
		for (int col = 0; col < limit; col++) {
			...
			if (header_render == null) {
				header_render = header_renderer_creator
						.createHeaderCellRenderer();
				tc.setHeaderRenderer(header_render);
			}
			if (header_render instanceof JComponent) {
				((JComponent) header_render).setToolTipText(model
						.getColumnDescription(col));
			} else
				org.genepattern.util.AbstractReporter.getInstance().logWarning(
						"Warning: Header Renderer cannot have tool tips-"
								+ header_render);
		}
		//System.out.println("tcm="+tcm+" header_render="+header_render+"
		// tc="+tc);
	}

	/**
	 * Sets the column tool tips where a column specific tool tip will appear
	 * when lingering over a cell. The index of the array of text corresponds to
	 * each column
	 *  
	 */
	private final void setColumnToolTips(String[] tips) {
		...
		for (int col = 0, limit = this.getColumnCount(); col < limit; col++) {
			...
			//            renderer = tablecolumn.getCellRender(); // prevent excess object
			// creation
			//            if(renderer == null || renderer == this.getDefaultRenderer
			// (dataModel.getColumnClass (col)))
	 ...
	 * from the default protected createDefaultColumnModel(). This means that
	 * even if the code changes this class will still get the proper renderer
	 * for displaying the Header cells.
	 */
	static public class HeaderCellRendererCreator extends JTableHeader {
		 ...
		 * createDefaultRenderer(). Allowing for changes in the code in the
		 * future that cause the Header cells to look different.
		 */
