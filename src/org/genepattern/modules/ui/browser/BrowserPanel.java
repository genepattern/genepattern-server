...
public class BrowserPanel extends javax.swing.JPanel implements Messenger{
    ...
    public BrowserPanel (final Frame frame, final String base_title) {
        ...
        setTitle("");
        ((SimpleHtmlViewer)htmlPane).setProgressButtonMessenger(progressBar, stopButton, this);
        ...
        htmlPane.addHyperlinkListener(new HyperlinkListener() {
            public final void hyperlinkUpdate(javax.swing.event.HyperlinkEvent event) {
	...
	public void loadFile(File f) {
		 try {
			...
			while((charsRead = fr.read(c)) != -1) {
				doc.insertString(doc.getLength(), new String(c, 0, charsRead), null);	
			}
			fr.close();
			} catch(Exception e) {
				e.printStackTrace();	
    ...
    private final URL internalLoadURL(final URL url) {
        if(url != null) {
            ...
            try {
                ...
                //setMessage ("[Error: couldn't get documentation from the web "+url+"]");
                JOptionPane.showMessageDialog(this, "[Page could not be loaded "+url+"] \nReason: "+e);
                return null;
//            } finally {
//                return (done ? url : null);
    ...
    /**
     * loads the url into the page
     * starts fresh; clears the cashe, disable the forward, and back buttons
     */
    ...
    public final void load(java.io.InputStream in, final Object desc) throws java.io.IOException{
    ...
    private void initComponents() {//GEN-BEGIN:initComponents
        ...
        htmlPane = new org.genepattern.modules.ui.browser.SimpleHtmlViewer();
    ...
    /**
     * The HTML EditorKit will generate hyperlink events if the JEditorPane
     * is not editable (JEditorPane.setEditable(false); has been called).
     *
     * If HTML frames are embedded in the document, the typical response
     * would be to change a portion of the current document.
     * The following code fragment is a possible hyperlink listener
     * implementation, that treats HTML frame events specially,
    ...
    private void processHyperlink (final javax.swing.event.HyperlinkEvent e)  {
        ...
        //if (type == HyperlinkEvent.EventType.ACTIVATED || type == HyperlinkEvent.EventType.ENTERED ) {
        if (type == HyperlinkEvent.EventType.ACTIVATED ) {
            ...
            if (e instanceof HTMLFrameHyperlinkEvent) {
                ...
                try {
                    ...
                    if( !done)
                        JOptionPane.showMessageDialog(this, "[Page could not be loaded "+link+"]");
                    //t.printStackTrace ();
                }
            }
