...
public class RunVisualizer {
	...
	public RunVisualizer(HashMap params, URL source, String[] supportFileNames,
			long[] supportFileDates) {
		this.params = params;
		this.source = source;
	...
	protected String[] doCommandLineSubstitutions() throws IOException {
		...
		String commandLine = (String) params
				.get(RunVisualizerConstants.COMMAND_LINE);
	...
	protected String variableSubstitution(String var, HashMap hmDownloadables) {
		...
		String variableName;
		for (start = var.indexOf(leftDelimiter, 0); start != -1; start = var
			...
			if (argValue != null) {
				...
				// argValue);
				if (hmDownloadables != null
						&& hmDownloadables.containsKey(variableName)) {
					try {
						argValue = ((File) hmDownloadables.get(variableName))
								.getCanonicalPath();
						if (DEBUG)
							System.out.println("replacing URL "
									+ (String) params.get(variableName)
									+ " with " + argValue);
					} catch (IOException ioe) {
						System.err.println(ioe
								+ " while getting canonical path for "
								+ ((File) hmDownloadables.get(variableName))
										.getName());
					}
				}
				var = replace(var, var.substring(start, end
	...
	protected String downloadSupportFiles() throws IOException {
		...
		if (host.equals("localhost")
			...
			if (DEBUG)
				System.out.println("using local libdir: "
						+ (String) params.get(RunVisualizerConstants.LIBDIR));
		...
		for (int curf = 0; curf < currentFiles.length; curf++) {
			...
			if (!found) {
				...
				if (DEBUG)
					System.out.println("deleting extraneous file "
							+ currentFiles[curf].getCanonicalPath());
				...
				if (currentFiles[curf].lastModified() != supportFileDates[supf]) {
					...
					if (DEBUG)
						System.out.println("deleting out-of-date file "
								+ currentFiles[curf].getCanonicalPath());
					currentFiles[curf].delete();
				}
			}
		...
		for (supf = 0; supf < supportFileNames.length; supf++) {
			if (!new File(fLibdir, supportFileNames[supf]).exists()) {
				// need to download it
				if (DEBUG)
					System.out.print("downloading missing file "
							+ supportFileNames[supf] + "...");
				Date startTime = new Date();
				URL urlFile = null;

				urlFile = new URL(source, "/gp/getFile.jsp?task=" + lsid
						+ "&file=" + encode(supportFileNames[supf]));

				File file = downloadFile(urlFile, fLibdir,
						supportFileNames[supf]);
				file.setLastModified(supportFileDates[supf]);
				long downloadTime = new Date().getTime() - startTime.getTime();
				if (DEBUG)
					System.out
							.println(" received " + file.length()
									+ " bytes in " + downloadTime / 1000.0
									+ " seconds");
			}
		}

		if (DEBUG)
			System.out.println("Total download time "
					+ (new Date().getTime() - startDLTime.getTime()) / 1000.0
					+ " seconds");
		return fLibdir.getCanonicalPath();
	}

	// download a URL to a local file and return a File object for it
	protected File downloadFile(URL url, File dir, String filename)
			throws IOException {
		InputStream is = null;
	...
	protected boolean validateCPU() throws Exception {
		...
		throw new Exception("Cannot run on this platform.  " + taskName
				+ " requires a " + expected + " CPU, but this is a " + actual);
	}
	...
	private String getURLFileName(URL url) {
		...
		if (queryIdx == -1) queryIdx = file.length();
		...
		if (baseName.indexOf("retrieveResults.jsp") != -1
			...
			String temp = decode(file.substring(j + "filename=".length(), file
					.length()));
		...
		if (baseName.indexOf("getFile.jsp") != -1
			...
			if (slashIdx >= 0) temp = temp.substring(slashIdx+1);
	...
	protected HashMap downloadInputURLs() throws IOException {
		...
		while (stDownloadables.hasMoreTokens()) {
			...
			if (DEBUG)
				System.out.println("downloading " + paramURL + " to " + tempdir
						+ " as " + filename);
	 ...
	 * @author Jim Lerner
	 */
	protected static final String replace(String original, String find,
			String replace) {
	...
	public static void main(String[] args) {
		...
		try {
			...
			StringTokenizer stFileNames = new StringTokenizer(
					args[SUPPORT_FILE_NAMES], ",");
			StringTokenizer stFileDates = new StringTokenizer(
					args[SUPPORT_FILE_DATES], ",");
			...
			while (stFileNames.hasMoreTokens()) {
				...
				if (stFileDates.hasMoreTokens()) {
					supportFileDates[f] = Long.parseLong(stFileDates
							.nextToken());
				} else {
