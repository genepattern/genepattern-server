...
public class RunR {
	 ...
	 * 
	 * @param rSourceFile
	 *            The full path to the R source file
	 * @param methodName
	 *            The name of the method in the R source file to invoke
	 * @param args
	 *            The command line arguments
	 * @param pathToRHome
	 *            The path to R Home
	 * @param captureStandardError
	 *            Whether to capture the standard error from R to System.error
	 * @param captureStandardOut
	 *            Whether to capture the standard out stream from R to
	 *            System.out
	 * @exception IOException
	 *                If an IO error occurs
	 */
	public static void execute(String rSourceFile, String methodName,
			String[] args, String pathToRHome, boolean captureStandardError,
			boolean captureStandardOut) throws IOException {
		...
		/*
		 * find R_HOME: 1. pathToRHome 2. environment variable 3. assume it is
		 * in the path
		 */
		...
		for (int i = 0; i < args.length; i++) {
			...
			hasQuotes = true;
			sendCmd(stdin, (!hasQuotes ? "\"" : "") + fixPath(args[i])
					+ (!hasQuotes ? "\"" : ""));
		}
		sendCmd(stdin, ")\n");
		...
		public void run() {
			...
			try {
				while ((line = in.readLine()) != null) {
					...
					boolean bNeedsBreak = (line.length() > 0 && (line
