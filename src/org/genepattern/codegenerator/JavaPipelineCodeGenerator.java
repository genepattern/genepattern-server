...
/**
 *  Generate Java code to form a pipeline of GenePatternAnalysis tasks,
 *  complete with
 *  <ul>
...
public class JavaPipelineCodeGenerator extends AbstractPipelineCodeGenerator implements TaskCodeGenerator {
    ...
    *  inheritance code for input from previously-run pipeline stages. At end
    *  of task, generate links for downloading output files individually.
    *  Invoked once for each task in the pipeline.
    *
    * @param  jobSubmission             description of task
    * @param  taskInfo                  TaskInfo for this task
    * @param  taskNum                   task number (indexed from zero)
    * @param  actualParams              Description of the Parameter
      ...
      if(formalParams != null) {
         for(int i = 0; i < formalParams.length; i++) {
            ...
            if(jobSubmission.getRuntimePrompt()[i]) {
               ...
               int inheritedTaskNum = Integer.parseInt((String) actualAttributes.get(INHERIT_TASKNAME));// task number of
               // task to get
               // output from
   ...
   private void appendParameter(ParameterInfo actual, StringBuffer invocation) {
      ...
      if((index = val.indexOf(getFile)) > 0) {
         String query = val.substring(index + getFile.length(),
               val.length());
