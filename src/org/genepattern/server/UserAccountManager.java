...
public class UserAccountManager {
    ...
    /**
     * Authenticate using the username:password pair. This matches the credentials in the GenePattern user database.
     * <br/>
     * Note: It handles a special case for pipelines executed from a command line. To avoid passing the users encrypted
     * password as a command line argument to the pipeline, an environment variable is stored in the server's JVM.
     * 
     * @param username
     * @param password - the user's password, or a token generated by {@link EncryptionUtil#pushPipelineUserKey(User)}
     * @return
     * @throws AuthenticationException
     */
    public boolean authenticateUser(String username, byte[] password) throws AuthenticationException {
        ...
        encryptedPassword = EncryptionUtil.getInstance().getPipelineUserEncryptedPassword(new String(password));
