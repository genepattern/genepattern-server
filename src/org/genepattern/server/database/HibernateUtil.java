...
public class HibernateUtil {
	...
	if (dbVendor.equals("ORACLE")) {
	    ...
	    return (Integer) getSession().createSQLQuery(
		    "SELECT NEXT VALUE FOR " + sequenceName + " FROM dual")
		    .uniqueResult();
	} else {
    ...
    /**
         ...
         * standard and are not portable. The syntax for hsql and oracle differ,
         * and MySql doesn't expose sequeneces at all. Thus we use a table based
         * scheme to simulate a sequence.
         * 
         ...
         * reason a new session and transaction is created and closed prior to
         * exit.
         */
    private static synchronized int getNextSequenceValueGeneric(
	    String sequenceName) {
	...
	try {
		...
		String errorMsg = "Sequence table does not have an entry for: "
			+ sequenceName;
		log.error(errorMsg);
