...
public class TaskUtil {
	 ...
	 * zip of zips. The 0th index of the returned array holds the TaskInfo for
	 * the pipeline itself. Note that the returned <code>TaskInfo</code>
	 * instances have getID() equal to -1, getParameterInfo() will be
	 * <code>null</code>, getUserId is <code>null</code>, and getAccessId
	 * is 0.
	 * 
	 * @param zipf
	 *            a zip of zips.
	 ...
	 *                if an error occurs during reading
	 */
	public static TaskInfo[] getTaskInfosFromZipOfZips(File zipf)
			throws IOException {
		...
		for (Enumeration eEntries = zipFile.entries(); eEntries
			...
			try {
				...
				// every file within the stream must be extracted before it can
				// be processed
