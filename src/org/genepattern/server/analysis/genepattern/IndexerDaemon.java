...
public class IndexerDaemon implements Runnable {
	...
	public static void main(String[] args) {
	    try {
		System.setProperty("genepattern.properties", System.getProperty("genepattern.properties", "C:/Program Files/GenePattern/Tomcat"));
		IndexerDaemon daemon = IndexerDaemon.getDaemon();
		Thread t = new Thread(daemon, "IndexerDaemon");
		t.setPriority(Thread.NORM_PRIORITY - 3);
		t.start();

		if (DEBUG) System.out.println("notifying daemon");
		synchronized(daemon.indexLock) {
			daemon.indexLock.notify();
		}

		if (DEBUG) System.out.println("notifying daemon again");
		synchronized(daemon.indexLock) {
			daemon.indexLock.notify();
		}

		if (DEBUG) System.out.println("IndexerDaemon done");
	    } catch (Throwable t) {
	...
	public static IndexerDaemon getDaemon() throws IOException {
		if (daemon == null) daemon = new IndexerDaemon();
		return daemon;
	}
	...
	public void run() {
		...
		String jobQuery =  "select job_no from analysis_job where isIndexed is null and status_id in (3,4)"; // Finished, Error
		...
		if (DEBUG) System.out.println("IndexerDaemon running on " + Thread.currentThread().getName() + " at priority " + Thread.currentThread().getPriority());
		...
		while (true) {
			...
			if (DEBUG) System.out.println("IndexerDaemon: waiting for notification on lock " + indexLock.hashCode());
			indexLock.wait();
			if (DEBUG) System.out.println("IndexerDaemon: awakening from notification");
		    }
		} 
	...
	protected static void notifyDaemon() {
		if ("true".equalsIgnoreCase(System.getProperty("disable.gp.indexing"))) return;

	    try {
		Object lock = getDaemon().indexLock;
		if (DEBUG) System.out.println("IndexerDaemon.notifyDaemon: waking up daemon for indexing, using lock " + lock.hashCode());
		synchronized (lock) { lock.notify(); }
		if (DEBUG) System.out.println("IndexerDaemon.notifyDaemon: woke up daemon for indexing");
