 ...
 * <p>Description: This class implements task loading (add and update) and deletion.
 * All task dbloader class will extends this class and implement setup method to setup task name,
 * description, class name and <code>ParameterInfo<code> array </p>
 * @author Hui Gong
 * @version 1.0
 */
...
public abstract class DBLoader {
     ...
     * task name, description, class name and parameters should be initialized in here.
     */
    public abstract void setup();

    /**
     * loads an analysis task into the database, it will add a new task if the task doesn't exist;
     * and it will update the task description and parameters if the task is already in the database.
    ...
    public void load() throws OmnigeneException{
        ...
        if(taskID<0){
            ...
            int id;
            try{
                id = ds.addNewTask(this._name,this.user_id, this.access_id, this._taskDescription, parameter_info, this._classname, this._taskInfoAttributes);
...
/*
        ...
        try{
        // search for an existing task with the same name
        taskID = getTaskIDByName(_name, user_id);
        }
        catch(OmnigeneException e){
            //this is a new task, no taskID exists
            // do nothing
		...
		update();
		return;
		//throw new OmnigeneException("Unable to create a new task: " + _name + " already exists.");
	}
*/
...
//            System.out.println(this._name+" is created with id "+id);
    ...
    public void delete() throws OmnigeneException, RemoteException {
        ...
        try{
            ...
            throw new OmnigeneException("Unable to stop "+this._name+". "+e.getMessage());
        }
        try{
    ...
    public void run(int type) throws OmnigeneException {
        try {
            if(type == DBLoader.CREATE){
                ...
                throw new OmnigeneException("Unable to run the loader, running type should be specified: 1 (create), 2 (update) and 3 (delete)");
            }
        } catch (RemoteException re) {
    ...
    private static AnalysisJobDataSource getDataSource() throws OmnigeneException{
