...
/**
 * CommandExecutor2 interface to the LSF queue, based on original LsfCommandExecutor, which uses the Broad Core library
 * for LSF integration.
 * 
 * @author pcarr
 *
 */
public class LsfCommandExecutor2 implements CommandExecutor2 {
    private static Logger log = Logger.getLogger(LsfCommandExecutor2.class);
    ...
    /**
     * @deprecated, this is here for legacy implementation of the CommandExecutor interface
     */
    private void runCommand(final String[] commandLine,
			...
			throws CommandExecutorException {
		log.debug("Running CommandExecutor(v1) job "+jobInfo.getJobNumber()+". "+jobInfo.getTaskName());
    ...
    /**
     * This is the preferred CommandExecutor2 implementation which uses the gpContext to get custom configuration parameters.
     */
    @Override
    public void runCommand(GpContext gpContext, String[] commandLine, Map<String, String> environmentVariables, File runDir, File stdoutFile, File stderrFile, File stdinFile) throws CommandExecutorException {
        if (gpContext==null) {
            throw new CommandExecutorException("server error, gpContext==null");
        }
        final JobInfo jobInfo=gpContext.getJobInfo();
        if (jobInfo == null) {
            throw new CommandExecutorException("server error, gpContext.jobInfo==null");
        }
        log.debug("Running CommandExecutor(v2) job "+jobInfo.getJobNumber()+". "+jobInfo.getTaskName());
        final LsfCommand2 cmd = new LsfCommand2(ServerConfigurationFactory.instance(), gpContext);
        cmd.runCommand(commandLine, environmentVariables, runDir, stdoutFile, stderrFile, stdinFile, LsfJobCompletionListener.class);
        LsfJob lsfJob = cmd.getLsfJob();
        lsfJob = submitJob(""+jobInfo.getJobNumber(), lsfJob);
        log.debug(jobInfo.getJobNumber()+". "+jobInfo.getTaskName()+" is dispatched.");
    }
    ...
    public void terminateJob(final JobInfo jobInfo) {
        ...
        if (lsfJobs.isEmpty()) {
            ...
            try {
                ...
                log.error("Unexpected error in terminateJob(gpJobId="+gpJobId+")", t);
            }
        }
        try {
            GenePatternAnalysisTask.handleJobCompletion(jobInfo.getJobNumber(), -1, "User terminated job #"+jobInfo.getJobNumber());
        }
        catch (Throwable t) {
