...
public class EmailNotificationManager {
	...
	public HashMap emailToAddresses(String addresses, String from,
		...
		while (stTo.hasMoreTokens()) {
			...
			try {
				...
				if (tmHosts == null || tmHosts.size() == 0) {
					failures.put(to, "No MX servers for recipient " + to
							+ ".  Bad domain name?");
						continue;
				}
				...
				for (Iterator eHosts = tmHosts.entrySet().iterator(); eHosts
						.hasNext();) {
					...
					try {
						...
						sb.append("" + ueMessage
								+ " while attempting to send to " + to
								+ " via " + host);
					} catch (Exception e) {
						sb.append("" + e + " while attempting to send to " + to
								+ " via " + host);
					}
				}
				if (!success) {
				...
				System.out.println(e + " while attempting to send to " + to
						+ " via " + host + "<br>");
				e.printStackTrace();
			}
...
class JobWaitThread extends Thread {
	...
	public void run() {
		try {
			...
			msg.append("<a href='http://gp21e-789.broad.mit.edu:8080/gp/pages/jobResults.jsf?jobID=jobID'>GenePattern Results Page</a>");
