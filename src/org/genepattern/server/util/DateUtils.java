...
public class DateUtils {
   ...
   /**
    ...
    private static long getDay(long date, int startOfWeek, int increment) {
        Calendar calendar = CALENDAR;
        System.out.println("getDay before sync");
        synchronized(calendar) {
            System.out.println("getDay after sync");
            calendar.setTimeInMillis(date);
    ...
    private static long getMonth(long date, int increment) {
		long result;
        Calendar calendar = CALENDAR;
        System.out.println("getMonth before sync");
        synchronized(calendar) {
        	  System.out.println("getMonth after sync");
                    calendar.setTimeInMillis(date);
            if (increment == -1) {
