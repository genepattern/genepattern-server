...
public abstract class JobBean {
	...
	public class JobResultsWrapper {
		...
		private int rootJobNumber;

		public JobResultsWrapper(JobInfo jobInfo,
				Map<String, Collection<TaskInfo>> kindToModules,
				Set<String> selectedFiles, Set<String> selectedJobs) 
		{
			this(jobInfo, kindToModules, selectedFiles, selectedJobs, 0, 0, jobInfo.getJobNumber());
		}
		
		public JobResultsWrapper(JobInfo jobInfo,
				Map<String, Collection<TaskInfo>> kindToModules,
				Set<String> selectedFiles, Set<String> selectedJobs,
				int level, int sequence, int rootJobNumber) {
			...
			this.rootJobNumber = rootJobNumber;
			...
			try {
				...
				for (JobInfo child : children) {
					childJobs.add(new JobResultsWrapper(child, kindToModules,
							selectedFiles, selectedJobs, childLevel, seq, getRootJobNumber()));
					seq++;
				}
			} catch (WebServiceException e) {
		...
		public int getRootJobNumber() {
			return rootJobNumber;
		}

		public void setRootJobNumber(int rootJobNumber) {
			this.rootJobNumber = rootJobNumber;
		}
