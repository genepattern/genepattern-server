...
public class HTMLPipelineView {
	...
	try {
	    ...
	    tmTasksByLSID = new org.genepattern.server.webservice.server.local.LocalAdminClient(userID)
		    .getTaskCatalogByLSID(tmCatalog);
	    writer.write("");
	} catch (Exception ioe) {
	    ioe.printStackTrace();
	}
	...
	writer.write("var thisLSIDNoVersion = lsid.authority + '" + LSID.DELIMITER + "' + lsid.identifier + '"
		+ LSID.DELIMITER + "';\n");
	...
	// writer
	// .write("&nbsp;&nbsp;&nbsp; <select name=\"changePipeline\"
	// onChange=\"window.location='pipelineDesigner.jsp?name=' +
	...
	// writer
	// .write(" document.writeln('<option value=\"' + pipelineLSID + '\"' + ((pipelineName == thisTaskName ||
	// pipelineLSID
	// == thisTaskName || key == thisLSIDNoVersion) ? ' selected' : '') + ' class=\"tasks-' + lsid.authorityType +
	// '\">' +
	// pipelineName.substring(0, pipelineName.lastIndexOf('.')) + '</option>');\n");
...
//			+ GPConstants.NAME + "=' + this.options[this.selectedIndex].value\">\n");
//		writer.write(versionSelector(taskInfo));
//		writer.write("</select>\n");
//	    } else {
...
//	}
	...
	// writer
	// .write("<input type=\"submit\" value=\"Delete...\" name=\"delete\" onclick=\"return deletePipeline()\"
	// class=\"little\">\n");
	// writer
	// .write("<input type=\"submit\" value=\"Clone...\" name=\"clone\" onclick=\"return clonePipeline()\"
	// class=\"little\">\n");
	// }
	writer.write("</td></tr>\n");
