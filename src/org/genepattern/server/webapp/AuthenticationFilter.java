...
public class AuthenticationFilter implements Filter {
	...
	private static final String[] NO_AUTH_REQUIRED_PAGES = {
			"getPipelineModel.jsp", "retrieveResults.jsp", "getFile.jsp",
			"getInputFile.jsp", "login.jsp", "login.jsf", "registerUser.jsf",
			"forgotPassword.jsf" };

	/** Forward to home page if logged in user requests these pages */
	private static final String[] LOGIN_PAGES = { "login.jsf",
			"registerUser.jsf", "forgotPassword.jsf" };
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if (!fqHostName.equalsIgnoreCase(serverName)) {
			redirectToFullyQualifiedHostName((HttpServletRequest) request,
		...
		if (isAuthenticated((HttpServletRequest) request, (HttpServletResponse) response)) {
			...
			setLoginPageRedirect((HttpServletRequest) request,
					(HttpServletResponse) response);
		}
	}
	...
	public void setLoginPageRedirect(HttpServletRequest request,
			HttpServletResponse response) {
		...
		try {
			...
			String basePath = request.getScheme() + "://" + fqHostName + ":"
	...
	public void redirectToFullyQualifiedHostName(HttpServletRequest request,
			HttpServletResponse response) {
		try {
			String fqHostName = System.getProperty("fullyQualifiedHostName");
			if (fqHostName == null) {
			...
			String fqAddress = request.getScheme() + "://" + fqHostName + ":"
