...
// END SSL
	...
	protected void startIndexerDaemon(Properties props) throws ServletException {
		...
		if (!findThreadByName(daemonName)) {
			...
			try {
				Thread tIndexerDaemon = new Thread(IndexerDaemon.getDaemon(),
						daemonName);
	...
	protected void startJSPCompiler(Properties props, ServletContext application) {
		...
		log("starting JSP precompiler, will notify " + webmaster
				+ " of any errors");
		Thread jspPrecompiler = new Thread(new JSPPrecompiler(application
				.getRealPath("/"), props.getProperty("GenePatternURL"),
				webmaster), "JSPPrecompiler");
	...
	protected void announceReady(Properties props) {
		String message = "GenePattern server version "
				+ System.getProperty("GenePatternVersion") + " build "
				+ System.getProperty("build.tag") + " built "
	...
	public void destroy() {
		...
		for (Enumeration eThreads = vThreads.elements(); eThreads
				.hasMoreElements();) {
	...
	protected void launchTasks() {
		...
		StringTokenizer stTasks = new StringTokenizer(props.getProperty(
				"launchTasks", ""), ",");
		...
		nextTask: while (stTasks.hasMoreTokens()) {
			try {
				...
				String expectedThreadName = props.getProperty(taskName
						+ ".taskName", taskName);
				...
				StringTokenizer classPathElements = new StringTokenizer(
						classPath, ";");
				...
				while (classPathElements.hasMoreTokens()) {
					classPathURLs[i++] = new File(classPathElements.nextToken())
							.toURL();
				...
				String userDir = props.getProperty(taskName + ".dir", System
				...
				if (theClass == null) {
					throw new ClassNotFoundException("unable to find class "
							+ className + " using classpath "
							+ classPathElements);
				}
				Method main = theClass.getMethod("main",
						new Class[] { String[].class });
				//		log("found main, creating LaunchThread");
				LaunchThread thread = new LaunchThread(taskName, main,
						argsArray, this);
	...
	protected void loadProperties(ServletConfig config) throws ServletException {
		...
		try {
			...
			System.setProperty("serverInfo", config.getServletContext()
					.getServerInfo());
			...
			for (Iterator iProps = tmProps.keySet().iterator(); iProps
					.hasNext();) {
			...
			throw new ServletException(path + " cannot be loaded.  "
					+ ioe.getMessage());
		} finally {
			try {
				if (fis != null)
		...
		if (config.getServletContext().getServerInfo().indexOf("Apache Tomcat") != -1) {
			try {
				...
				if (hmProps.containsKey("docBase")) {
					props.setProperty("GP_docBase", (String) hmProps
							.get("docBase"));
				}

			} catch (Exception e) {
				System.err.println(e.getMessage()
						+ " in StartupServlet.parseTomcatServerXml");
			}
		} else {
			// unknown server
		}
	...
	protected void dumpThreads() {
		...
		for (int i = 0; i < numThreads; i++) {
			...
			if (!t.isAlive())
				continue;
			log(t.getName() + " is running at " + t.getPriority()
					+ " priority.  " + (t.isDaemon() ? "Is" : "Is not")
					+ " daemon.  " + (t.isInterrupted() ? "Is" : "Is not")
					+ " interrupted.  ");
		}
...
class LaunchThread extends Thread {
	...
	public LaunchThread(String taskName, Method mainMethod, String[] args,
			StartupServlet parent) {
		super(taskName);
	...
	public void run() {
		try {
			parent.log("invoking " + mainMethod.getDeclaringClass().getName()
					+ "." + mainMethod.getName());
			...
			parent.log("InvocationTargetException for " + getName(), ite
					.getTargetException());
		} catch (Exception e) {
