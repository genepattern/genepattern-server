...
/**
 *  Generate MATLAB code to form a pipeline of GenePatternAnalysis tasks, complete
 *  with
 *  <ul>
...
public class MATLABPipelineCodeGenerator extends AbstractPipelineCodeGenerator {
	 ...
	 */
	public MATLABPipelineCodeGenerator(PipelineModel model, String serverName, int serverPort, String invokingURL, Collection tmTasks) {
	...
	public String emitProlog() throws GenePatternException {
		...
		prolog.append("\n% regenerate with: ");
		prolog.append(getBaseURL() + "getPipelineCode.jsp?language=MATLAB&name=" + model.getLsid());
			...
			for(int i = 0; i < parameterInfo.length; i++) {
				if(parameterInfo != null) {
					...
					if(jobSubmission.getRuntimePrompt()[i]) {
						prompts.add("input('Enter " + parameterInfo[i].getName().replace('.', ' ') + " for task " + taskName + ": ','s')");
					}
				}
			}
		}
		...
		prolog.append("gpServer =  GenePatternServer('" + server + "', '" + model.getUserID() + "');\n");
	...
	public String emitTask(JobSubmission jobSubmission, TaskInfo taskInfo, ParameterInfo[] actualParams, int taskNum) throws GenePatternException {
		...
		invocation.append( "\n"+paramVar + " = getMethodParameters(gpServer,'"+lsid +"');\n" );		
		...
		if(formalParams != null) {
			for(int i = 0; i < formalParams.length; i++) {
				...
				if(jobSubmission.getRuntimePrompt()[i]) {
					invocation.append("\n"+ paramVar +"." + matlabEncodeName(formal.getName()) + "= prompts" + numPrompts + ";\n");
					numPrompts++;
					...
					try {
						fname = String.valueOf(Integer.parseInt(fname));	// 1st index is 1st output
		...
		//XXX - file from tasklib				
							invocation.append(paramVar +"." + matlabEncodeName(formal.getName()) + "= getTaskFileURL(gpServer, '" + task + "', '" + file + "');");
						} else {
		...
		if(!jobSubmission.isVisualizer()) {
			...
			invocation.append("results" + numNonVisualizersSeen + " = runAnalysis(gpServer,'");
