...
public class Indexer {
    ...
    // TODO: Need to write a custom Lucene Analyzer and Tokenizer to handle these files.
	...
	if (indexDir.exists() && new File(indexDir, "segments").exists()) return;
	...
	// Should probably ignore floating point numbers, use punctuation and spaces as separators, ignore single-letter words
	indexDir.mkdirs();
    ...
    public static void indexJob(IndexWriter writer, JobInfo jobInfo, AnalysisJobDataSource ds) throws IOException, OmnigeneException {
	synchronized (getConcurrencyLock()) {
		...
		try {
			if (taskID == -1) {
				taskInfo = GenePatternAnalysisTask.getTaskInfo(jobInfo.getInputFileName(), null);
			} else {
		...
		if (taskID != -1) {
			try {
				lsid = new LSID((String)taskInfo.getTaskInfoAttributes().get(GPConstants.LSID));
		...
		if (params != null) {
			for (int p = 0; p < params.length; p++) {
				...
				if (params[p].isOutputFile()) { // && waitFileExists(params[p].getValue())) {
					hasOutput = true;
				}
			}
		}
		doc.add(Field.Text(JOB_PARAMETERS, content.toString()));
		if (hasOutput) doc.add(Field.Text(JOB_HAS_OUTPUT, "1"));
...
/*
					boolean bAlreadyDone = false;
					for (int p2 = 0; p2 < p; p2++) {
						if (params[p2].isOutputFile() && filename.equals(params[p2].getValue())) {
							bAlreadyDone = true;
							break;
						}
					}
					if (bAlreadyDone) continue;
*/					
					doc = new Document();
					...
					try {
						...
						doc.add(Field.UnIndexed(URL, "retrieveResults.jsp?job=" + childJobNumber + "&filename=" + URLEncoder.encode(f.getName(), "UTF-8")));
	...
	out.println("Indexing user manual and tutorial");
        File[] docs = new File("webapps/gp/docs").listFiles(new FilenameFilter() {
	...
	synchronized (getConcurrencyLock()) {
		...
		ParameterInfo[] parameterInfoArray = new ParameterFormatConverter().getParameterInfoArray(ti.getParameterInfo());
		...
		// index documentation files for this task
		    ...
		    try {
				...
				cls = Class.forName(indexerClassName);
			    	...
			    	//out.println("no indexer class found for files of type " + getExtension(filename) + ": " + filename + ", using text indexer.");
...
/*
				...
				out.println(fld.name() + "=" + fld.stringValue());
			    }
*/
			    doc.add(Field.UnIndexed(FILENAME, filename));
		            doc.add(Field.Keyword(TASKNAME, name));
			    doc.add(Field.Text(TYPE, "doc"));
			    doc.add(Field.UnIndexed(URL, "getTaskDoc.jsp?" + GPConstants.NAME + "=" + lsid + "&file=" + URLEncoder.encode(filename, "UTF-8")));
			    doc.add(Field.Text(TASKID, "" + ti.getID()));
			    doc.add(Field.Text(LSID, lsid));
		            writer.addDocument(doc);
		    } catch (Throwable t) {
	...
	String name = "";
	try { name = f.getCanonicalPath(); } catch (IOException ioe) {}
