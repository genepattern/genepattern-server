...
public class IndexerDaemon implements Runnable {
	...
	public static void main(String[] args) {
		try {
			System.setProperty("genepattern.properties", System.getProperty(
	...
	public IndexerDaemon() throws IOException {
		...
		FSDirectory.getDirectory(indexDir, false).makeLock(
	...
	public void run() {
		...
		try {
			...
			if (DEBUG)
				System.out.println("IndexerDaemon running on "
						+ Thread.currentThread().getName() + " at priority "
			...
			while (true) {
						...
						while (rs.next()) {
							synchronized (Indexer.getConcurrencyLock()) {
								...
								try {
									...
									ds
											.executeUpdate(taskIndexedUpdate
													+ taskID);
									didWork = true;
								} catch (Throwable t) {
									System.err
											.println(t
													+ " in IndexerDaemon.run while processing task "
													+ taskID);
									t.printStackTrace();
								}
						...
						while (rs.next()) {
							synchronized (Indexer.getConcurrencyLock()) {
								...
								try {
									...
									System.err
											.println(t
													+ " in IndexerDaemon.run while processing job "
													+ jobID);
									t.printStackTrace();
								}
								writer = Indexer.releaseWriter();
				...
				synchronized (indexLock) {
					if (DEBUG)
						System.out
								.println("IndexerDaemon: waiting for notification on lock "
										+ indexLock.hashCode());
					indexLock.wait();
					if (DEBUG)
						System.out
	...
	protected static void notifyDaemon() {
		if ("true".equalsIgnoreCase(System.getProperty("disable.gp.indexing")))
			return;
