...
public class JobInputFileUtil {
     ...
     * @return
     */
    public static GpFilePath createTmpDir(final GpConfig gpConfig, final GpContext context) throws Exception {
        if (context==null) {
    ...
    public JobInputFileUtil(final GpConfig gpConfig, final GpContext jobContext) throws GpFilePathException, IOException {
    ...
    throws GpFilePathException, IOException 
    {
    ...
    private String initUploadPath(final String uploadPath) throws GpFilePathException, IOException {
        if (uploadPath == null) {
            ...
            boolean success=tmpFile.delete();
            if (!success) {
                throw new GpFilePathException("Unable to create uplodate directory for job, couldn't delete the tmpFile: "+tmpFile.getPath());
            }
            success=tmpFile.mkdirs();
            if (!success) {
                throw new GpFilePathException("Unable to create upload directory for job: "+tmpFile.getPath());
            }
     ...
     * @param relativePath
     */
    public static void addUploadFileToDb(final HibernateSessionManager mgr, final GpConfig gpConfig, final GpContext context, final GpFilePath gpFilePath) throws Exception {
        if (!(gpFilePath instanceof UserUploadFile)) {
            throw new IllegalArgumentException("Expecting a GpFilePath instance of type UserUploadFile");
        }
