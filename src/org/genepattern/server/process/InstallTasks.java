...
public class InstallTasks extends CommandLineAction implements FilenameFilter {
	...
	/**
	  * args[0]: directory in which to find zip files
	  * assumes resources directory is at same level, containing genepattern.properties
	  */
	...
	public boolean accept(File dir, String name) {
		...
		if (authToExclude == null) return isZip;

		// filter further, look inside zip for manifest. read lsid and do not accept
	...
	public void run(String[] args) {
	    if (args.length < 1) {
		System.err.println("usage: InstallTasks [dir where modules are stored, peer to resources directory]");
		return;
	    }
		...
		if (moduleList == null || moduleList.length == 0) {
			System.err.println("There are no modules meeting the criteria to install in " + moduleDir.getCanonicalPath());
			return;
		}
		...
		for (int i = 0; i < moduleList.length; i++) {
			...
			String lsid = gp.installNewTask(module.getCanonicalPath(), "GenePattern", GPConstants.ACCESS_PUBLIC);
		}
