...
public class ZipTask extends CommandLineAction {
	...
	if (tia == null) {
		throw new Exception("Sorry, " + name + " is not a GenePattern analysis task and cannot be packaged.");
	}
	...
	if (parameterInfo != null) {
		...
		for (i = 0; i < parameterInfo.length; i++) {
			...
			if (parameterInfo[i].getDescription() != null) props.setProperty("p" + i2 + "_description", parameterInfo[i].getDescription());
			if (parameterInfo[i].getValue() != null) props.setProperty("p" + i2 + "_value", parameterInfo[i].getValue());
...
try {
	...
	if (numRead != fileLength) {
		throw new Exception("only read " + numRead + " of " + fileLength + " bytes in " + f.getPath());
	}
	zos.closeEntry();
	//System.out.println("zipTaskFile: zipped " + f.getAbsolutePath() + ", length=" + numRead);
} catch (Exception t) { t.printStackTrace(); throw t; }
    }
	...
	File dir = new File(GenePatternAnalysisTask.getTaskLibDir(name, (String)taskInfo.getTaskInfoAttributes().get(GPConstants.LSID), userID));
