...
public class ModuleRepository {
	...
	public InstallTask[] GetModules(String url) throws IOException,
		...
		try {
			...
			doc = DocumentBuilderFactory.newInstance().newDocumentBuilder()
					.parse(new InputSource(new InputStreamReader(is)));
						...
						if (c_elt.getTagName().equals(NODE_SITEMODULE)) {
								...
								if (c_childNode.getNodeType() == Node.ELEMENT_NODE) {
									...
									if (childElt.getTagName().equals(
										...
										manifest = valueNode.getNodeValue();
											...
											NODE_SUPPORTFILE)) {
										support_file = childElt
												.hasAttribute("url") ? childElt
												.getAttribute("url") : null;
							...
							for (int j = 0; j < motdNodes.getLength(); j++) {
								...
								if (c_node.getNodeType() == Node.ELEMENT_NODE) {
									...
									if (c_elt.getTagName().equals(
											NODE_MOTD_MESSAGE)) {
										motd_message = c_node.getChildNodes()
												.item(0).getNodeValue();
									} else if (c_elt.getTagName().equals(
											NODE_MOTD_URL)) {
										motd_url = c_node.getChildNodes().item(
												0).getNodeValue();
									} else if (c_elt.getTagName().equals(
											NODE_MOTD_URGENCY)) {
										try {
											motd_urgency = Integer
													.parseInt(c_node
															.getChildNodes()
															.item(0)
															.getNodeValue());
										...
										try {
											...
											try {
												motd_timestamp = new SimpleDateFormat(
														"dd-MMM-yyyy")
														.parse(
																c_node
																		.getChildNodes()
											...
											NODE_MOTD_LATESTSERVERVERSION)) {
										motd_latestServerVersion = c_node
												.getChildNodes().item(0)
												.getNodeValue();
									} else {
										System.out
												.println("Got non-MOTD node where motd expected: "
														+ c_elt.getTagName());
									}
								}
							}
