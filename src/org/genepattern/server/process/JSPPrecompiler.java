...
public class JSPPrecompiler implements Runnable {
	...
	public JSPPrecompiler(String jspDir, String urlBase, String webmaster) {
		if (DEBUG) System.out.println("JSPPrecompiler using " + jspDir + " and " + urlBase);
		this.jspDir = jspDir;
	...
	public void run() {
		...
		if (DEBUG) System.out.println("jspFilenames=" + jspFilenames + " for directory " + jspDir);
		...
		if (message.length() > 0) {
			...
			if (webmaster == null) {
				System.err.println("No webmaster to notify regarding problem in JSP compilation: " + message.toString());
			} else {
				String hostName = "";
				try {
				...
				try {
					...
					if (DEBUG) System.out.println("notifying webmaster");
					String utf8 = "UTF-8";
					URL sendMail = new URL(urlBase + 
							"sendMail.jsp?to=" + URLEncoder.encode(to, utf8) +
							"&from=" + URLEncoder.encode(from, utf8) + 
							"&subject=" + URLEncoder.encode(subject, utf8) + 
							"&mimeType=" + URLEncoder.encode("text/plain", utf8) +
							"&message=" + URLEncoder.encode(message.toString(), utf8));
	...
	protected void oneDirectory(String[] jspFilenames, String urlBase) {
		...
		for (int i = 0; i < jspFilenames.length; i++) {
			...
			if (DEBUG) System.out.println("Precompiling " + urlString);
...
//					BUG: response code and message don't seem to work with the
//					?pre_compile request
					if (DEBUG) {
				...
				if (!(e instanceof java.io.FileNotFoundException))
					errorMessage = errorMessage + e.getMessage() + " while precompiling.  ";
...
/*
	TODO: knowing that there are errors, attempt to figure out which is the latest
	.java file for this .jsp file, compile it with javac and read the Process.inputStream
	to pipe the errors from the compiler into the error message text.
*/			
			}
