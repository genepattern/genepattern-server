...
public class JSPPrecompiler implements Runnable {
    ...
    private static Logger log = Logger.getLogger(JSPPrecompiler.class);
    ...
    public JSPPrecompiler(String jspDir, String urlBase, String webmaster) {
        if (log.isDebugEnabled()) log.debug("JSPPrecompiler using " + jspDir + " and " + urlBase);
        this.jspDir = jspDir;
    ...
    public void run() {
        if (jspDir == null || urlBase == null) return;
        log.info("compiling JSPs");
            ...
            jspFilenames = new File(jspDir).list(new FilenameFilter() {
                ...
                public boolean accept(File dir, String name) {
                    ...
                    if (possibleDirectory.isDirectory()) {
                        oneDirectory(possibleDirectory.list(new FilenameFilter() {
                            // INNER CLASS !!!
                            public boolean accept(File dir, String name) {
        ...
        if (log.isDebugEnabled()) log.debug("jspFilenames=" + jspFilenames + " for directory " + jspDir);
        ...
        if (message.length() > 0) {
            ...
            if (webmaster == null) {
                System.err
                        .println("No webmaster to notify regarding problem in JSP compilation: " + message.toString());
            }
            else {
                String hostName = "";
                try {
                ...
                try {
                    String to = webmaster;
                    String from = webmaster + "(GenePattern JSP precompiler on " + hostName + ")";
                    String subject = numErrors + " JSP precompilation problem" + (numErrors > 1 ? "s" : "") + " on "
                            + hostName;
                    if (log.isDebugEnabled()) log.debug("notifying webmaster");
                    String utf8 = "UTF-8";
                    URL sendMail = new URL(urlBase + "sendMail.jsp?to=" + URLEncoder.encode(to, utf8) + "&from="
                            + URLEncoder.encode(from, utf8) + "&subject=" + URLEncoder.encode(subject, utf8)
                            + "&mimeType=" + URLEncoder.encode("text/plain", utf8) + "&message="
                            + URLEncoder.encode(message.toString(), utf8));

                    BufferedReader in = new BufferedReader(new InputStreamReader(sendMail.openStream()));
                    ...
                    log.error(e + " - unable to email webmaster");
                }
            }
        }
        log.info("JSPPrecompiler done.");
    }
    ...
    protected void oneDirectory(String[] jspFilenames, String urlBase) {
        ...
        for (int i = 0; i < jspFilenames.length; i++) {
            ...
            if (log.isDebugEnabled()) log.debug("Precompiling " + urlString);
            ...
            try {
                ...
                do {
                    ...
                    if (log.isDebugEnabled()) {
                        ...
                        log.debug(jspFilenames[i] + " compiled");
                    }
                    break;
                }
                while (true);
                ...
                String errorMessage = "\n" + new Date() + ": ";
                if (!(e instanceof java.io.FileNotFoundException)) errorMessage = errorMessage + e.getMessage()
                        + " while precompiling.  ";
                if (!(e instanceof java.net.ConnectException)) errorMessage = errorMessage + "Probable syntax error.";
    ...
    public static void main(String[] args) {
        ...
        try {
            JSPPrecompiler jspc = new JSPPrecompiler("/local/omnigene_analysis_engine/Jetty-4.2.2/webapps/gp",
