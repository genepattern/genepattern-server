...
public class Purger extends TimerTask {
	...
	public void run() {
		if (purgeInterval != -1) {
				...
				System.out.println("Purger: purging jobs completed before " + gcPurgeDate.getTime());
				...
				for (int jobNum = 0; jobNum < purgeableJobs.length; jobNum++) {
					...
					// TODO: figure out which input files to purge.  This is hard, because an input file could be shared among
					// numerous jobs, some of which are not old enough to purge yet
...
//				System.out.println(moreFiles[i].getName() + ": " + new Date(moreFiles[i].lastModified()).toString());
...
//						System.out.println("Purger: deleting pipeline " + moreFiles[i]);
						...
						if (files != null) {
							for (int j = 0; j < files.length; j++) {
								...
								files[j].delete();
							        ...
							        System.err.println("unable to delete " + files[j].getPath());
							    }
							}
						}
					}
					try {
						moreFiles[i].delete();
					        ...
					        System.err.println("unable to delete " + moreFiles[i].getPath());
					}
				    } catch (SecurityException se) {
				        System.err.println("unable to browse " + moreFiles[i].getPath());
				    }
				}
			}
		}
