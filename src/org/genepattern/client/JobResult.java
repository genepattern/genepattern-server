...
public class JobResult {
	 ...
	 *
	 *@param  server      The server on which this job was run.
	 *@param  jobNumber   The job number of this job.
	 *@param  fileNames   The array of file names that this job created.
	 *@param  stdout      Whether a standard out file was created.
	 *@param  stderr      Whether a standard err file was created.
	 ...
	 *  this job created excluding the standard output file and the standard error
	 *  file.
	 *
	...
	/**
	 *  Returns the url to download the file that was created in the given creation
	 *  order
	 *
	 *@param  creationOrder  The file creation order, starting at 0.
	 *@return                The url to retrieve the file from.
    *@throws IllegalArgumentException if <code>creationOrder < 0</code> or <code>creatorOrder >= getOutputFileNames().length</code>
	 */
	public URL getURL(int creationOrder) {
		try {
			...
			return new URL(getServerURL() + "/gp/retrieveResults.jsp?job=" + getJobNumber() + "&filename=" + URLEncoder.encode(fileNames[creationOrder], "UTF-8"));
	...
	public URL getURL(String fileName) {
		try {
			return new URL(getServerURL() + "/gp/retrieveResults.jsp?job=" + getJobNumber() + "&filename=" + URLEncoder.encode(fileName, "UTF-8"));
		} catch(MalformedURLException x) {
			throw new Error(x);
		} catch(java.io.UnsupportedEncodingException uee) {
...
/**
	 ...
	 *  download directory. If the directory
	 *  already exists and contains files of the same name, it will overwrite the
	 *  files.
	 *
	 *@param  downloadDirectory      The pathname of a directory to create the
	 *      files. The directory will be created if it does not exist.
	 *@return                        The array of files.
	 *@throws  IOException           If an error occurs while downloading the
	 *      files.
	 *@throws  NullPointerException  If the <code>downloadDirectory</code> argument
	 *      is <code>null</code>
	 */
	 ...
	 *  download directory.  If the directory
	 *  already exists and contains files of the same name, it will overwrite the
	 *  files if <code>overwrite</code> is <code>true</code>, and if <true>false</code> it will prepend job_#_ to the file
	 *  name of any preexisting file.
	 ...
	 *  The name of the downloaded file will be equal to the given file name. If
	 *  a file of the same name already exists in the given directory it will be
	 *  overwritten.
	 *
	 *@param  fileName               The file name.
	 *@param  downloadDirectory      The pathname of a directory to create the
	 *      file. The directory will be created if it does not exist.
	 *@return                        The file or <code>null</code> if the file does
	 *      not exist on the server.
	 *@throws  IOException           If an error occurs while downloading the file.
	 *@throws  NullPointerException  If the <code>fileName</code> or <code>downloadDirectory</code>
	...
	/**
	 *  Downloads the result file from the server to the given download directory.
	 *  The name of the downloaded file will be equal to the given file name. If overwrite
	 *  is fale, if a file already existis of the same name in the given directory then
	 *  the file will be written with job_#_ prefixing the filename.  If a file of this name
