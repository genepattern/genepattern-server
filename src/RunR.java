...
/**
 * RunR is a simple interface that translates a command line into something that the R 
 * interpreter can handle.  The goal is to start R running, have it read in a script,
 * and then begin execution of a particular method in the script with specified arguments.
 * R is not natively capable of doing this.  So the RunR class invokes R, then feeds it
 * a series of commands via the standard input stream and copies the results to its own
...
public class RunR extends Thread {
	...
	/**
	 * Invoke the R interpreter, create a few lines of input to feed to the stdin input stream of R,
	 * and spawn two threads that copy stdout and stderr from R to this process' version of the same.
	 *
	 * @param args[0] 	job ID (gives R self-awareness and provides a filename for the stdout output file)
	 * @param args[1]	R script file to source
	 * @param args[2]	method to invoke (usually defined within script file or referring to something within it)
	 * @param args[3]	beginning of zero or more arguments to the method being invoked
	 *
	 * @author Jim Lerner
	 */
	public static void main(String[] args) {
		...
		if (args.length < MIN_ARGS) {
			System.err.println("Insufficient arguments to RunR.  Must have job_id, source filename, method name, and optional args.");
			System.exit(0);
		}
	...
	public RunR(String[] args) {
		...
		*/
		...
		try {
			...
			/* 
			XXX: this code assumes a file structure like:
			GenePatternServer
				resources/
				Tomcat/
					temp/
						[job]10/ <- your are here, running a job in this directory
			*/

			String resourceDir = System.getProperty("resources", ".." + File.separator + ".." + File.separator + ".." + File.separator + "resources");
			...
			try {
				...
				props.load(fis);
				GenePatternURL = props.getProperty("GenePatternURL", "http://127.0.0.1:8080/gp");
				if (GenePatternURL.endsWith("/")) GenePatternURL = GenePatternURL.substring(0, GenePatternURL.length() -"/".length());
				if (GenePatternURL.endsWith("/gp")) GenePatternURL = GenePatternURL.substring(0, GenePatternURL.length()-"/gp".length());
			} catch (IOException ioe) {
				throw new IOException(propFile.getAbsolutePath() + " cannot be loaded.  " + ioe.getMessage());
			} finally {
				try {
			...
			if (DEBUG) {
				System.setOut(new PrintStream(new FileOutputStream(args[JOBID] + ".html")));
			}
			...
			if (DEBUG) {
	                        System.out.println("<html><head><title>job " + args[JOBID] + ": " + 
					(args[R_METHOD] != null ? args[R_METHOD] : args[R_SOURCE]) + "</title>");
			...
			sendCmd(")\n");
			// move all of the output files to *my* directory so that they will appear to have come from me
			...
			sendCmd("			newName <- file.path(lastTaskDir, ifelse(is.null(names(files[i])) || nchar(names(files[i])) == \"\", files[[i]], substring(names(files[i]), " + (".server.".length() + 1) + ")))\n");
			// output files get associated with this jobID here (in case they are in some other sandbox)
			sendCmd("			close(url(paste(\"" + GenePatternURL + "/gp/updatePipelineStatus.jsp?jobID=" + args[JOBID] + "&name=\", basename(f), \"&filename=\", f, sep=\"\")))\n");
			sendCmd("		} else {\n");
			sendCmd("			newName <- file.path(lastTaskDir, ifelse(is.null(names(files[i])) || nchar(names(files[i])) == \"\", class(files[[i]]), names(files[i])))\n");
			//sendCmd("			cat(\"saving\", names(files[i]), \", class\", class(files[[i]]), \") as\", newName, \"\\\n\")\n");
			sendCmd("			foo <- files[[i]]\n");
			...
			sendCmd("}\n");
			sendCmd("q(save=\"no\")\n");
                        if (DEBUG) System.out.println("</pre>");
			stdin.close();
			System.out.flush();
			...
			System.err.println(ioe + " while running R command " + commandLine.toString());
		} catch (InterruptedException ie) {
			System.err.println(ie + " while running R command " + commandLine.toString());
		}
	}
	...
	// Runnable for streamCopier thread
		...
		while((line = in.readLine())!=null){
			...
			bNeedsBreak = (line.length() > 0 && (line.indexOf("<") == -1 || line.indexOf("<-") != -1));
			os.print(line);
			if (bNeedsBreak) {
				if (DEBUG) {
					os.println("<br>");
