...
def randomsleep(lower_limit=1, upper_limit = 5):
    """
    >> randomsleep [<lower_limit>] [<upper_limit>]

    Sleep for a random time between the interval:
    [lower_limit, upper_limit). If no interval is given, 
    sleep for a random time between 1 and 5 seconds.
    """
    import random

    if float(lower_limit) >= float(upper_limit):
        print 'ERROR: lower_limit must be strictly less than upper_limit'
    else:
        random.seed(time.time())
        randnum = (float(upper_limit)-float(lower_limit))*random.random()+float(lower_limit)
        print 'Sleeping for %s seconds' % randnum
        time.sleep(randnum)
...
def submit(submit_button=None, formnum=None):
    """
    >> submit [<buttonspec>]
    ...
    """
    browser.submit(submit_button, formnum)
...
def timer(operation = None):
    """
    >> timer <summary>

    Time multiple sets of commands by using timer before and after each code block.
    The <summary> parameter is optional.  Leave blank to start/stop    
    
    Set summary equal to
        1. 'print' to get statistics on the total response times
        2. 'clear' to reset the timer and statistics

    """
    if operation is None:
        browser.timer.set_time()
    elif operation == 'print':
        browser.timer.print_stats()
    elif operation == 'clear':
        browser.timer.clear_timer()
    elif operation == 'log':
        browser.timer.logstats()
    else: 
        print 'ERROR: Unknown operation for timer'
