...
class TwillBrowser(object):
    ...
    def submit(self, fieldname=None, formnum=None):
        """
        ...
        if form is None:
            ...
            if len(forms) == 1:
                ...
                if formnum is not None:
                    input = formnum
                else:
                    input = raw_input("multiple forms, enter the desired form number: ")
		input = int(input)
		if input < 0:
	            raise Exception("form number must be a postive integer >= 1")
                form = forms[int(input) -1]
