...
then
    ...
    sed s+__PPWWDD__+$PWD+ ./resources/config_custom.yaml > ./resources/config_custom.yaml
    # and backup for old one with the developers bath
    sed s+/Users/liefeld/GenePattern/gp_dev/genepattern-server/docker+$PWD+ ./resources/config_custom.yaml > ./resources/config_custom.yaml
